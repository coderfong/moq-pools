...
export default function AdminAlertsClient({ initialAlerts }: { initialAlerts: any[] }) {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      const read = get('read'); if (read === 'unread' || read === 'read') setReadFilter(read as any);
      const tri = get('triage'); if (tri === 'open' || tri === 'resolved' || tri === 'archived') setTriageFilter(tri as any);
      const asg = get('assignee');
      if (asg) {
        if (asg === 'assigned' || asg === 'unassigned' || asg === 'me') setAssigneeFilter(asg as any);
        else if (asg.startsWith('id:')) { setAssigneeFilter('id'); setAssigneeIdFilter(asg.slice(3)); }
      }
      if (get('priority') === '1') setShowPriority(true);
      const qq = get('q'); if (qq) setQ(qq);
      const df = get('from'); if (df) setDateFrom(df);
      const dt = get('to'); if (dt) setDateTo(dt);
      const ps = parseInt(get('pageSize') || '0', 10); if (ps >= 1 && ps <= 200) setPageSize(ps);
      const pg = parseInt(get('page') || '0', 10); if (pg >= 1) setPage(pg);
      const sb = get('sortBy'); if (sb === 'timestamp' || sb === 'priority' || sb === 'triage' || sb === 'status') setSortBy(sb as any);
      const sd = get('sortDir'); if (sd === 'asc' || sd === 'desc') setSortDir(sd as any);
    } finally {
  ...
  // Keep URL in sync with filter/sort/pagination state (omit defaults)
  React.useEffect(() => {
    if (!initializedFromUrl.current) return;
    const p = new URLSearchParams();
    if (page !== 1) p.set('page', String(page));
    if (pageSize !== 50) p.set('pageSize', String(pageSize));
    if (readFilter !== 'all') p.set('read', readFilter);
    if (triageFilter !== 'all') p.set('triage', triageFilter);
    if (assigneeFilter === 'id' && assigneeIdFilter) p.set('assignee', `id:${assigneeIdFilter}`);
    else if (assigneeFilter !== 'all') p.set('assignee', assigneeFilter);
    if (showPriority) p.set('priority', '1');
    if (q) p.set('q', q);
    if (dateFrom) p.set('from', dateFrom);
    if (dateTo) p.set('to', dateTo);
    if (sortBy !== 'timestamp') p.set('sortBy', sortBy);
    if (sortDir !== 'desc') p.set('sortDir', sortDir);
    const qs = p.toString();
    router.replace(qs ? `${pathname}?${qs}` : pathname, { scroll: false });
  }, [router, pathname, page, pageSize, readFilter, triageFilter, assigneeFilter, assigneeIdFilter, showPriority, q, dateFrom, dateTo, sortBy, sortDir]);
  ...
  async function updateAlert(patch: { id: string, title?: string, body?: string, link?: string|null, adminNotes?: string, priority?: boolean, assigneeId?: string|null, triageStatus?: 'OPEN'|'RESOLVED'|'ARCHIVED' }) {
    setBusy(true);
    try {
        ...
        <table className="w-full text-sm">
          <thead className="bg-neutral-50 text-neutral-600">
            <tr>
              ...
              <th className="p-2">
                <button className="inline-flex items-center gap-1 hover:underline" onClick={() => { if (sortBy==='timestamp') setSortDir(d=>d==='asc'?'desc':'asc'); else { setSortBy('timestamp'); setSortDir('desc'); } }}>
                  When {sortBy==='timestamp' ? (<span>{sortDir==='asc'?'▲':'▼'}</span>) : null}
                </button>
              </th>
              <th className="p-2">
                <button className="inline-flex items-center gap-1 hover:underline" onClick={() => { if (sortBy==='status') setSortDir(d=>d==='asc'?'desc':'asc'); else { setSortBy('status'); setSortDir('desc'); } }}>
                  Read {sortBy==='status' ? (<span>{sortDir==='asc'?'▲':'▼'}</span>) : null}
                </button>
              </th>
              <th className="p-2">
                <button className="inline-flex items-center gap-1 hover:underline" onClick={() => { if (sortBy==='triage') setSortDir(d=>d==='asc'?'desc':'asc'); else { setSortBy('triage'); setSortDir('desc'); } }}>
                  Triage {sortBy==='triage' ? (<span>{sortDir==='asc'?'▲':'▼'}</span>) : null}
                </button>
              </th>
              <th className="p-2">
                <button className="inline-flex items-center gap-1 hover:underline" onClick={() => { if (sortBy==='priority') setSortDir(d=>d==='asc'?'desc':'asc'); else { setSortBy('priority'); setSortDir('desc'); } }}>
                  Priority {sortBy==='priority' ? (<span>{sortDir==='asc'?'▲':'▼'}</span>) : null}
                </button>
              </th>
