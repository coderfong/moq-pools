...
export default function InformationPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.log('Submitting data:', { countryCode: country.code, companyName, individualBuyer, fullName, mobile: normalizedMobile });
      ...
      if (!res.ok || !json?.ok) {
        ...
        const errorMsg = reason === "unauthorized"
            ? "Please sign in again."
            : reason === "invalid_mobile"
            ? "Enter a valid mobile number."
            : `Failed to submit information. Reason: ${reason}`;
        console.error('Submission failed:', errorMsg, json);
        setError(errorMsg);
        setSubmitting(false);
        return;
      }
  ...
  <div className="order-1 lg:order-2 lg:col-span-7 xl:col-span-6 px-3 sm:px-6 md:px-8 py-3 md:py-4 self-center justify-self-center w-full max-w-[540px] transform translate-x-2 sm:translate-x-8 md:translate-x-12">
          <header className="mb-4 sm:mb-6">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-display font-bold text-center bg-gradient-to-r from-white via-orange-100 to-amber-100 bg-clip-text text-transparent drop-shadow-lg">Complete your profile</h1>
            <p className="text-sm sm:text-base text-white/90 mt-1 sm:mt-2 text-center font-medium drop-shadow">Just a few more details to get started.</p>
          </header>

          <form onSubmit={onSubmit} className="bg-gradient-to-br from-white/95 to-orange-50/90 dark:bg-gradient-to-br dark:from-gray-900/80 dark:to-gray-800/70 backdrop-blur-xl rounded-2xl sm:rounded-3xl border-2 border-orange-200/50 p-4 sm:p-6 md:p-8 space-y-3 sm:space-y-4 shadow-2xl">
            {/* Country/Region (Combobox) */}
            <div ref={comboRef} className="relative">
              <label htmlFor="country-combobox" className="block text-xs sm:text-sm font-semibold text-gray-900 mb-1">Country/Region<span className="text-red-600 ml-1">*</span></label>
              <input
                id="country-combobox"
                role="combobox"
            ...
            {/* Mobile Number */}
            <div>
              <label className="block text-xs sm:text-sm font-semibold text-gray-900 mb-1">Mobile Number<span className="text-red-600 ml-1">*</span></label>
            ...
            {error && (
              <p className="text-xs sm:text-sm text-red-600 font-semibold bg-red-50 border-2 border-red-200 rounded-lg p-2 sm:p-3" role="alert">
                ⚠️ {error}
              </p>
            )}

            <div className="pt-1 sm:pt-2">
              <button type="submit" disabled={submitting} className="w-full inline-flex items-center justify-center rounded-full bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 sm:px-8 py-2.5 sm:py-3 text-sm sm:text-base font-bold hover:from-orange-600 hover:to-orange-700 disabled:opacity-60 transition-all shadow-lg hover:shadow-xl hover:shadow-orange-500/30 hover:scale-105 duration-300">
                {submitting ? (
                  <>
