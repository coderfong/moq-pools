...
export async function POST(req: Request) {
  try {
    ...
    // Validate SMTP config/auth early to surface issues
    try {
      ...
      return NextResponse.json(
        { ok: false, reason: "smtp_config_invalid", error: String(err?.message || err) },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ ok: false, reason: "server_error", error: String(e?.message || e) }, { status: 500 });
  }
}