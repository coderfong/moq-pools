...
export default async function OrderTrackingPage() {
  ...
  try {
    ...
    if (me) {
      // Check unread messages - skip for now as Message model structure is different
      unreadMessagesCount = 0;

      // Check unread alerts
      unreadAlertsCount = await prisma.alert.count({
        where: {
          userId: me.id,
          status: 'UNREAD'
        }
      ...
      // Fetch pool items
      console.log('[TRACKING PAGE] Fetching pool items for user:', me.id);
      ...
      console.log('[TRACKING PAGE] Found pool items:', poolItems.length);
    }
  } catch (e: any) {
    hasDb = false;
    console.error('[TRACKING PAGE] Failed to fetch pool items:', e);
  }
