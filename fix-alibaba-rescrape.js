 ...
 * Uses the /api/rescrape endpoint that has the proven detail.ts logic
 */
...
// Use the API endpoint instead of direct import
  ...
  for (let i = 0; i < listings.length; i += batchSize) {
    ...
    for (const listing of batch) {
      try {
        ...
        // Use API endpoint with existing scraper infrastructure
        const apiUrl = `${API_URL}?id=${listing.id}&force=1&headless=1`;
        ...
        if (!result.success || !result.data?.heroImage) {
          console.log(`  ⚠️  No heroImage found: ${result.error || 'unknown error'}`);
          failedCount++;
          continue;
        }

        const heroImage = result.data.heroImage;
        ...
        await prisma.savedListing.update({
          ...
          data: { 
            ...
            detailJson: result.data,
          },
        });

        console.log(`  ✅ Updated: ${heroImage.slice(0, 70)}...`);
        successCount++;
