...
async function main() {
  ...
  for (let i = 0; i < listings.length; i++) {
    ...
    console.log(`Current image: ${listing.image || 'NONE'}`);
    ...
    const result = await scrapeDetailPage(listing.url);
    ...
    // Update database with new primary image
    try {
      ...
      console.log(`  ✅ Updated image (total ${result.allImages.length} found)`);
      console.log(`     ${result.primaryImage.substring(0, 100)}...`);
      fixed++;
    } catch (error) {
