...
async function fixAllMadeInChina() {
  ...
  while (offset < needFixCount && !isShuttingDown) {
    ...
    for (let i = 0; i < batch.length; i += MAX_CONCURRENT_REQUESTS) {
      ...
      // If we hit rate limits, back off before next chunk
      if (rateLimitHit && i + MAX_CONCURRENT_REQUESTS < batch.length) {
        const rateLimitCount = chunkResults.filter(r => r.type === 'ratelimit').length;
        console.log(`⏳ Rate limits detected (${rateLimitCount}), backing off ${RATE_LIMIT_BACKOFF / 1000}s...`);
