...
export default async function Products({ searchParams }: { searchParams: { platform?: string, q?: string } }) {
  ...
  // Debug ensure groupBy shape (remove after verifying)
  const groupByArgs = { by: ['sourcePlatform'] as any, where: { isActive: true }, _count: { _all: true } };
  // eslint-disable-next-line no-console
  console.log('DEBUG groupByArgs', groupByArgs);
  ...
  const [items, counts] = await Promise.all([
    ...
    prisma.product.groupBy(groupByArgs),
  ]);
