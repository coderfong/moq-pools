...
function price(x: any) { return x?.toString?.() ?? String(x); }
...
export default async function Products({ searchParams }: { searchParams: { platform?: string, q?: string } }) {
  ...
  // Fetch your products + pools and counts
  ...
  // Sort by "near fulfillment" first (desc by pledged/target)
  ...
  // Tab counts
  const countMap: Record<string, number> = counts.reduce((acc, row) => {
    acc[row.sourcePlatform as string] = row._count._all;
    return acc;
  }, {} as Record<string, number>);
  countMap['__all'] = Object.values(countMap).reduce((a, b) => a + b, 0);

  // External explore URL for this tab
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      {/* Your pools/products (near-MOQ first) */}
      <div className="mt-3">
        <h2 className="text-sm font-semibold text-gray-700 mb-2">Pools on {platform === 'ALL' ? 'All Platforms' : platformLabel(platform)}</h2>
        {sorted.length === 0 ? (
                ...
                <Link key={p.id} href={`/p/${p.pool?.id ?? ''}`} className="border rounded-xl p-3 hover:shadow-sm transition">
                  <div className="aspect-video bg-gray-100 rounded-lg mb-3 overflow-hidden">
                    {(() => {
                      ...
                      const src = imgs?.[0];
                      return src ? <img src={src} alt={p.title} className="w-full h-full object-cover" /> : null;
                    })()}
                  </div>
      ...
      {platform !== 'ALL' && (
        <div className="mt-6 border rounded-xl p-4 bg-gray-50">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Search opens the official site with your query. Pick items to curate into pools.
          </p>
        </div>
      )}
    </div>
