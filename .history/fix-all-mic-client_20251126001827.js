...
const MAX_CONCURRENT_REQUESTS = 2; // Reduced to 2 when running both scripts simultaneously
const RATE_LIMIT_BACKOFF = 15000; // 15 seconds backoff on rate limit (increased for dual runs)
const INTER_CHUNK_DELAY = 500; // 500ms between chunks within a batch
...
async function fixAllMadeInChina() {
  ...
  while (offset < needFixCount && !isShuttingDown) {
    ...
    // Adaptive rate limit handling - back off more aggressively when running multiple scripts
    if (rateLimitCount > batch.length * 0.5) {
      console.log(`⚠️  Heavy rate limiting (${rateLimitCount}/${batch.length}), backing off ${RATE_LIMIT_BACKOFF / 1000}s...`);
      await new Promise(resolve => setTimeout(resolve, RATE_LIMIT_BACKOFF));
    } else if (rateLimitCount > batch.length * 0.3) {
      console.log(`⏳ Moderate rate limiting (${rateLimitCount}/${batch.length}), short pause 5s...`);
      await new Promise(resolve => setTimeout(resolve, 5000));
    } else if (rateLimitCount > 0) {
      console.log(`⚡ ${rateLimitCount} rate limits in batch (continuing)`);
    }
