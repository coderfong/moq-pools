...
Fixed the OAuth token exchange by switching from the **API gateway** to the **OAuth server**. The previous code was calling `https://gw.api.alibaba.com/openapi/...` which caused `gw.AppKeyNotFound` errors. Now correctly using `https://openapi-auth.alibaba.com/oauth/token` for standard OAuth2 token exchange.
