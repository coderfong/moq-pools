...
const DELAY_BETWEEN_BATCHES = 500; // 500ms between batches (much faster!)
...
const MAX_CONCURRENT_REQUESTS = 10; // Max parallel API calls
          ...
          if (quality === 'good') {
            ...
            if (debugSource && debugSource.includes('fallback')) {
              if (consecutiveFails >= MAX_CONSECUTIVE_FALLBACKS) {
                ...
                saveProgress({ offset: offset + i + 1, good, partial, bad, errors, consecutiveFails });
                
                // Sleep and reset counter
                await new Promise(resolve => setTimeout(resolve, BLOCK_COOLDOWN_MS));
                consecutiveFails = 0; // Reset and keep going
