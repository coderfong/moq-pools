...
export async function middleware(req: NextRequest) {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    if (!globalLimiter.tryRemoveTokens(1)) {
      console.warn(`🚨 RATE LIMITED (global): ${clientIp} - ${pathname}`);
      return new NextResponse(
        JSON.stringify({ error: 'Too many requests globally' }), 
        { 
          status: 429,
          headers: { 
    ...
    if (!ipLimiter.tryRemoveTokens(1)) {
      console.warn(`🚨 RATE LIMITED (IP): ${clientIp} - ${pathname}`);
      return new NextResponse(
