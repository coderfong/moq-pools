    ...
    for (const listing of batch) {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (newAttrs >= 10) {
            console.log(`    ✅ Fixed! Now has ${newAttrs} attributes (source: ${debugSource})`);
            good++;
            consecutiveFails = 0;
          } else if (newAttrs > 0) {
            console.log(`    ⚠️  Partial: ${newAttrs} attributes (source: ${debugSource})`);
            partial++;
            consecutiveFails = 0;
          } else {
            console.log(`    ❌ Still 0 attributes (source: ${debugSource})`);
            bad++;
            consecutiveFails++;
          }
        } else {
