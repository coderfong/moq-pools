 ...
 * Uses the proven detail.ts scraping logic
 */
...
// Import the detail scraping function
let scrapeProductDetail;

async function loadDetailScraper() {
  try {
    // Try to load compiled version
    const mod = await import('./src/lib/providers/detail.js');
    scrapeProductDetail = mod.scrapeProductDetail;
    return true;
  } catch (e1) {
    try {
      // Try ts-node
      ...
      const mod = require('./src/lib/providers/detail.ts');
      scrapeProductDetail = mod.scrapeProductDetail;
      return true;
    } catch (e2) {
      console.error('❌ Could not load detail scraper');
      console.error('Please run: pnpm build or ensure ts-node is available');
      return false;
    }
  }
}
