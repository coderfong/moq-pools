...
## Webhook: POST /api/webhooks/carrier

Normalize carrier/3PL callbacks into a single endpoint.

- Security: require `CARRIER_WEBHOOK_SECRET` via either:
...
```
{
  ...
  "status": "in_transit|out_for_delivery|delivered|exception|label_created",
  "event": { "ts": "2025-10-25T12:40:00Z", "loc": "Hong Kong, CN", "desc": "Departed facility" }
}
```

Behavior:
- Find Shipment by `trackingNumber`.
- Append to `eventsJson` (time/location/message/status).
- Recompute `Shipment.status` (maps OOD→IN_TRANSIT, delivered→DELIVERED, exception→EXCEPTION).
- Recompute ETA using `computeDynamicEta` and set `etaDate`.
- Emit alerts using `emitShipmentAlertsOnUpdate` (pickup/OFD) and `DELIVERED` event for delivered.
- Create a message into user's conversation and send push; deep link to `/orders/:orderId/track`.
- Publish an SSE event `shipment_update` to the owner if connected to `/api/events`.

## Real-time updates (SSE)

- Endpoint: `GET /api/events` (Node runtime). Requires session; streams only your own events.
- Events:
  - `hello` — initial handshake
  - `shipment_update` — payload `{ orderId, trackingNumber, status, event: { ts, loc, desc, carrier }, link }`
- Client example:

```
const es = new EventSource('/api/events');
es.addEventListener('shipment_update', (e) => {
  const data = JSON.parse(e.data);
  // refresh tracking UI for data.orderId
});
```

Implementation: `app/api/webhooks/carrier/route.ts`.
...
## Dev: Simulate Shipment Alerts

Endpoint: `/api/dev/shipments/simulate` (non-production only; requires session)

- GET: `?poolItemId=<id>` — returns shipments for the order.
- POST JSON body:

```
{ "poolItemId": "<id>", "action": "pickup|out_for_delivery|delivered", "trackingNo": "YT...", "location": "Optional City, CC" }
```

What it does:
- `pickup`: sets status to IN_TRANSIT and adds a pickup event → emits “Picked up by courier.”
- `out_for_delivery`: adds OOD event → emits “Arriving today.”
- `delivered`: sets status to DELIVERED and adds event → emits “Delivered — rate your order.”

File: `app/api/dev/shipments/simulate/route.ts`.
