  ...
  const byAttributes = await prisma.$queryRaw`
    ...
    ORDER BY 
      CASE 
        ...
        WHEN jsonb_array_length(COALESCE("detailJson"->'attributes', '[]'::jsonb)) BETWEEN 1 AND 4 THEN 3
        WHEN jsonb_array_length(COALESCE("detailJson"->'attributes', '[]'::jsonb)) BETWEEN 5 AND 9 THEN 4
        WHEN jsonb_array_length(COALESCE("detailJson"->'attributes', '[]'::jsonb)) BETWEEN 10 AND 14 THEN 5
        WHEN jsonb_array_length(COALESCE("detailJson"->'attributes', '[]'::jsonb)) BETWEEN 15 AND 19 THEN 6
        ELSE 7
      END
  `;
