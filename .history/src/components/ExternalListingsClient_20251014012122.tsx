...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  {rowsForUI.map((it) => {
          ...
          const moqLabel = moqMin && moqMin > 1 ? `≥${moqMin.toLocaleString()}` : '';
          let isAlibaba = false;
          let isMIC = false;
          try {
            ...
            <a key={it.url} href={hrefInternal} target={isIndiaMart ? '_self' : '_blank'} rel={isIndiaMart ? undefined : 'noopener'} className="border rounded-xl p-3 hover:shadow transition bg-white">
              ...
              {moqMin && moqMin > 1 ? (
                <div className="mt-2">
                  ...
                  <TieredPriceProgress tiers={[{ min: moqMin, priceText: '', label: moqLabel }]} currentQty={0} />
                </div>
              ) : (() => {
                ...
                return (
                  <div className="mt-2">
                    <div className="flex items-center justify-between text-sm text-gray-700 dark:text-gray-300">
                      <span>{(typeof qMin === 'number' && qMin > 1) ? `≥${qMin.toLocaleString()}` : range}</span>
                      <span>0%</span>
                    </div>
