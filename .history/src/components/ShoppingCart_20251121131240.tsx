...
export default function ShoppingCartComponent({ className = '' }: ShoppingCartProps) {
  ...
  // Calculate subtotal - extract numeric price from string if possible
  const subtotal = cartItems.reduce((sum, item) => {
    // Try to extract a number from the price string
    const priceMatch = item.productPrice.match(/[\d,]+(?:\.\d{1,2})?/);
                ...
                <div className="flex gap-4">
                  ...
                  <div className="relative w-24 h-24 flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden">
                    <Image
                      ...
                      unoptimized={item.productImage.startsWith('/cache/')}
                    />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mt-1">{item.platform || 'Unknown Platform'}</p>
                      </div>
                      <button
                    ...
                    <div className="mt-3 text-xs text-gray-600 bg-orange-50 px-3 py-2 rounded-lg">
                      Pool MOQ: {item.productMoq} units
                    </div>
                  </div>
                </div>
              </div>
