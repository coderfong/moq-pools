...
function NavbarContent({ scrolled, progress }: { scrolled: boolean; progress: number }) {
  ...
  return (
    <>
      <header className={`sticky top-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-2xl shadow-xl border-b border-orange-100' : 'bg-white/80 backdrop-blur-xl shadow-md'}`}>
        {/* Reading progress bar */}
        <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-orange-200 to-amber-200">
          <div 
            className="h-full bg-gradient-to-r from-orange-500 via-orange-600 to-amber-500 transition-all duration-300 ease-out"
            style={{ width: `${progress * 100}%` }}
          />
        </div>
        
        <div className={`w-full max-w-full px-4 md:px-6 lg:px-8 h-16 md:h-20 flex items-center justify-between`}>
          {/* Enhanced Brand */}
          <Link href="/" className="flex items-center gap-2 md:gap-3 group z-50">
            <div className="relative">
              <span className="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-500 via-orange-600 to-amber-500 flex items-center justify-center shadow-lg group-hover:shadow-2xl transition-all duration-300 group-hover:scale-105">
          ...
          <button
            ...
            className="lg:hidden z-50 p-2 rounded-lg hover:bg-orange-50 transition-colors"
          ...
          {/* Enhanced Primary nav with hover underline effect - Desktop Only */}
          <nav className="hidden lg:flex items-center gap-4 xl:gap-6 text-sm xl:text-base font-semibold mx-auto">
            ...
            <Link href="/how-it-works" className="relative px-2 xl:px-3 py-2 text-gray-700 hover:text-orange-600 transition-colors whitespace-nowrap group">
              How It Works
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-orange-500 to-amber-500 scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></span>
            </Link>
            <Link href="/#benefits" className="relative px-2 xl:px-3 py-2 text-gray-700 hover:text-orange-600 transition-colors group">
              Benefits
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-orange-500 to-amber-500 scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></span>
            </Link>
            <Link href="/about" className="relative px-2 xl:px-3 py-2 text-gray-700 hover:text-orange-600 transition-colors group">
              About
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-orange-500 to-amber-500 scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></span>
            </Link>
            <Link href="/support" className="relative px-2 xl:px-3 py-2 text-gray-700 hover:text-orange-600 transition-colors group">
              Support
              <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-orange-500 to-amber-500 scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></span>
            </Link>
          </nav>

          {/* Enhanced Auth area (right) - Desktop Only */}
      ...
      {/* Mobile Menu Overlay */}
      <div className={`lg:hidden fixed inset-0 z-40 bg-black/50 backdrop-blur-sm transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setMobileMenuOpen(false)}>
        <div className={`fixed top-0 right-0 bottom-0 w-4/5 max-w-sm bg-white shadow-2xl transition-transform duration-300 ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={(e) => e.stopPropagation()}>
          <div className="flex flex-col h-full overflow-y-auto">
            ...
            <nav className="flex flex-col p-4 space-y-2">
              <Link href="/products" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors" onClick={() => setMobileMenuOpen(false)}>
                Browse Products
              </Link>
              <Link href="/how-it-works" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors" onClick={() => setMobileMenuOpen(false)}>
                How It Works
              </Link>
              <Link href="/#benefits" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors" onClick={() => setMobileMenuOpen(false)}>
                Benefits
              </Link>
              <Link href="/about" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors" onClick={() => setMobileMenuOpen(false)}>
                About
              </Link>
              <Link href="/support" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors" onClick={() => setMobileMenuOpen(false)}>
                Support
              </Link>
              <div className="h-px bg-gray-200 my-2"></div>
              <Link href="/cart" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors flex items-center gap-2" onClick={() => setMobileMenuOpen(false)}>
              ...
              <Link href="/account/wishlist" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors flex items-center gap-2" onClick={() => setMobileMenuOpen(false)}>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/>
                </svg>
                Wishlist
              </Link>
              <Link href="/account/messages" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors flex items-center gap-2" onClick={() => setMobileMenuOpen(false)}>
                Messages
              </Link>
              <Link href="/account" className="px-4 py-3 rounded-lg text-gray-700 hover:bg-orange-50 hover:text-orange-600 font-medium transition-colors flex items-center gap-2" onClick={() => setMobileMenuOpen(false)}>
                My Account
              </Link>
            </nav>
          </div>
...
function NotificationBell() {
  ...
  // Real-time notifications via WebSocket - TEMPORARILY DISABLED
  // Issue: Session cookie not accessible from client-side
  // Using polling fallback (60 seconds) for now
  /*
  const wsStatus = useWebSocket({
    ...
    onConnect: () => {
      if (process.env.NODE_ENV !== 'production') {
        console.log('✅ WebSocket connected! Status:', status);
      }
      if (status === 'authenticated') {
        if (process.env.NODE_ENV !== 'production') {
          console.log('✅ WebSocket connected for real-time notifications');
        }
        // Fetch current notifications on connect
        fetchNotifications();
      }
    },
    onDisconnect: () => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log('[NotificationBell] WebSocket status:', wsStatus.status, 'Auth status:', status);
  }
  */
          ...
          <div className="max-h-96 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-gray-500">
                ...
                <p className="text-xs text-gray-500">We'll notify you when there's something new</p>
              </div>
            ) : (
              notifications.map((notification) => (
                <div
                  key={notification.id}
                  className="p-4 hover:bg-orange-50 cursor-pointer border-b border-gray-100 transition-colors"
                  onClick={async () => {
                    // Mark as read first
                ...
                >
                  <div className="flex items-start gap-3">
                    {!notification.read && <div className="w-2 h-2 rounded-full bg-orange-600 mt-2 flex-shrink-0" />}
                    {notification.read && <div className="w-2 h-2 rounded-full bg-gray-300 mt-2 flex-shrink-0" />}
