...
import { IndiaMartCategoryMenu } from '@/components/IndiaMartCategoryMenu';
...
export default function CategoryDropdown({ platform, currentQuery }: { platform: string; currentQuery?: string }) {
  ...
  return (
    <div ref={rootRef} className="relative inline-block text-left">
      ...
      {/* Desktop mega menu */}
      {open && platform === 'INDIAMART' && (
        <div className="hidden md:block absolute z-20 mt-2 w-[min(90vw,760px)] max-h-[70vh] overflow-auto rounded-2xl border border-gray-200 bg-white/95 backdrop-blur shadow-xl p-3">
          <IndiaMartCategoryMenu
            onSelect={(leaf) => {
              // Navigate with first term
              const term = encodeURIComponent(leaf.terms[0]);
              router.push(`/products?platform=INDIAMART&q=${term}&per=50&page=1&imLeaf=${leaf.key}`);
              setOpen(false);
            }}
          />
        </div>
        ...
        <div className={`absolute inset-x-0 bottom-0 bg-white rounded-t-2xl shadow-2xl p-4 transition-transform duration-200 ${open ? 'translate-y-0' : 'translate-y-full'}`} role="dialog" aria-modal="true">
          ...
          {platform === 'INDIAMART' ? (
            <div className="max-h-[60vh] overflow-auto">
              <IndiaMartCategoryMenu
                onSelect={(leaf) => {
                  router.push(`/products?platform=INDIAMART&q=${encodeURIComponent(leaf.terms[0])}&per=50&page=1&imLeaf=${leaf.key}`);
                  setOpen(false);
                }}
              />
            </div>
