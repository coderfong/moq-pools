  ...
  return (
    <div className="container card elev reveal" id="tiered-pricing">
      ...
      <div className="mt-4 w-full">
        <div className="relative h-2 w-full overflow-hidden rounded-full border border-gray-200 bg-gray-100" role="progressbar" aria-valuemin={0} aria-valuemax={end || 0} aria-valuenow={qty} aria-label="Order quantity toward best price tier">
          <div className="absolute inset-y-0 left-0 bg-black transition-[width] duration-200 ease-out" style={{ width: `${percent.toFixed(1)}%` }} />
        </div>
        <div className="mt-2 flex justify-between text-[12px] text-gray-500 w-full">
          <span>0</span>
          {tiers.map((t, i) => (
            <span key={`stop-${i}`}>{t.max === "inf" || t.max === undefined ? (tiers[tiers.length - 1]?.min || 0).toLocaleString() : Number(t.max).toLocaleString()}</span>
          ))}
          <span>End</span>
        </div>
      </div>
