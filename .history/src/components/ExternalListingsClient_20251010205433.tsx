...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  // Pick the first non-seed/non-bad image from: resolved -> fallback -> raw
  const pickBestNonSeed = (row: ExternalListing): string | undefined => {
    const urlKey = String(row.url || '');
    const resolved = imgMap?.[urlKey];
    const title = computeDisplayTitle(row);
    const fallback = resolveFallbackImage(String(row.image || ''), row.title || title, row.description || '');
    const candidates = [resolved, fallback, row.image].filter(Boolean) as string[];
    return candidates.find((u) => !isSeedOrBad(u));
  };
            ...
            <a key={it.url} href={hrefInternal} target={isIndiaMart ? '_self' : '_blank'} rel={isIndiaMart ? undefined : 'noopener'} className="border rounded-xl p-3 hover:shadow transition bg-white">
              <div className="aspect-square bg-gray-100 rounded-md mb-3 overflow-hidden">
                {(() => {
                  ...
                  const candidate = derivedDetail && !isSeedOrBad(derivedDetail)
                    ? derivedDetail
                    : (safeImg ?? rawImg);
                  if (candidate && !isSeedOrBad(candidate)) {
                    return <img src={candidate} alt={it.title} className="w-full h-full object-cover" loading="lazy" />;
                  }
                  // Show a small skeleton while we resolve a real image (never render sleeves)
