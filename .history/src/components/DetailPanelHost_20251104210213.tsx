...
function ProductDetailsSidebar({ 
  ...
  const keyAttributes = useMemo(() => {
    ...
    if (extractedAttributes && extractedAttributes.length > 0) {
      console.log(`[DetailPanelHost] Using ${extractedAttributes.length} extracted attributes`);
  ...
  const fetchAttributes = async (url: string) => {
    try {
      ...
      const resp = await fetch(
        `${base}/api/products/extract-attributes?url=${encodeURIComponent(url)}`,
        { cache: 'no-store' }
      );
      ...
      if (resp.ok) {
        ...
        console.log(`Extracted ${attrs.length} attributes from ${url}`);
      }
    } catch (error) {
