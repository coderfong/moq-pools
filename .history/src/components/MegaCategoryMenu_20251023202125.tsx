...
const ICON_KEY_ALIASES: Record<string, string> = {
  'home-and-living': 'home-garden',
  'fashion-and-apparel': 'apparel-accessories',
  ...
  'household-and-cleaning': 'personal-home-care',
...
export default function MegaCategoryMenu({ onSelect, categories = SHARED_CATEGORIES }: Props) {
  ...
  return (
    <div className="flex gap-3 p-3 md:h-[60vh] md:overflow-hidden">
      {/* Left rail */}
      <nav className="w-64 shrink-0 border-r border-gray-100 pr-2 md:h-full md:overflow-auto" aria-label="Categories">
      ...
      {/* Right content */}
      <div className="min-w-[520px] grow pl-1 md:h-full md:overflow-auto">
            ...
            >
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {active.children && active.children.map((sub) => (
                  <div key={sub.key} className="min-w-0 pb-4 mb-2 border-b border-gray-100 last:border-b-0 last:pb-0 last:mb-0 text-left">
                    <SubHeader nodeKey={sub.key} label={sub.label} onSelect={onSelect} />

                    {/* Direct leaves */}
                    {sub.leaves && sub.leaves.length > 0 && (
                      <ul className="flex flex-col items-start gap-1 mb-3 text-left">
                        {sub.leaves.map((lf) => (
                          <li key={lf.key} className="w-full"><LeafPill leaf={lf} onSelect={onSelect} /></li>
                        ))}
                      </ul>
                    )}
                        ...
                        {sub.children.map((subsub) => (
                          <div key={subsub.key} className="min-w-0">
                            ...
                            {subsub.leaves && subsub.leaves.length > 0 && (
                              <ul className="flex flex-col items-start gap-1 mt-1 text-left">
...
function SubHeader({ nodeKey, label, onSelect }: { nodeKey: string; label: string; onSelect: (leaf: SharedLeaf) => void }) {
  return (
    <button
      type="button"
      className="group flex w-full items-center gap-1.5 text-left text-sm font-semibold mb-2 text-gray-800 hover:text-orange-700 focus:text-orange-700 focus:outline-none"
      onClick={() => {
...
function SubSubHeader({ nodeKey, label, onSelect }: { nodeKey: string; label: string; onSelect: (leaf: SharedLeaf) => void }) {
  return (
    <button
      type="button"
        className="group flex w-full items-center gap-1 text-left text-xs font-medium text-gray-600 hover:text-orange-700 focus:text-orange-700 focus:outline-none"
      onClick={() => {
        const terms = getSharedSearchTerms(nodeKey);
        onSelect({ key: nodeKey, label, terms: terms.length ? terms : [label] });
      }}
...
function LeafPill({ leaf, onSelect }: { leaf: SharedLeaf; onSelect: (leaf: SharedLeaf) => void }) {
  ...
  return (
    <button
      ...
      className="block w-full text-left px-0 py-0 text-sm text-gray-700 hover:text-orange-700 focus:text-orange-700 transition-colors"
      title={title}
    >
      {leaf.label}
    </button>
