...
export default function CategoryDropdown({ platform, currentQuery }: { platform: string; currentQuery?: string }) {
  ...
  return (
    <div className="relative inline-block text-left">
      ...
      {open && (
        <div className="absolute z-20 mt-2 w-[min(90vw,960px)] max-h-[70vh] overflow-auto rounded-2xl border border-gray-200 bg-white/95 backdrop-blur shadow-xl p-3">
                ...
                {CATEGORIES.map((c, idx) => {
                  ...
                  return (
                    <li key={c.key}>
                      <button
                        type="button"
                        className={`w-full text-left text-sm px-3 py-2 rounded-lg ${isActive ? 'bg-gray-900 text-white' : 'hover:bg-gray-100'}`}
