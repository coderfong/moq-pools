...
export default function Progress({ value, min = 0, max = 100, className = '', label = 'Progress' }: { value: number; min?: number; max?: number; className?: string; label?: string }) {
  ...
  return (
    <div className={`h-2 w-full rounded-full bg-gray-200 overflow-hidden ${className}`.trim()}>
      <progress value={Math.round(pct)} max={100} aria-label={label} className="sr-only" />
