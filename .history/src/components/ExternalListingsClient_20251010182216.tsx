...
import { hasGoodImage, canonicalizeUrl } from '@/lib/imageUtils';
import TieredPriceProgress, { type PriceTier } from './TieredPriceProgress';
...
function cleanTitle(title: string) {
  return (title || '').replace(/\b(certified|verified|supplier)\b/ig, '').replace(/\s+/g, ' ').trim();
}
...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  const displayItems = useMemo(() => {
    const list = items || [];
    const bestByKey = new Map<string, ExternalListing>();
    ...
    const score = (x: ExternalListing) => {
      ...
      const derived = imgMap[urlKey];
      const chosen = derived && !isBadImageHashFromPath(derived)
        ? derived
        : resolveFallbackImage(String(x.image || ''), x.title || '', x.description || '') || String(x.image || '');
    ...
    for (const row of list) {
      if (!isIndiaMart(row)) {
        bestByKey.set(`OTHER|${row.url}`, row);
        continue;
      }
      const key = `IM|${canonicalizeUrl(row.url)}`;
      const prev = bestByKey.get(key);
      if (!prev) { bestByKey.set(key, row); continue; }
      if (score(row) > score(prev)) bestByKey.set(key, row);
    }

    // For non-IndiaMART, just include as-is
    const others = list.filter(l => !isIndiaMart(l));
    const ims = Array.from(bestByKey.entries()).filter(([k]) => k.startsWith('IM|')).map(([,v]) => v);
    const merged = [...others, ...ims];

    // Final pass: hide any remaining IndiaMART items that still point to seed sleeves
    return merged.filter((it) => {
      if (!isIndiaMart(it)) return true;
      const urlKey = String(it.url || '');
      const derivedDetail = imgMap[urlKey];
      if (derivedDetail && !isBadImageHashFromPath(derivedDetail)) return true;
      const rawImg = String(it.image || '');
      const title = computeDisplayTitle(it);
      const safeImg = resolveFallbackImage(rawImg, it.title || title, it.description || '');
      const isSleevesSeed = (/\/seed\/sleeves\.jpg$/i.test(String(safeImg || ''))) || (/\/seed\/sleeves\.jpg$/i.test(rawImg));
      return !isSleevesSeed;
    });
  }, [items, imgMap]);
  ...
  if (!toResolve.length && !upgradeQueue.length) return;
    (async () => {
      ...
      // Attempt upgrades for IndiaMART bad hash images via a (future) detail upgrade endpoint
    ...
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 mt-3">
        {displayItems.map((it) => {
          ...
          const tiersHeader = sortedTiers.length ? sortedTiers.map(t => t.label || (t.max != null ? `${t.min}-${t.max}` : `${t.min}+`)).join(' ') : '';
          let isAlibaba = false;
          try { const u = new URL(urlKey); isAlibaba = u.hostname.toLowerCase().includes('alibaba.com'); } catch {}
          ...
          return (
            <a key={it.url} href={hrefInternal} target={isIndiaMart ? '_self' : '_blank'} rel={isIndiaMart ? undefined : 'noopener'} className="border rounded-xl p-3 hover:shadow transition bg-white">
              <div className="aspect-square bg-gray-100 rounded-md mb-3 overflow-hidden">
                {(() => {
                  // Prefer a resolved detail image if we have one
                  const derivedDetail = imgMap[String(it.url || '')];
                  if (derivedDetail && !isBadImageHashFromPath(derivedDetail)) {
                    return <img src={derivedDetail} alt={it.title} className="w-full h-full object-cover" loading="lazy" />;
                  }
                  // Otherwise, use curated safe image if available (and not a bad hash)
                  if (safeImg && !isBadImageHashFromPath(safeImg)) {
                    return <img src={safeImg} alt={it.title} className="w-full h-full object-cover" loading="lazy" />;
                  }
                  // If we're in a transition (seed or empty), show upgrading indicator
                  if (!rawImg || rawImg.startsWith('/seed/')) {
                    return <div className="w-full h-full flex items-center justify-center text-[10px] text-gray-400">Upgrading…</div>;
                  }
                  // Last resort
                  return <div className="w-full h-full flex items-center justify-center text-[10px] text-gray-400">No Image</div>;
                })()}
              ...
              <div className="font-medium text-sm line-clamp-2 mt-0.5 text-gray-900 dark:text-gray-100">{title || 'See listing'}</div>
              <div className="mt-1 text-lg font-semibold text-orange-600">{it.price ? `${it.price} / unit` : 'See listing'}</div>
              {tiers.length ? (
                <div className="mt-2">
                  <div className="flex items-center justify-between text-xs text-gray-700 dark:text-gray-300">
                    <span className="truncate" title={tiersHeader}>{tiersHeader}</span>
                    <span>0%</span>
                  </div>
                  <TieredPriceProgress tiers={tiers} currentQty={0} />
                </div>
              ) : (() => {
              ...
              {/* Lazy-fetch detailed price tiers for Alibaba listings when not already available */}
              {isAlibaba && urlKey && !detailTiers[urlKey] ? (
  ...
  <div className="text-xs text-gray-500 dark:text-gray-400 mt-3">Showing {displayItems.length} listings</div>
