...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  // Resolve current batch of item images to local /cache paths
  useEffect(() => {
    let aborted = false;
    // 1) Direct external image URLs to cache
    const directImages = items
      .map((it) => String(it.image || ''))
      .filter((u) => u && !u.startsWith('/cache/') && !u.startsWith('/seed/') && !imgMap[u]);
          ...
          return (
            <a key={it.url} href={hrefInternal} target={isIndiaMart ? '_self' : '_blank'} rel={isIndiaMart ? undefined : 'noopener'} className="border rounded-xl p-3 hover:shadow transition bg-white">
              <div className="aspect-square bg-gray-100 rounded-md mb-3 overflow-hidden">
                {(() => {
                  // 1. Use curated safe image if available
                  if (safeImg) {
                    return <img src={safeImg} alt={it.title} className="w-full h-full object-cover" loading="lazy" />;
                  }
                  // 2. Try dynamic resolved mapping (detail-derived) if no curated fallback
                  const derived = imgMap[String(it.url || '')];
                  if (derived && !isBadImageHashFromPath(derived)) {
                    return <img src={derived} alt={it.title} className="w-full h-full object-cover" loading="lazy" />;
                  }
                  // 3. Last resort: nothing to show (optionally could render skeleton)
