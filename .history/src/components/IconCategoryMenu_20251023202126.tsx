              ...
              <li key={node.key}>
                <button
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (!total) return;
                    let next = activeIdx;
                    if (e.key === 'ArrowDown') { next = (activeIdx + 1) % total; e.preventDefault(); }
                    else if (e.key === 'ArrowUp') { next = (activeIdx - 1 + total) % total; e.preventDefault(); }
                    else if (e.key === 'Home') { next = 0; e.preventDefault(); }
                    else if (e.key === 'End') { next = total - 1; e.preventDefault(); }
                    if (next !== activeIdx) {
                      setActiveIdx(next);
                      // move focus to the new button for SR/keyboard users
                      setTimeout(() => {
                        const btn = document.querySelector(`[data-top-btn="${next}"]`) as HTMLButtonElement | null;
                        btn?.focus();
                      }, 0);
                    }
                  }}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {node.children.map((sub, i) => (
            <div key={sub.key} className="min-w-0 pb-4 mb-2 border-b border-gray-100 last:border-b-0 last:pb-0 last:mb-0">
              ...
              {/* If subgroup has direct leaves, show them first */}
              ...
              {/* If subgroup has further children (sub-sub), render each with a small section */}
...
function SubHeader({ nodeKey, label, onSelect }: { nodeKey: string; label: string; onSelect: (leaf: SharedLeaf) => void }) {
  return (
    <button
      ...
      className="group flex items-center gap-1.5 text-sm font-semibold mb-2 text-gray-800 hover:text-orange-700"
    >
      <span>{label}</span>
      <svg className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
    </button>
  );
}
...
function SubSubHeader({ nodeKey, label, onSelect }: { nodeKey: string; label: string; onSelect: (leaf: SharedLeaf) => void }) {
  return (
    <button
      ...
      className="group inline-flex items-center gap-1 text-xs font-medium text-gray-600 hover:text-orange-700"
    >
      <span>{label}</span>
      <svg className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
    </button>
  );
}
