      ...
      if (poolsRes.ok) {
        ...
        if (pools.length > 0) {
          const mapped = pools.map((pool: any) => {
            ...
            // Parse imagesJson if it's a string
          ...
          console.log('📦 Using pool data. First item:', {
            title: mapped[0]?.title,
            image: mapped[0]?.image?.substring(0, 50),
            poolId: mapped[0]?.poolId,
          });
          setFeed(mapped);
          setLoading(false);
          return;
        ...
        console.error('❌ Pools API failed:', poolsRes.status, poolsRes.statusText);
      }
      
      // Fallback: Get products from the products page
      console.log('🔍 Fetching products from /api/products...');
      const productsRes = await fetch('/api/products?limit=10', { cache: 'no-store' });
      if (productsRes.ok) {
        ...
        const mapped = products.map((product: any, idx: number) => {
          let images: string[] = [];
          ...
          const firstImage = images[0] || RANDOM_LISTING_IMAGES[idx % RANDOM_LISTING_IMAGES.length];
          // Mock progress for products without pools
        ...
        console.log('📦 Using product data. First item:', {
          title: mapped[0]?.title,
          image: mapped[0]?.image?.substring(0, 50),
          sourcePlatform: products[0]?.sourcePlatform,
        });
        setFeed(mapped);
      } else {
