...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  const displayItems = useMemo(() => {
    const isIM = (it: ExternalListing) => {
      if (String((it as any)?.platform || '').toUpperCase() === 'INDIAMART') return true;
      try { return new URL(String(it.url || '')).hostname.toLowerCase().includes('indiamart.com'); } catch { return false; }
    };
