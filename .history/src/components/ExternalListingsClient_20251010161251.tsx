...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  // Hide any listing (all platforms) that would still show the seed sleeves.jpg placeholder image
  const displayItems = useMemo(() => {
    return (items || []).filter((it) => {
      const urlKey = String(it.url || '');
      const derivedDetail = imgMap[urlKey];
      // If we have a resolved detail image and it's not a known bad hash, keep it
      ...
      // If either the curated safe image or the raw image points to the sleeves seed, hide it
      const isSleevesSeed = (/\/seed\/sleeves\.jpg$/i.test(String(safeImg || ''))) || (/\/seed\/sleeves\.jpg$/i.test(rawImg));
      if (isSleevesSeed) return false;

      // Otherwise, keep (including good /cache/ images and other non-seed fallbacks)
      return true;
    });
  }, [items, imgMap]);
