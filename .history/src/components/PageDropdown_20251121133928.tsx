...
export default function PageDropdown() {
  ...
  React.useEffect(() => {
    ...
    // Check if user is the specific admin (jonfong78@gmail.com)
    let active = true;
    (async () => {
      try {
        ...
        // Only show for jonfong78@gmail.com
        if (meData?.ok && meData?.user?.email === 'jonfong78@gmail.com') {
          setIsAdmin(true);
        } else {
          setIsAdmin(false);
          return; // Don't fetch routes if not admin
        }
      } catch {
        if (!active) return;
        setIsAdmin(false);
