...
function ProductDetailsSidebar({ 
  ...
  extractedAttributes?: Array<{ label: string; value: string }>;
}) {
  ...
  const keyAttributes = useMemo(() => {
    // Priority 1: Use extracted attributes from live scraping
    if (extractedAttributes && extractedAttributes.length > 0) {
      return extractedAttributes;
    }
    
    // Priority 2: Try detailJson.attributes array (stored in database)
    ...
    // Priority 3: Extract from common detailJson fields
    ...
    // Fallback: Add platform origin if nothing found
    if (attrs.length === 0) {
    ...
    return attrs;
  }, [detail, platform, extractedAttributes]);
      ...
      <div className="space-y-3">
        ...
        <section className="rounded-xl bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200 shadow-sm">
          ...
          <div className="p-4 space-y-3">
            ...
            <a href="http://localhost:3007/products" className="mt-3 w-full rounded-lg bg-gradient-to-r from-orange-500 to-orange-600 text-white text-base font-bold py-3 hover:from-orange-600 hover:to-orange-700 transition-all shadow-md hover:shadow-lg inline-block text-center">
              Join Pool & Save
            </a>
...
export default function DetailPanelHost() {
  ...
  // Helper to fetch attributes from live product page
  const fetchAttributes = async (url: string) => {
    try {
      const base = process.env.NEXT_PUBLIC_BASE_URL || '';
      const apiUrl = `${base}/api/products/extract-attributes?url=${encodeURIComponent(url)}`;
      console.log('[DetailPanelHost] Fetching attributes from:', apiUrl);
      ...
      if (resp.ok) {
        ...
        console.log('[DetailPanelHost] Received attributes:', attrs.length, attrs);
        attrsCache.current.set(url, attrs);
        setExtractedAttributes(attrs);
      } else {
        console.error('[DetailPanelHost] API returned error:', resp.status);
      }
    } catch (error) {
  ...
  const onOpen = useCallback(async (payload: OpenDetailPayload) => {
    ...
    if (passedData) {
      ...
      setLoading(false);
      // Still fetch attributes in background
      fetchAttributes(url);
      return;
    }
    ...
    try {
      ...
      if (cached) {
        ...
        setLoading(false);
        // Still fetch attributes if not cached
        if (!attrsCache.current.has(url)) {
          fetchAttributes(url);
        } else {
          setExtractedAttributes(attrsCache.current.get(url) || []);
        }
        return;
      }
      ...
      try {
        ...
        if (data?.product) {
          ...
          // Fetch attributes in background
          fetchAttributes(url);
        } else {
