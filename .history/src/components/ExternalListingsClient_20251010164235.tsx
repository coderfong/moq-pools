...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  // Display items as-is (do not drop listings due to placeholder images); image rendering below avoids seed/sleeves.jpg.
  const displayItems = useMemo(() => items, [items, imgMap]);
            ...
            <a key={it.url} href={hrefInternal} target={isIndiaMart ? '_self' : '_blank'} rel={isIndiaMart ? undefined : 'noopener'} className="border rounded-xl p-3 hover:shadow transition bg-white">
              <div className="aspect-square bg-gray-100 rounded-md mb-3 overflow-hidden">
                {(() => {
                  ...
                  // Otherwise, use curated safe image if available (and not a bad hash or specific sleeves seed)
                  const isSeedSleeves = !!(safeImg && /\/seed\/sleeves\.jpg$/i.test(safeImg));
                  if (safeImg && !isSeedSleeves && !isBadImageHashFromPath(safeImg)) {
                  ...
                  if (!rawImg || rawImg.startsWith('/seed/') || isSeedSleeves) {
