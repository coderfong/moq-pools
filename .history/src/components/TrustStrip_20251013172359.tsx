  ...
  return (
    <section className="w-screen max-w-none -mx-6 md:-mx-10 xl:-mx-16" data-reveal>
      <div className="px-6 md:px-10 xl:px-16 py-10 bg-surface border-t border-b border-hairline text-center">
        <div className="text-base md:text-lg font-semibold text-muted dark:text-gray-300">Sourced from</div>
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-10 md:gap-16 justify-items-center items-center">
          {items.map((x) => (
            <div key={x.name} className="flex flex-col items-center text-center">
              ...
              <div className="mt-2 text-lg md:text-xl font-semibold text-gray-900 dark:text-gray-100">{x.name}</div>
            </div>
          ))}
        </div>
      </div>
