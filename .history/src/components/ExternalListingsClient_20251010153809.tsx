...
import { isRenderableImage } from '@/lib/imageUtils';
...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  // Remove IndiaMART listings that would still show seed/bad images; keep others unchanged
  const displayItems = useMemo(() => {
    return (items || []).filter((it) => {
      const platform = String((it as any)?.platform || '').toUpperCase();
      if (platform === 'INDIAMART') {
        // Only render when image is truly renderable (handles Next proxy + bad-hash)
        return isRenderableImage(it.image);
      }
      return true;
    });
  }, [items]);
