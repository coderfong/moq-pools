...
function computeDisplayTitle(listing: ExternalListing): string {
  ...
  const stripSuffixes = (s: string) => (
    (s || '')
      // drop known file extensions if they somehow leaked into title
  ...
  const t1 = stripSuffixes(removeNoise(String(listing.title || '')));
  ...
  try {
    ...
    // Strip extensions and trailing numeric IDs often appended by marketplaces (e.g., -1600082383276.html)
    let seg = raw
      .replace(/\.(?:s?html?|php|asp|aspx)\b.*$/i, '') // drop extension and anything after
      .replace(/[-_]+/g, ' ');
    // Remove trailing long numeric token (keep meaningful numbers like 80gsm/A4 in the middle)
