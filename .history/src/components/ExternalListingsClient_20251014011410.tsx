...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  {rowsForUI.map((it) => {
          ...
          const fmtNum = (n: number | undefined) => (typeof n === 'number' && Number.isFinite(n) ? n.toLocaleString() : '');
          ...
          const tiersHeader = visibleTiers.length ? visibleTiers.map(cleanLabel).join(' ') : '';
          const currentTierLabel = visibleTiers.length ? cleanLabel(visibleTiers[0]) : '';
          let isAlibaba = false;
          let isMIC = false;
          try {
            ...
            <a key={it.url} href={hrefInternal} target={isIndiaMart ? '_self' : '_blank'} rel={isIndiaMart ? undefined : 'noopener'} className="border rounded-xl p-3 hover:shadow transition bg-white">
              ...
              {visibleTiers.length ? (
                <div className="mt-2">
                  ...
                  <TieredPriceProgress tiers={visibleTiers} currentQty={0} />
                </div>
              ) : (() => {
                ...
                const syntheticTiers: PriceTier[] = (typeof qMin === 'number' && qMin > 0)
                  ? [{ min: qMin, priceText: '', label: `≥${qMin.toLocaleString()}` }]
                  : [];
                return (
                  <div className="mt-2">
                    <div className="flex items-center justify-between text-sm text-gray-700 dark:text-gray-300">
                    ...
                    {syntheticTiers.length
                      ? <TieredPriceProgress tiers={syntheticTiers} currentQty={0} />
