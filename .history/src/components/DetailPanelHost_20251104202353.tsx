...
function ProductDetailsSidebar({ productData, userLocation }: { productData?: ProductData; userLocation?: UserLocation }) {
  ...
  const keyAttributes = useMemo(() => {
    ...
    // Extract from detailJson if available
    if (detail?.attributes && Array.isArray(detail.attributes)) {
      detail.attributes.forEach((attr: any) => {
        if (attr.label && attr.value) {
          attrs.push({ label: attr.label, value: attr.value });
        }
      });
    } else if (detail?.specifications) {
      // Try specifications object
      Object.entries(detail.specifications).forEach(([key, value]) => {
        if (value && typeof value === 'string') {
          const label = fieldMap[key] || key.replace(/([A-Z])/g, ' $1').trim();
          attrs.push({ label, value });
        }
      });
    } else {
      // Fallback: scan detailJson for common fields
      Object.entries(detail || {}).forEach(([key, value]) => {
        if (fieldMap[key] && value && typeof value === 'string') {
          attrs.push({ label: fieldMap[key], value });
        }
      });
    }

    // Add platform-specific standard attributes
