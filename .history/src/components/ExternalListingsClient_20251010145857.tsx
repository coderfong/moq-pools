...
export default function ExternalListingsClient({ platform, q, initialItems, batch = 100, max = 1200, minPrice, maxPrice, minMoq, maxMoq }: Props) {
  ...
  // Remove IndiaMART listings that would still show the seed sleeves.jpg image
  const displayItems = useMemo(() => {
    const isIM = (it: ExternalListing) => {
      const p = String((it as any)?.platform || '').toUpperCase();
      if (p.includes('INDIAMART')) return true;
      const u = String(it.url || '');
      // Match absolute, relative, or encoded references containing 'indiamart'
      if (/indiamart/i.test(u)) return true;
      try { return new URL(u).hostname.toLowerCase().includes('indiamart.com'); } catch { return false; }
    };
    return (items || []).filter((it) => {
      if (!isIM(it)) return true;
      const urlKey = String(it.url || '');
      const derivedDetail = imgMap[urlKey];
      // Keep if we have a resolved detail image and it's not a bad hash
      if (derivedDetail && !isBadImageHashFromPath(derivedDetail)) return true;
      const rawImg = String(it.image || '');
      // Keep if listing already has a good cached image
      if (rawImg.startsWith('/cache/') && !isBadImageHashFromPath(rawImg)) return true;
      // Otherwise hide (this covers seeds, empties, and any non-cached fallbacks)
      return false;
    });
  }, [items, imgMap]);
