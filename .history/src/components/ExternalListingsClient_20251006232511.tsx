          ...
          return (
            <a key={it.url} href={hrefInternal} target={isIndiaMart ? '_self' : '_blank'} rel={isIndiaMart ? undefined : 'noopener'} className="border rounded-xl p-3 hover:shadow transition bg-white">
              <div className="aspect-square bg-gray-100 rounded-md mb-3 overflow-hidden">
                {(() => {
                  const img = String(it.image || '');
                  // Prefer direct image if present and not a seed
                  if (img && !img.startsWith('/seed/')) {
                    const src = img.startsWith('/cache/') || img.startsWith('/seed/')
                      ? img
                      : (imgMap[img] || `/api/external/cache-img?src=${encodeURIComponent(img)}`);
                    return (
                      <img
                        src={src}
                        alt={it.title}
                  ...
                  // Otherwise, try a resolved image derived from the product URL (detail page scrape)
