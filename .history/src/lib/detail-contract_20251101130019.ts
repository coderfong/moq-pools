...
// Stricter "correctness" predicate used for audits/healing. Designed to match
// the "good page" bar: hero image, at least one tier, enough specs (attrs>=3 or packaging>=1),
// at least one protection card, and a supplier name.
export function isCorrect(d: NormalizedDetail | null | undefined): boolean {
  if (!d) return false;
  const hasHero = !!d.heroImage && d.heroImage.trim().length > 0;
  const hasTiers = Array.isArray(d.priceTiers) && d.priceTiers.length > 0;
  const hasSpecs = (Array.isArray(d.attributes) && d.attributes.length >= 3) || (Array.isArray(d.packaging) && d.packaging.length >= 1);
  const hasProtections = Array.isArray(d.protections) && d.protections.length > 0;
  const hasSupplier = !!d.supplier?.name && String(d.supplier.name).trim().length > 0;
  const hasTitle = !!d.title && d.title.trim().length > 0;
  return hasTitle && hasHero && hasTiers && hasSpecs && hasProtections && hasSupplier;
}

// Convenience alias matching the proposal; keep existing isWeakDetail for compatibility
export const isWeak = (d: NormalizedDetail | null | undefined): boolean => !isCorrect(d as any);
