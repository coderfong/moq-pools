...
export async function cacheExternalImage(rawSrc: string, opts?: { preferJpgForIndiaMart?: boolean }): Promise<{ localPath: string; absPath: string }> {
  ...
  // Block obvious Alibaba placeholder/logo/badge/trademark images before fetching
  if (url.hostname.includes('alicdn') || url.hostname.includes('alibaba')) {
    const ustr = url.toString();
    if (isAlibabaBadImageUrl(ustr)) {
      throw new Error('Bad placeholder image rejected');
    }
    // Extra URL keyword guardrails for non-product assets that occasionally slip through
    if (/(trademark|assurance|verified|cert|certificate|warranty|badge|sprite|favicon)/i.test(ustr)) {
      throw new Error('Non-product emblem image rejected');
    }
  }
  ...
  try {
    ...
    if (sharp) {
      ...
      // Use a slightly higher threshold to avoid caching 16-160px UI icons.
      if (w > 0 && h > 0) {
