      ...
      try {
        ...
        for (; pagesDone < maxPages && out.length < limit; pagesDone++) {
          ...
          // If we didn't get any new items, attempt to go to next page
                  const cookiesHk = cookieStr.split(';').map(s => s.trim()).filter(Boolean).map((kv) => {
                    const [name, ...rest] = kv.split('=');
                    return { name, value: rest.join('='), domain: '.globalsources.com.hk', path: '/', httpOnly: false } as any;
                  });
          if (out.length >= limit) break;
                  try { await context.addCookies(cookiesHk); } catch {}
          ...
          const nextLoc = page.locator('a[aria-label="Next"], .pagination .next a, .pager-next a, a:has-text("Next"), button:has-text("Next")').first();
                    // Log responses for diagnostics
                    page.on('response', (res) => {
                      const u = res.url();
                      if (u.includes('globalsources') && res.status() >= 400) {
                        console.warn('[GS][HTTP]', res.status(), u);
                      }
                    });
                    // Ensure cookie header is applied on the first navigation
                    if (cookieStr) {
          ...
          if (await nextLoc.count()) {
            try {
              ...
              if (href && href.trim().length > 0) {
                ...
                continue;
                    // Infinite scroll + load-more + next pagination (env configurable)
                    const maxPages = Math.max(1, Number(process.env.GS_MAX_PAGES || 5));
                    const scrollsPerPage = Math.max(2, Number(process.env.GS_SCROLLS_PER_PAGE || 6));
                    const scrollDelayMs = Math.max(200, Number(process.env.GS_SCROLL_DELAY_MS || 600));

                    for (let p = 1; p <= maxPages; p++) {
                      for (let s = 0; s < scrollsPerPage; s++) {
                        await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
                        await page.waitForTimeout(scrollDelayMs);
                      }
                      // Click load more if available
                      const moreSel = 'button:has-text("Load more"), button.load-more, a.load-more';
                      const hasMore = await page.$(moreSel);
                      if (hasMore) {
                        try {
                          await Promise.all([
                      ...
                      // Parse current document + iframes
                      const frames = [page, ...page.frames()];
                      for (const f of frames) {
                        try {
                          const html = await f.content();
                          const origin = (() => { try { return new URL(f.url()).origin; } catch { return new URL(url).origin; } })();
                      ...
                      const nextSel = 'a[rel="next"], button:has-text("Next"), a:has-text("Next"), .pagination .next a';
                    ...
                    // If still empty, dump artifacts for diagnosis
                    if (out.length === 0) {
                      try {
