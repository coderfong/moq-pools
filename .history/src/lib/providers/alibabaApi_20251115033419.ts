 ...
 * 
 * OAuth Flow (Alibaba IOP - International Open Platform):
 * - Authorize URL: https://openapi-auth.alibaba.com/oauth/authorize
 * - Token API: https://openapi-auth.alibaba.com/auth/token/create (signed GET request)
 * - Business APIs: https://openapi.alibaba.com/openapi/... (requires access token)
 * 
 * IMPORTANT: Token requests must be signed using IOP protocol:
 * - Parameters: code, app_key, timestamp, sign_method, sign
 * - Signature: MD5(app_secret + sorted_params + app_secret)
 * - Method: GET (not POST like standard OAuth2)
 * 
 * This is NOT standard OAuth2 - it's Alibaba's proprietary IOP protocol.
 */

import crypto from 'crypto';

// OAuth2 endpoints for Alibaba IOP (International Open Platform)
const ALIBABA_AUTH_BASE = 'https://openapi-auth.alibaba.com';
const ALIBABA_TOKEN_API = 'https://openapi-auth.alibaba.com/auth/token/create';
...
export class AlibabaApiClient {
   ...
   * 
   * IMPORTANT: Alibaba's IOP (International Open Platform) uses:
   * - Authorize URL: https://openapi-auth.alibaba.com/oauth/authorize
   * - Token API: https://openapi-auth.alibaba.com/auth/token/create (signed request)
   * 
   * Unlike standard OAuth2, this requires signing the request with app_key and sign
   */
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    ...
    try {
      ...
      console.log('[AlibabaAPI] Request params:', Object.keys(params).join(', '));
      console.log('[AlibabaAPI] Sign (first 10 chars):', sign.substring(0, 10) + '...');

      const response = await fetch(fullUrl, {
        method: 'GET', // IOP APIs typically use GET for token creation
        headers: {
