...
export async function fetchExportProductDetail(url: string, debug = false): Promise<ExportListing | null> {
  try {
    ...
    let img = $('img.prd-img, .product-image img, img').first().attr('src') || '';
...
export async function fetchExportSearch(q: string, opts: { limit?: number; detail?: boolean; detailLimit?: number; debug?: boolean } = {}): Promise<ExportListing[]> {
  const { limit = 120, detail = true, detailLimit = 12, debug = false } = opts;
  const { listings } = await fetchExportCategoryPage(q, debug);
  if (!listings.length) return [];
  let out = listings.slice(0, limit);
  if (detail) {
    const enriched: ExportListing[] = [];
    for (const l of out.slice(0, detailLimit)) {
      const det = await fetchExportProductDetail(l.url, debug).catch(()=>null);
      if (det) enriched.push({ ...l, ...det }); else enriched.push(l);
    }
    // Keep non-enriched remainder
    if (out.length > detailLimit) enriched.push(...out.slice(detailLimit));
    out = enriched;
  }
  return out;
}
