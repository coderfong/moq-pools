 ...
 * 
 * OAuth Flow (IMPORTANT - uses mixed endpoints):
 * - Authorize URL: https://openapi-auth.alibaba.com/oauth/authorize
 * - Token Exchange: https://openapi.alibaba.com/openapi/param2/1/system.oauth2/getToken/{APP_KEY}
 * - Business APIs: https://openapi.alibaba.com/openapi/... (requires access token)
 * 
 * CRITICAL: The callback URL in your .env MUST match EXACTLY (including path) 
 * with what you configured in the Alibaba Open Platform console.
 * 
 * Note: Unlike standard OAuth2, Alibaba uses the auth server for authorization but
 * the Open Platform gateway for token exchange. This is their documented pattern.
 */

import crypto from 'crypto';

// OAuth2 endpoints - Alibaba uses mixed approach
const ALIBABA_AUTH_BASE = 'https://openapi-auth.alibaba.com';
const ALIBABA_API_GATEWAY = 'https://openapi.alibaba.com/openapi';
...
export class AlibabaApiClient {
   ...
   * 
   * IMPORTANT: Alibaba's OAuth flow uses:
   * - Authorize URL: https://openapi-auth.alibaba.com/oauth/authorize
   * - Token URL: https://oauth.alibaba.com/token (standard OAuth2 endpoint)
   */
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    ...
    try {
      ...
      // IMPORTANT: Use the standard OAuth2 token endpoint
      // This is the correct endpoint for Alibaba.com (ICBU) seller authorization
      const TOKEN_URL = 'https://oauth.alibaba.com/token';
      ...
      console.log('[AlibabaAPI] Raw token response:', text);
      console.log('[AlibabaAPI] Response status:', response.status, response.statusText);
      console.log('[AlibabaAPI] Response Content-Type:', response.headers.get('content-type'));

      if (!response.ok) {
        throw new Error(`Token request failed: ${response.status} ${response.statusText} - ${text}`);
      }

      // Parse response (should be JSON from Alibaba API)
      let data: any;
      try {
        data = JSON.parse(text);
      } catch (parseError) {
        console.error('[AlibabaAPI] Failed to parse JSON response:', parseError);
        throw new Error(`Failed to parse token response: ${text}`);
      }

      console.log('[AlibabaAPI] Parsed token response:', data);

      // Check for error in response (Alibaba returns code: "0" for success)
      if (data.error_response || (data.code && data.code !== '0')) {
        const errorMsg = data.error_response?.msg || data.error || data.message || 'Unknown error';
        throw new Error(`API error: ${errorMsg} - ${JSON.stringify(data)}`);
      }
      ...
      // Cache the token
      const expiresIn = Number(data.expires_in) || 3600; // Default 1 hour if not provided
      this.tokenCache = {
      ...
      console.log('[AlibabaAPI] Token expires in:', expiresIn, 'seconds');
      console.log('[AlibabaAPI] User ID:', data.user_id || 'N/A');
      console.log('[AlibabaAPI] Seller ID:', data.seller_id || 'N/A');
      console.log('[AlibabaAPI] Account:', data.account || 'N/A');

      // Normalize response to match TokenResponse interface
      return {
        access_token: data.access_token,
        refresh_token: data.refresh_token,
        expires_in: expiresIn,
        refresh_expires_in: Number(data.refresh_expires_in) || undefined,
        resource_owner: data.user_id || data.resource_owner,
