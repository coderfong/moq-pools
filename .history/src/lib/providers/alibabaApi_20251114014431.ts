...
export class AlibabaApiClient {
   ...
   * Uses Taobao/Alibaba router REST API: alibaba.open.auth.token.create
   */
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    const { appKey, appSecret, apiGateway } = this.config;
    
    try {
      console.log('[AlibabaAPI] Exchanging authorization code for token...');
      
      // Use the Taobao router REST API for auth token creation
      const timestamp = Date.now().toString();
      const params: Record<string, string> = {
        method: 'alibaba.open.auth.token.create',
        app_key: appKey,
        timestamp,
        format: 'json',
        v: '2.0',
      ...
      // Generate signature for auth API (different from product APIs)
      const signStr = appSecret + Object.keys(params).sort().map(k => k + params[k]).join('') + appSecret;
      const crypto = require('crypto');
      const sign = crypto.createHash('md5').update(signStr).digest('hex').toUpperCase();
      params.sign = sign;

      // Build URL with all params
      const gateway = apiGateway || 'https://eco.taobao.com/router/rest';
      const url = new URL(gateway);
      Object.entries(params).forEach(([key, value]) => {
        url.searchParams.set(key, value);
      });

      console.log('[AlibabaAPI] Token API Request URL:', url.toString().replace(/code=[^&]+/, 'code=***'));
      ...
      if (!response.ok) {
        ...
        console.error('[AlibabaAPI] Token API Error Response:', errorText);
        throw new Error(`Token API request failed: ${response.status} - ${errorText.substring(0, 200)}`);
      }

      const data = await response.json();
      console.log('[AlibabaAPI] Token exchange response:', data);

      // Check for error response
      if (data.error_response) {
        console.error('[AlibabaAPI] Token API Error:', data.error_response);
        throw new Error(`Token API Error: ${data.error_response.msg || JSON.stringify(data.error_response)}`);
      }

      // Extract token from response
      const tokenData = data.alibaba_open_auth_token_create_response;
      ...
      console.error('[AlibabaAPI] No access token in response');
      return null;
    } catch (error) {
