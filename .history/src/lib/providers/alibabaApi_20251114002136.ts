...
export class AlibabaApiClient {
  ...
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    ...
    try {
      const params = {
        ...
        redirect_uri: process.env.ALIBABA_REDIRECT_URI || 'https://localhost:3007/api/alibaba/callback',
      };
  ...
  generateAuthorizationUrl(state?: string): string {
    ...
    const redirectUri = process.env.ALIBABA_REDIRECT_URI || 'https://localhost:3007/api/alibaba/callback';
    ...
    // Use appropriate authorization endpoint based on your region
    // For Alibaba.com (international): https://auth.alibaba.com/oauth/authorize
    // For 1688.com (China): https://auth.1688.com/oauth/authorize
    const authEndpoint = process.env.ALIBABA_AUTH_ENDPOINT || 'https://auth.alibaba.com/oauth/authorize';
