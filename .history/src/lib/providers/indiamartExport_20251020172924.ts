...
export async function fetchExportSearchCards(q: string, debug = false): Promise<ExportListing[]> {
  try {
    const res = await fetch(SEARCH_URL(q), { headers: { 'User-Agent': 'Mozilla/5.0', 'Accept': 'text/html,*/*;q=0.9' }, cache: 'no-store' });
    if (!res.ok) return [];
    const html = await res.text();
    const $ = cheerio.load(html);
    const items: ExportListing[] = [];
    // Common card wrappers; keep broad selectors to survive minor DOM changes
    $('.prd-list .prd-card, .prod-list .prd-card, .list_prd .prd-card, .prd-card, .prod-card').each((_i, el) => {
      const card = $(el);
      const a1 = card.find('a[href*="/products/?id="]').first();
      const a2 = card.find('a[data-href*="/products/?id="]').first();
      let href = abs(a1.attr('href') || a2.attr('data-href') || card.find('a').attr('href') || '');
      if (!isExportProductUrl(href)) {
        const rel = a1.attr('href') || a2.attr('data-href') || '';
        if (rel.includes('/products/?id=')) href = abs(rel);
      }
      ...
      if (href && title) items.push({ platform: 'INDIAMART', title, url: href, image, price, moq: '', storeName, description: '' });
    });
