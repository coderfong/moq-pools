...
export class AlibabaApiClient {
   ...
   * For ICBU (Alibaba.com International): Uses /auth/token/create endpoint
   */
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    ...
    try {
      ...
      // ICBU uses a simple REST endpoint for token creation
      // Endpoint: https://gw.api.alibaba.com/openapi/param2/1/system.oauth2/getToken/{appkey}
      ...
      if (data.error_response || data.error) {
        ...
        throw new Error(`Token API Error: ${data.error_response?.msg || data.error || 'Unknown error'}`);
      }

      // ICBU returns token data directly
