...
export class AlibabaApiClient {
  ...
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    ...
    try {
      const params = {
        ...
        redirect_uri: process.env.ALIBABA_REDIRECT_URI || 'http://localhost:3007/api/alibaba/callback',
      };
  ...
  generateAuthorizationUrl(state?: string): string {
    ...
    const redirectUri = process.env.ALIBABA_REDIRECT_URI || 'http://localhost:3007/api/alibaba/callback';
