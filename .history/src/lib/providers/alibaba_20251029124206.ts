...
// Fetch Alibaba product detail once and extract:
// - Best gallery/meta image
// - Review summary from module_comment/detail-product-comment (rating and review count)
// - Sold count if present
export async function getAlibabaDetailMeta(detailUrl: string): Promise<{ image?: string; rating?: string; orders?: string; ratingValue?: number; ratingCount?: number; soldCount?: number; }>
{
  ...
  try {
    ...
    // Image extraction: reuse the same priorities as getAlibabaDetailFirstJpg
    ...
    if (!image) {
      ...
      if (metaImg) {
        ...
        if (isRealPicture(abs)) image = upgradeAlibabaThumbnail(abs) || abs;
      }
    }

    // Reviews/sold from module_comment → .detail-product-comment
    ...
    try {
      const $mod = $('[data-module-name="module_comment"], .module_comment').first();
      const $c = ($mod.length ? $mod.find('.detail-product-comment').first() : $('.detail-product-comment').first());
      if ($c && $c.length) {
        const ratingCluster = ($c.find('.detail-review-item.detail-star').first().text() || '').replace(/\s+/g, ' ').trim();
        const rv = ratingCluster.match(/(\d+(?:\.\d+)?)/);
        if (rv) {
          const v = parseFloat(rv[1]);
          if (isFinite(v)) ratingValue = v;
        }
        const reviewText = ($c.find('.detail-review-item.detail-review').first().text() || ratingCluster || '').replace(/\s+/g, ' ').trim();
        const rc = reviewText.match(/(\d[\d,]*)\s*review/i);
        if (rc) {
          const n = parseInt(rc[1].replace(/,/g, ''), 10);
          if (isFinite(n)) ratingCount = n;
        } else if (/no reviews yet/i.test(reviewText)) {
    ...
    if (!rating && (ratingValue != null || ratingCount != null)) {
      const parts: string[] = [];
      if (typeof ratingValue === 'number') parts.push(String(ratingValue));
      if (typeof ratingCount === 'number') parts.push(`(${ratingCount} reviews)`);
      rating = parts.join(' ');
    }
    if (!orders && typeof soldCount === 'number') {
      orders = `${soldCount.toLocaleString()} sold`;
    }

    return { image, rating, orders, ratingValue, ratingCount, soldCount };
  } catch {
    return {};
  }
}
...
export async function getAlibabaDetailFirstJpg(detailUrl: string): Promise<string> {
  // Delegate to the meta scraper for single-fetch behavior
  const meta = await getAlibabaDetailMeta(detailUrl);
  return meta.image || '';
}
...
export async function fetchAlibaba(q: string, limit = 10, opts?: FetchOptions): Promise<ExternalListing[]> {
  ...
  async function worker() {
    while (idx < items.length) {
      ...
      const it = items[i];
      if (!it?.url) continue;
      // Always fetch detail meta once to get best image and review/sold info
      const meta = await getAlibabaDetailMeta(it.url).catch(() => ({} as any));
      if (meta?.image) it.image = meta.image;
      if (meta?.rating) it.rating = meta.rating;
      if (meta?.orders) it.orders = meta.orders;
    }
  }
