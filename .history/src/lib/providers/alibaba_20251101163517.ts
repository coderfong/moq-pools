...
export function isAlibabaBadImageUrl(url: string): boolean {
  try {
    ...
    if (/@img|sprite|logo|favicon|badge|watermark|trademark|assurance|verified/.test(x)) return true;
...
function pickBestCardImage($: cheerio.CheerioAPI, card: cheerio.Cheerio<any>, base: string, preferredWithin?: cheerio.Cheerio<any>): string {
  ...
  const gatherFrom = (root: cheerio.Cheerio<any>) => {
    ...
    imgs.each((_, el) => {
      ...
      if (/(^|\b)(ce|rohs|fcc|ul|tuv|iso|cert|certificate|compliant|compliance|quality\s*assurance|assurance|trade\s*assurance|trademark|verified|authentic|brand)(\b|$)/i.test(label)) {
  ...
  const filtered = pool.filter((u) => {
    ...
    const isBadge = /s\.alicdn\.com\/@img\//.test(x)
      || /tps-\d+-\d+\.png$/.test(x)
      || /(sprite|logo|store[-_]?logo|favicon|badge|watermark|trademark|assurance|verified)/.test(x);
