...
export async function fetchC1688(q: string, limit = 10, opts?: FetchOptions): Promise<ExternalListing[]> {
  ...
  async function parse(html: string, base = 'https://s.1688.com') {
    ...
    $('a').each((_, el) => {
      ...
      if (REQUIRE_MOQ) {
        if (!moqNum || moqNum <= 0) return; // Enforce MOQ presence for 1688 when required
      }
...
async function fetchC1688Headless(q: string, limit = 60): Promise<ExternalListing[]> {
  ...
  const ctx = await browser.newContext({
    userAgent: 'Mozilla/5.0 (Linux; Android 12; Pixel 5 Build/SP1A.210812.015) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36',
    locale: 'zh-CN',
