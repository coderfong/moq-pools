...
export async function fetchAlibaba(q: string, limit = 10, opts?: FetchOptions): Promise<ExternalListing[]> {
  // Use scraping methods (headless or HTML parsing)
  if (opts?.headless) {
...
/**
 * Extract price tiers from an Alibaba product detail page
 * Returns array of {price: string, range: string} objects
 */
export async function fetchAlibabaPriceTiers(detailUrl: string): Promise<{ price: string; range: string }[]> {
  const headers = buildHeaders();
  try {
    const res = await fetch(detailUrl, { headers });
    if (!res.ok) return [];
    ...
    const tiers: { price: string; range: string }[] = [];

    // Method 1: Look for data-testid="ladder-price" (multiple tiers)
    const ladderPrice = $('[data-testid="ladder-price"]');
    if (ladderPrice.length) {
      ladderPrice.find('.price-item').each((_, item) => {
        const $item = $(item);
        // Range is in the first div with text like "1 - 11 pairs" or ">= 12 pairs"
        const rangeText = $item.find('div').first().text().trim();
        // Price is in the span with class containing price/bold
        const priceText = $item.find('span').first().text().trim();
        
        if (rangeText && priceText) {
          tiers.push({ price: priceText, range: rangeText });
        }
      });
    }

    // Method 2: Look for data-testid="range-price" (single MOQ)
    if (tiers.length === 0) {
      const rangePrice = $('[data-testid="range-price"]');
      if (rangePrice.length) {
        const rangeText = rangePrice.find('div').first().text().trim();
        const priceText = rangePrice.find('span').first().text().trim();
    ...
    // Method 3: Fallback - look for common price tier patterns
    if (tiers.length === 0) {
      // Look for patterns like "1-99 pieces US$10.68"
      $('*').each((_, el) => {
        const text = $(el).text();
        // Match patterns like "1 - 99 pieces" followed by price
        const matches = text.matchAll(/(\d+\s*-\s*\d+|>=?\s*\d+)\s*(?:pieces?|pairs?|units?|pcs?|sets?).*?(US\$|USD|¥|￥|\$)\s*[\d,]+(?:\.\d{1,2})?(?:\s*-\s*(?:US\$|USD|¥|￥|\$)?\s*[\d,]+(?:\.\d{1,2})?)?/gi);
        for (const match of matches) {
          const range = match[1];
          const fullPrice = match[0].match(/(US\$|USD|¥|￥|\$)\s*[\d,]+(?:\.\d{1,2})?(?:\s*-\s*(?:US\$|USD|¥|￥|\$)?\s*[\d,]+(?:\.\d{1,2})?)?/)?.[0];
          if (range && fullPrice && tiers.length < 10) {
            tiers.push({ price: fullPrice.trim(), range: range.trim() + ' pieces' });
          }
        }
      });
    }
    ...
    console.error('[fetchAlibabaPriceTiers] Error:', err);
    return [];
  }
}

/**
 * Extract the full product title from an Alibaba product detail page
 * Returns the complete product name/title
 */
export async function fetchAlibabaProductTitle(detailUrl: string): Promise<string> {
  const headers = buildHeaders();
  try {
    const res = await fetch(detailUrl, { headers });
    if (!res.ok) return '';
    ...
    // Method 1: Look for h1 tag (most common for product titles)
    ...
    // Method 3: Look for common Alibaba product title selectors
    ...
    // Method 4: Look for title tag as last resort
    const pageTitle = $('title').text().trim();
    if (pageTitle) {
      // Clean up common suffixes like " - Alibaba.com"
      return pageTitle.replace(/\s*[-–|]\s*Alibaba\.com.*$/i, '').trim();
    }

    return '';
  } catch (err) {
    console.error('[fetchAlibabaProductTitle] Error:', err);
    return '';
  }
}
