...
async function tryParse(html: string, base = 'https://www.aliexpress.com') {
  ...
  $('a').each((_, el) => {
    ...
    out.push({ platform: 'ALIEXPRESS', title, image, price, url: absolute, orders, storeName });
  });
  return out;
}

export async function fetchAliExpress(q: string, limit = 10, opts?: FetchOptions): Promise<ExternalListing[]> {
  if (opts?.headless) {
...
async function fetchAliExpressHeadless(q: string, limit = 60): Promise<ExternalListing[]> {
  let chromium: any;
  try {
  ...
  const cards = Array.from(document.querySelectorAll('a[href*="/item/"] , a[href*="/i/"]')) as HTMLAnchorElement[];
        ...
        for (const a of cards) {
          ...
          out.push({ platform: 'ALIEXPRESS', title, image, price, url, orders, storeName });
        }
        return out;
      });
