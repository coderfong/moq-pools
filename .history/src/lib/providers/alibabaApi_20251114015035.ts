...
export class AlibabaApiClient {
   ...
   * Uses IOP (International Open Platform) endpoint: /auth/token/create
   */
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    ...
    try {
      ...
      // IOP endpoint for token creation
      // Format: https://gw.api.alibaba.com/openapi/param2/1/aliexpress.open/auth/token/create/{appkey}
      const url = `https://gw.api.alibaba.com/openapi/param2/1/aliexpress.open/auth/token/create/${appKey}`;
      
      const params = new URLSearchParams({
        code: authorizationCode,
      });

      console.log('[AlibabaAPI] Token API Request URL:', url);
      ...
      console.log('[AlibabaAPI] Token API Response:', responseText);

      if (!response.ok) {
        console.error('[AlibabaAPI] Token API Error Response:', responseText);
        throw new Error(`Token API request failed: ${response.status} - ${responseText.substring(0, 200)}`);
      }
      ...
      if (data.error_response || data.error) {
        console.error('[AlibabaAPI] Token API Error:', data.error_response || data.error);
        throw new Error(`Token API Error: ${data.error_response?.msg || data.error_message || data.error || 'Unknown error'}`);
      }

      // Token data is in the response directly
      if (data.access_token) {
        ...
        console.log('[AlibabaAPI] Seller ID:', data.seller_id);
