...
function parseC1688SearchHTML(html: string, base = 'https://s.1688.com', requireMoq = true): ExternalListing[] {
  ...
  $('a').each((_, el) => {
    ...
    const isItem = (
      /(?:detail\.1688\.com\/offer\/|\/offer\/|offer[_-]?detail|offerDetail|offer_detail)/i.test(href)
      || (/id=\d{6,}/.test(href) && /1688\.com/i.test(href))
    );
    if (!isItem) return;
...
async function fetchC1688Headless(q: string, limit = 60): Promise<ExternalListing[]> {
  ...
  // Capture dynamic search API responses to extract offerList JSON
  const capturedFromApi: ExternalListing[] = [];
  ...
  function pushFromOfferList(list: any[]) {
    for (const o of Array.isArray(list) ? list : []) {
      try {
        const id = o?.offerId || o?.id || o?.offerNumId || o?.docId || '';
        const url = (
          o?.detailUrl || o?.pcDetailUrl || o?.url || (id ? `https://detail.1688.com/offer/${id}.html` : '')
        );
        const title = (o?.title || o?.subject || o?.offerTitle || '').toString().trim();
        let image = (
          o?.imageUrl || o?.imgUrl || o?.image || o?.picUrl || o?.mainPic || ''
        );
        image = normalizeImageUrl(image);
        const price = (
          o?.priceDisplay || o?.price || o?.priceString || o?.priceText || ''
        ).toString();
        const moqNum: number | null = (() => {
          const n = Number(o?.minOrderQuantity || o?.minOrder || o?.startQuantity || o?.moq || o?.minNum || 0);
          return Number.isFinite(n) && n > 0 ? n : null;
        })();
        if (REQUIRE_MOQ && (!moqNum || moqNum <= 0)) continue;
        const moq = moqNum ? `≥ ${moqNum}` : '';
        if (!url || !title) continue;
        const key = (() => { try { const u = new URL(url); return u.origin + u.pathname; } catch { return url; } })();
        if (seenFromApi.has(key)) continue;
        seenFromApi.add(key);
        capturedFromApi.push({ platform: 'C1688', title, image, price, url, moq });
      } catch {}
    }
  }
  async function tryParseJsonResponse(res: any) {
    try {
      const req = res.request?.();
      const rt = req?.resourceType?.() || '';
      const url: string = res.url?.() || '';
      if (!/xhr|fetch/i.test(String(rt))) return;
      if (!/(offer|search|getOfferList|mtop|gateway|search|ajax)/i.test(url)) return;
      const ct = String((await res.headerValue?.('content-type')) || '');
      if (ct && !/json|javascript/i.test(ct)) return;
      const bodyText = await res.text();
      if (!bodyText) return;
      let data: any = null;
      try { data = JSON.parse(bodyText); } catch {}
      if (!data && /\{/.test(bodyText)) {
        // Sometimes JSONP-like or stringified JSON nested; try to extract
        const m = bodyText.match(/\{[\s\S]*\}/);
        if (m) { try { data = JSON.parse(m[0]); } catch {} }
      }
      if (!data) return;
      // Traverse object to find offerList arrays
      ...
      while (stack.length) {
        ...
        if (typeof cur === 'object') {
          ...
          for (const k of keys) {
            ...
            if (k.toLowerCase().includes('offerlist') && Array.isArray(v)) {
              pushFromOfferList(v);
  ...
  try {
    page.on('response', (res: any) => { tryParseJsonResponse(res); });
  } catch {}
  ...
  try {
    ...
    try {
      await page.waitForFunction(
        () => document.cookie.includes('x5sec') || document.querySelectorAll('a[href*="/offer/"]').length > 5,
        { timeout: 20000 }
      );
    } catch {}
    // Wait briefly for XHRs to populate capturedFromApi
    try { await page.waitForTimeout(1500); } catch {}
    // If still on a challenge page with few anchors, try mobile search as fallback
    let anchorsCount = 0;
    try { anchorsCount = await page.evaluate(() => document.querySelectorAll('a[href*="/offer/"]').length); } catch { anchorsCount = 0; }
    if (anchorsCount < 3) {
      try {
        await page.goto(`https://m.1688.com/offer_search/-${enc}.html`, { waitUntil: 'domcontentloaded', timeout: 45000 });
        await page.waitForTimeout(1500);
        try { await page.waitForLoadState('networkidle', { timeout: 8000 }); } catch {}
        try {
          await page.waitForFunction(
            () => document.cookie.includes('x5sec') || document.querySelectorAll('a[href]')?.length > 20,
            { timeout: 15000 }
          );
        } catch {}
      } catch {}
    }
    // If API already yielded enough, return early
    if (capturedFromApi.length >= Math.min(limit, 20)) return capturedFromApi.slice(0, limit);
    // Try to read window.data.offerresultData if present
    try {
      const jsonStr = await page.evaluate(() => {
        try { return JSON.stringify((window as any).data?.offerresultData || null); } catch { return null; }
      });
      if (jsonStr) {
        try { const data = JSON.parse(jsonStr); if (data?.offerList) pushFromOfferList(data.offerList); } catch {}
      }
    } catch {}
    let idleRounds = 0;
    for (let round = 0; round < 20 && items.length < limit; round++) {
      // Prefer API-captured items; top them up with DOM-scraped ones if needed
      if (capturedFromApi.length >= limit) break;
      ...
      // Merge API items as priority
      if (capturedFromApi.length) {
        for (const it of capturedFromApi) {
          try {
            const key = it.url ? new URL(it.url).origin + new URL(it.url).pathname : '';
            if (key && !seen.has(key)) { seen.add(key); items.push(it); }
          } catch {}
...
export async function searchC1688NoLogin(q: string, page = 1): Promise<ExternalListing[]> {
  ...
  const desktopMini = `https://s.1688.com/selloffer/offer_search.htm?keywords=${enc}&n=y&viewtype=mini&beginPage=${page}`;
  ...
  // Try desktop mini view (lighter markup sometimes bypasses blocks)
  try { console.log('[C1688] Try desktop mini HTML…'); } catch {}
  try {
    const res = await fetch(desktopMini, { headers: headersDesktop });
    const html = await res.text();
    if (!/login\.1688\.com|x5secdata/.test(html)) {
      const rows = parseC1688SearchHTML(html, 'https://s.1688.com', REQUIRE_MOQ);
      if (rows.length) { try { console.log('[C1688] desktop mini ok:', rows.length); } catch {} return rows; }
    } else { try { console.log('[C1688] desktop mini blocked (x5sec)'); } catch {} }
  } catch {}
