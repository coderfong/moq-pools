...
function scoreAlibabaImage(url: string): number {
  ...
  if (u.endsWith('.png') && !/_\d{2,4}x\d{2,4}/.test(u)) s -= 120; // likely icon/badge
...
function pickBestCardImage($: cheerio.CheerioAPI, card: cheerio.Cheerio<any>, base: string, preferredWithin?: cheerio.Cheerio<any>): string {
  ...
  const filtered = pool.filter((u) => {
    ...
    const tooSmall = dims.w && dims.h ? Math.min(dims.w, dims.h) < 140 : false;
    return !isBadge && !isSvg && !isTinyPng && !tooSmall && !isBadPng;
  });
  // If both jpg and png variants exist, try jpg set first
  const jpgFirst = filtered.filter(u => /\.(jpe?g)(?:_|\.|$)/i.test(u));
  const pass1 = jpgFirst.length ? jpgFirst : filtered;
