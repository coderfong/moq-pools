...
export class AlibabaApiClient {
  ...
  async exchangeCodeForToken(authorizationCode: string): Promise<TokenResponse | null> {
    ...
    try {
      ...
      // IOP endpoint for token creation - app_key as parameter, not in path
      const url = `https://gw.api.alibaba.com/openapi/param2/1/aliexpress.open/auth/token/create`;
      ...
      console.log('[AlibabaAPI] Token API Request URL:', `${url}?${params.toString()}`);
