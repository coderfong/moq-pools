...
export async function emitEvent(ev: DomainEvent) {
  switch (ev.type) {
    ...
    case 'SHIPPED': {
      ...
      const link = '/orders';
      await createAlert({ userId: pi.userId, type: 'SHIPPING', title: 'Order shipped', body: tracking ? `Tracking ${tracking}` : 'Your order has shipped.', link });
      break;
    }
    case 'DELIVERED': {
      const pi = await getPoolItemWithOwner(ev.data.poolItemId);
      if (!pi) return;
      const link = '/orders';
      await createAlert({ userId: pi.userId, type: 'SHIPPING', title: 'Delivered — thanks!', body: 'Hope you love it. Rate your order.', link });
      break;
    }
    case 'PROMO_TRENDING': {
