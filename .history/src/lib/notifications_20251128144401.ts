  ...
  try {
    ...
    const alert = await (prisma as any).alert.create({
      data: {
        ...
        // Note: metadata field doesn't exist in schema, removed to prevent errors
      },
    });
    
    console.log('[NOTIFICATIONS] ✅ Alert created:', alert.id, '- Type:', alert.type, '- Title:', alert.title);
    return alert;
  } catch (error) {
    console.error('[NOTIFICATIONS] ❌ Failed to create user alert:', error);
    return null;
  }
}
