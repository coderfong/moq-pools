...
export function BAD(d?: NormalizedDetail | null): boolean {
  ...
  return (
    ...
    !d.priceText ||
    !d.priceTiers?.length ||
    // TEMP relax: allow slightly fewer attributes as long as we have some packaging
    // Previously: attributes < 3 AND packaging < 1
    ((d.attributes?.length ?? 0) < 2 && (d.packaging?.length ?? 0) < 1)
  );
}
...
// Contract helpers: prefer positive naming in new code while preserving existing exports
export const isCorrect = (d: NormalizedDetail): boolean => {
  try { return !BAD(d); } catch { return false; }
};

export const isWeak = (d: NormalizedDetail): boolean => {
  try { return BAD(d); } catch { return true; }
};
