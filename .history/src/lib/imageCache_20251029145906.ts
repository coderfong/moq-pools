...
async function fileHeader(filePath: string, max = 16): Promise<Buffer | null> {
  try {
    ...
    const cacheRoot = path.join(process.cwd(), 'public', 'cache') + path.sep;
    const normalized = path.normalize(filePath);
    if (!normalized.startsWith(cacheRoot)) return null;

    // Read only the first few bytes efficiently without loading whole file
    const fh = await fs.open(normalized, 'r');
    try {
