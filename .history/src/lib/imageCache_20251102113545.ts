...
// Read only the first bytes of a cached file under public/cache by basename.
// Accepts only a basename like "<sha1>.<ext>" and refuses paths with separators.
async function fileHeader(fileName: string, max = 16): Promise<Buffer | null> {
  try {
    // Enforce simple, safe basename contract to avoid dynamic FS issues
    const base = path.basename(fileName);
    if (base !== fileName) return null; // contains path separators
    if (!/^[a-f0-9]{40}\.(?:jpe?g|png|webp)$/i.test(base)) return null;

    const cacheRootAbs = path.join(process.cwd(), 'public', 'cache');
    const abs = path.join(cacheRootAbs, base);

    // Read just the first N bytes with fs.open/read (avoid createReadStream)
...
// Parse PNG width/height from the IHDR chunk using only the first ~32 bytes.
// Returns null if header is not PNG or malformed.
function parsePngDimsFromHeader(header: Buffer): { width: number; height: number } | null {
  try {
    if (!header || header.length < 24) return null;
    // PNG signature
    if (!(header[0] === 0x89 && header[1] === 0x50 && header[2] === 0x4e && header[3] === 0x47 && header[4] === 0x0d && header[5] === 0x0a && header[6] === 0x1a && header[7] === 0x0a)) return null;
    // Next 4 bytes: length (big-endian, should be 13 for IHDR)
    // Next 4 bytes: chunk type 'IHDR'
    if (!(header[12] === 0x49 && header[13] === 0x48 && header[14] === 0x44 && header[15] === 0x52)) return null;
    // Width/Height big-endian at bytes 16..23
    const w = (header[16] << 24) | (header[17] << 16) | (header[18] << 8) | (header[19] << 0);
    const h = (header[20] << 24) | (header[21] << 16) | (header[22] << 8) | (header[23] << 0);
...
export async function cacheExternalImage(rawSrc: string, opts?: { preferJpgForIndiaMart?: boolean }): Promise<{ localPath: string; absPath: string }> {
  ...
  for (const ext of exts) {
    ...
    try {
      ...
      // Read header using only the basename to keep the path analysis trivial
      const hdr = await fileHeader(`${hash}.${ext}`);
      if (hdr && headerMatchesExt(hdr, ext)) {
        // If cached PNG, try to reject tiny icons based on IHDR without requiring sharp
        if (ext === 'png') {
          const dims = parsePngDimsFromHeader(hdr);
          if (dims && Math.min(dims.width, dims.height) < 120) {
            try { await fs.unlink(p); } catch {}
            throw new Error('Cached tiny PNG image rejected');
          }
        }
        // Reject previously cached tiny icons by inspecting dimensions
        try {
          ...
          if (sharp) {
            ...
            if (w > 0 && h > 0 && Math.min(w, h) < 120) {
              try { await fs.unlink(p); } catch {}
              throw new Error('Cached tiny image rejected');
            }
          }
        } catch {}
  ...
  const isAliCdn = url.hostname.includes('alicdn') || url.hostname.includes('alibaba');
  const ct = (upstream.headers.get('content-type') || '').toLowerCase();
  const extBySig = detectImageExtFromBytes(buf);
  const isPng = extBySig === 'png' || ct.includes('png');
  const minBytes = (isAliCdn && isPng) ? 10000 : 4000;
  ...
  // If sharp not available, try PNG header parsing for dimension-based rejection
  try {
    const sharp = await getSharp();
    if (!sharp && isPng) {
      // For PNGs we can read dimensions from IHDR in header
      const header = buf.subarray(0, 32);
      const dims = parsePngDimsFromHeader(header);
      if (dims && Math.min(dims.width, dims.height) < 120) {
        throw new Error(`PNG too small (${dims.width}x${dims.height})`);
      }
    }
  } catch {}
