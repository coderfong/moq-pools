// import 'server-only'; // Commented out to allow script usage
...
export async function cacheExternalImage(rawSrc: string, opts?: { preferJpgForIndiaMart?: boolean }): Promise<{ localPath: string; absPath: string }> {
  ...
  // Made-in-China uses efficient WebP compression, so use a lower threshold for them
  const isMadeInChina = url.hostname.includes('made-in-china') || url.hostname.includes('micstatic');
  const sizeThreshold = isMadeInChina ? 500 : 4000; // 500 bytes for MIC WebP, 4KB for others
  if (buf.byteLength < sizeThreshold) {
  ...
  try {
    ...
    if (sharp) {
      ...
      // Made-in-China sometimes has smaller but legitimate product images
      const minDimension = isMadeInChina ? 60 : 120; // Very lenient for MIC WebP
      if (w > 0 && h > 0) {
  ...
  // BUT for Made-in-China WebP images, re-encode at higher quality to fix blurriness
  const isMadeInChinaWebP = decidedExt === 'webp' && url.hostname.includes('made-in-china.com');
  
  if ((decidedExt === 'webp' || decidedExt === 'png') && !forceJpgIndiaMart) {
    // For Made-in-China WebP, re-encode at high quality to improve clarity
    if (isMadeInChinaWebP && sharp) {
      try {
        destExt = 'webp';
        dest = path.join(outDir, `${hash}.webp`);
        // Re-encode WebP at quality 90 (much higher than the blurry originals)
        await sharp(buf).webp({ quality: 90, effort: 4 }).toFile(dest);
        return { localPath: `/cache/${hash}.webp`, absPath: dest };
      } catch {
