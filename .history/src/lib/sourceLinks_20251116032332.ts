export type PlatformKey = 'C1688' | 'ALIBABA' | 'MADE_IN_CHINA' | 'INDIAMART' | 'INDIAMART_EXPORT' | 'TAOBAO' | 'GLOBAL_SOURCES' | 'ALL';
...
export function buildExternalSearchUrl(platform: PlatformKey, q: string) {
  ...
  switch (platform) {
    ...
    case 'GLOBAL_SOURCES':
      return `https://www.globalsources.com/search-results?query=${term}`;
    case 'ALL':
    default:
      return '';
  }
