...
export function BAD(d?: NormalizedDetail | null): boolean {
  if (!d) return true;
  if (!d.priceTiers || d.priceTiers.length === 0) return true;
  const first = d.priceTiers[0];
  if (!(d.priceText || (first && first.price))) return true;
...
export const isWeakDetail = (d: NormalizedDetail): boolean => {
  try {
    const firstRange = (d.priceTiers?.[0]?.range || '').toString();
    const noRealTiers = (d.priceTiers?.length || 0) <= 1 && /≥\s*1\b/.test(firstRange);
    const noAttrs = (d.attributes?.length || 0) < 3;
    const noPacks = (d.packaging?.length || 0) === 0;
    return !!(noRealTiers && noAttrs && noPacks);
  } catch { return true; }
};
