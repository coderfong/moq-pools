  ...
  try {
    ...
    $('img[src*="alicdn.com"]').each((_, img) => {
      ...
      if (src && 
        ...
        // Only replace if there's actually a size suffix
        if (cleanSrc.match(/_\d+x\d+q?\d*\.jpg$/i)) {
          cleanSrc = cleanSrc.replace(/_\d+x\d+q?\d*\.jpg$/i, '.jpg');
        }
    ...
    if (images.length > 0) {
      ...
      // Priority: sc04.alicdn.com/kf/ images (product images) > others
      const best = uniqueImages.find(img => img.includes('sc04.alicdn.com/kf/') && !img.match(/_\d+x\d+/)) ||
