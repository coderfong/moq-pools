...
Fixed the OAuth token exchange by:
1. Switching from the **API gateway** to the **OAuth server** 
2. Handling **both JSON and form-urlencoded responses** correctly

**Previous issues:**
- ❌ Calling `https://gw.api.alibaba.com/openapi/...` → `gw.AppKeyNotFound` errors
- ❌ Assuming response is JSON → `SyntaxError: Unexpected end of JSON input`

**Fixed:**
- ✅ Using `https://openapi-auth.alibaba.com/oauth/token` 
- ✅ Reading response as text first, then parsing based on Content-Type
...
```typescript
// ❌ WRONG: Trying to get OAuth tokens from the API gateway
...
```
**Result:** `"error_code":"gw.AppKeyNotFound","error_message":"Invalid appKey"`

### New (Correct) Implementation  
```typescript
// ✅ CORRECT: Get OAuth tokens from the OAuth server
const TOKEN_URL = `${ALIBABA_AUTH_BASE}/oauth/token`;
// = https://openapi-auth.alibaba.com/oauth/token

// ✅ CORRECT: Read as text first, parse based on Content-Type
...
if (contentType.includes('application/json')) {
  ...
  // Parse as x-www-form-urlencoded
...
2. **Response Format Handling**: Reads response as text first, then parses based on Content-Type (JSON or form-urlencoded)
