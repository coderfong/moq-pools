...
async function rescrapeListing(listing, retryAttempt = 1) {
  const url = `${API_URL}?id=${listing.id}&force=1&enhanced=1`;
  ...
  try {
    ...
    if (result.success && result.listing) {
      const evaluation = evaluateListing(result.listing.detailJson);
      return {
        success: true,
        listing: result.listing,
        evaluation,
        improved: evaluation.category === 'GOOD'
      };
    }
...
async function retryProblematicListings(category = 'PARTIAL') {
  ...
  let whereClause;
  if (category === 'MISSING') {
    whereClause = `"detailJson" IS NULL`;
  } else if (category === 'BAD') {
    whereClause = `jsonb_array_length(COALESCE("detailJson"->'attributes', '[]'::jsonb)) = 0`;
  } else { // PARTIAL
    whereClause = `jsonb_array_length(COALESCE("detailJson"->'attributes', '[]'::jsonb)) BETWEEN 1 AND 9`;
  }
  ...
  const countResult = await prisma.$queryRaw`
    ...
    WHERE platform = 'ALIBABA' 
      AND (${prisma.$queryRawUnsafe(whereClause)})
  `;
  ...
  while (offset < totalCount && !isShuttingDown) {
    ...
    for (const listing of listings) {
      ...
      if (result.success && result.listing) {
        const afterEval = evaluateListing(result.listing.detailJson);
        console.log(`   📊 After: ${afterEval.category} (${afterEval.attrCount} attrs)`);
        
        if (afterEval.category === 'GOOD') {
          console.log(`   ✅ Improved to GOOD!`);
          improved++;
        } else if (afterEval.attrCount > beforeEval.attrCount) {
          console.log(`   📈 Partial improvement (+${afterEval.attrCount - beforeEval.attrCount} attrs)`);
          unchanged++;
        } else {
