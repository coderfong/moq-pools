  ...
  for (let i = offset; i < totalCount; i += BATCH_SIZE) {
    ...
    for (const listing of batch) {
      ...
      try {
        ...
        if (result.success) {
          const newAttrs = result.data?.attributes?.length || 0;
          
          if (newAttrs >= 10) {
            console.log(`    ✅ Fixed! Now has ${newAttrs} attributes`);
            good++;
            consecutiveFails = 0;
          } else if (newAttrs > 0) {
            console.log(`    ⚠️  Partial: ${newAttrs} attributes`);
            partial++;
            consecutiveFails = 0;
          } else {
            console.log(`    ❌ Still 0 attributes`);
            bad++;
            consecutiveFails++;
          }
        } else {
