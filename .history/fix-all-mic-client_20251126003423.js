...
const BATCH_SIZE = 40; // Larger batches for single script
const DELAY_BETWEEN_BATCHES = 500; // Minimal delay between batches
const PROGRESS_FILE = path.join(__dirname, 'mic-fix-progress.json');
const MAX_CONSECUTIVE_FALLBACKS = 20; // Allow more fallbacks before backing off
const BLOCK_COOLDOWN_MS = 5 * 60 * 1000; // 5 minutes cooldown when blocked (reduced from 30)
const MAX_CONCURRENT_REQUESTS = 8; // 8 parallel when running solo
const RATE_LIMIT_BACKOFF = 5000; // 5 seconds backoff
const INTER_CHUNK_DELAY = 50; // Minimal 50ms between chunks
...
async function fixAllMadeInChina() {
  ...
  while (offset < needFixCount && !isShuttingDown) {
    ...
    if (rateLimitPercent > 0.8) {
      consecutiveRateLimits++;
      console.log(`🔴 Heavy throttling (${rateLimitCount}/${batch.length}) - reducing concurrency`);
      
      // Reduce concurrency dramatically
      currentConcurrency = Math.max(1, Math.floor(currentConcurrency * 0.5));
      console.log(`⬇️  Concurrency reduced to ${currentConcurrency}`);
      
      // Short pause to let API recover
      await new Promise(resolve => setTimeout(resolve, 2000));
      
    } else if (rateLimitPercent > 0.5) {
      consecutiveRateLimits++;
      ...
      // No significant rate limiting - increase concurrency
      consecutiveRateLimits = 0;
      if (currentConcurrency < MAX_CONCURRENT_REQUESTS) {
        currentConcurrency = Math.min(MAX_CONCURRENT_REQUESTS, currentConcurrency + 1);
        console.log(`⬆️  Concurrency increased to ${currentConcurrency}`);
      }
    }
    
    // If consistently rate limited, take a longer break
    if (consecutiveRateLimits >= 5) {
      console.log(`⏸️  Consistently rate limited for 5 batches, cooling down 10s...`);
      await new Promise(resolve => setTimeout(resolve, 10000));
      consecutiveRateLimits = 0;
      currentConcurrency = 1; // Start conservatively
    }
