  ...
  try {
    ...
    if (user) {
      const poolItems = await db.$queryRaw`
        SELECT pi.id, pi."poolItemStatus", pi.quantity, p.status as payment_status, p.amount
        FROM "PoolItem" pi
      ...
      poolItems.forEach((item, idx) => {
        console.log(`  [${idx + 1}] ${item.id}: ${item.poolItemStatus} - Qty: ${item.quantity} - Payment: ${item.payment_status || 'None'}`);
      });
    }
