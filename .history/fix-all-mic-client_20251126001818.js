...
async function fixAllMadeInChina() {
  ...
  while (offset < needFixCount && !isShuttingDown) {
    ...
    // Only back off if majority of batch was rate limited (aggressive throttling)
    if (rateLimitCount > batch.length * 0.7) {
      ...
      // Just note it but don't slow down
      console.log(`⚡ ${rateLimitCount} rate limits in batch (continuing at normal speed)`);
    }
