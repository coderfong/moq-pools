...
export default async function ListingsView({ searchParams }: { searchParams: Record<string,string|undefined> }) {
  ...
  const selectedCols = ([2,3,4,5] as number[]).includes(colsQ) ? colsQ : 5;
  const GRID_COLS: Record<number, string> = {
    2: 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2',
    3: 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3',
    4: 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4',
    5: 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 lg:grid-cols-5',
  };
        ...
        {(() => {
          ...
          return (
            <div className="flex items-center gap-2">
              ...
              {sizes.map((cc) => (
                <Link key={cc} href={buildHref(cc)} className={`px-2 py-1 rounded border ${cc === selectedCols ? 'bg-black text-white' : 'bg-white dark:bg-gray-800'}`}>{cc}</Link>
              ))}
            </div>
          );
        })()}
        ...
        {listings.map(l => {
          ...
          return (
            <li key={l.id} className="border rounded p-3 bg-white/50 dark:bg-gray-900/30 shadow-sm flex flex-col">
              <div className="relative w-full h-32 mb-2 bg-gray-100 dark:bg-gray-800 rounded overflow-hidden flex items-center justify-center group">
                {img ? (
