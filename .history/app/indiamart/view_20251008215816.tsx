  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <Link href={`/indiamart?category=${cat}&page=${pg}&pageSize=${pageSize}&tax=${tax ? '1':'0'}&coverage=${showCoverage ? '0':'1'}`} className="underline text-blue-600">{showCoverage ? 'Hide coverage' : 'Show coverage'}</Link>
      </div>
      ...
      {counts.map((c,i)=> {
        ...
        return (
          <div key={i} className="flex flex-col items-center">
            <div style={{ height: height, minWidth: '10px' }} className="bg-blue-500/70 rounded-t w-3" title={`${label}: ${c}`}></div>
