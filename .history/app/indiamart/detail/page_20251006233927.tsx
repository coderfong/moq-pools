...
export default async function IndiaMartDetailPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {detail?.htmlSnippet ? (
        <pre className="text-xs bg-gray-900 text-gray-100 p-4 rounded-md overflow-x-auto max-h-[600px] whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: detail.htmlSnippet.replace(/[<>&]/g, c => ({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]||c)) }} />
      ) : (
