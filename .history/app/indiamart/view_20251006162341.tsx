...
export default async function ListingsView({ searchParams }: { searchParams: Record<string,string|undefined> }) {
  ...
  const size = Math.min(100, Math.max(1, Number(searchParams.pageSize || '40')));
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-end gap-4 justify-between">
        ...
        <form className="flex gap-2" action="/indiamart" method="get">
          ...
          <input name="pageSize" defaultValue={String(pageSize)} className="border rounded px-2 py-1 w-20 text-sm" />
