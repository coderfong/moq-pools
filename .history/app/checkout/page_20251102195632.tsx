...
export default async function CheckoutPage({ searchParams }: { searchParams: { poolId?: string } }) {
  ...
  // Get product image, but upgrade from /seed/ placeholder to cached Alibaba image if available
  const img = await (async () => {
    ...
    // If it's a seed placeholder and we have a sourceUrl, try to get the real cached image from SavedListing
    if (imgFromJson && /^\/seed\//i.test(imgFromJson) && sourceUrl && prisma) {
      try {
        const listing = await prisma.savedListing.findUnique({
          where: { url: sourceUrl },
          select: { image: true }
        });
        if (listing?.image && /^\/cache\//i.test(listing.image)) {
