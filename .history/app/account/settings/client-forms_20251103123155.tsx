...
export function SettingsForm({ 
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg(j?.ok ? 'Saved successfully!' : (j?.message || 'Save failed'));
    } catch {
      setMsg('Save failed');
    } finally {
      setSaving(false);
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <OutlineButton type="button" onClick={() => alert('Change Password: Contact support to reset your password.')}>
          Change Password
        </OutlineButton>
    ...
    <div className="grid gap-4">
      <div className="space-y-3">
        <Toggle 
          ...
          label="📧 Email Notifications (Order updates, pool progress)" 
        />
        <Toggle 
          checked={prefs.smsAlerts} 
          onChange={(v) => setPrefs({ ...prefs, smsAlerts: v })} 
          label="📱 SMS Notifications (Shipping updates)" 
        />
        <Toggle 
          checked={prefs.pushAlerts} 
          onChange={(v) => setPrefs({ ...prefs, pushAlerts: v })} 
          label="🔔 Push Notifications (Important alerts)" 
        />
      </div>
      <div className="flex items-center gap-3">
