...
async function fetchMessages(threadId: string): Promise<Message[]> {
  ...
  // Get current user ID to determine message sides
  const meRes = await fetch('/api/me', { cache: 'no-store' });
  const meData = await meRes.json();
  const myId = meData?.ok ? meData.user?.id : null;
  
  console.log('fetchMessages debug:', {
    myId,
    messagesCount: data.messages?.length,
    firstMessage: data.messages?.[0],
    allMessageUsers: data.messages?.map((m: any) => ({ id: m.user?.id, role: m.user?.role }))
  });
  
  // Transform API response to client format
  const transformedMessages = (data.messages || []).map((msg: any) => {
    const isFromMe = msg.user?.id === myId;
    const result = {
      id: msg.id,
      threadId: msg.threadId,
      sender: isFromMe ? 'me' : 'them' as 'me' | 'them',
      text: msg.text,
    ...
    console.log('Message transform:', {
      ...
      messageUserId: msg.user?.id,
      messageUserRole: msg.user?.role,
      myId,
      isFromMe,
...
export default function MessagesClient() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      fetchMessages(activeId).then((m) => {
        console.log('Polling update: fetched', m.length, 'messages for thread', activeId);
        setMsgs(m);
      }).catch(err => {
  ...
  // Poll for new threads every 5s (so users see new admin messages)
  useEffect(() => {
    const id = setInterval(() => {
      fetchThreads().then((t) => {
        setThreads(t);
        // If no active thread selected yet and threads exist, select first
        if (!activeId && t.length > 0) {
          setActiveId(t[0].id);
        }
      }).catch(() => {});
    }, 5000);
  ...
  async function handleSend() {
    ...
    try {
      ...
      setMsgs((curr) => curr.filter(m => m.id !== tempMessage.id));
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <section className="h-screen w-full bg-neutral-50 text-neutral-900 flex flex-col overflow-hidden">
      {/* Top toggle */}
      <div className="w-full px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between text-gray-900">
        <h1 className="text-xl font-bold bg-gradient-to-r from-gray-900 to-blue-600 bg-clip-text text-transparent">Messages</h1>
      </div>

      {mode === 'alerts' ? (
        <div className="flex-1 w-full px-4 sm:px-6 lg:px-8 min-h-0 grid grid-cols-[minmax(140px,180px)_minmax(250px,300px)_1fr] lg:grid-cols-[minmax(160px,200px)_minmax(280px,340px)_1fr] border-2 border-blue-200/50 rounded-2xl overflow-hidden shadow-lg bg-white">
          ...
          <aside className="min-h-0 flex flex-col border-r-2 border-blue-200/30 bg-white overflow-hidden">
      ...
      <div className="flex-1 w-full px-4 sm:px-6 lg:px-8 min-h-0 grid grid-cols-[minmax(140px,180px)_minmax(250px,300px)_1fr] lg:grid-cols-[minmax(160px,200px)_minmax(280px,340px)_1fr] border-2 border-blue-200/50 rounded-2xl overflow-hidden shadow-lg bg-white">
        {/* Far-left: Mode selector */}
        <aside className="min-h-0 border-r-2 border-blue-200/30 bg-gradient-to-b from-blue-50/20 to-white p-4 overflow-hidden">
          <div className="space-y-3">
            <div>
              <div className="text-xs uppercase tracking-wide font-bold bg-gradient-to-r from-gray-700 to-blue-600 bg-clip-text text-transparent mb-3">View</div>
              <div className="flex flex-col gap-2">
          ...
          <div className="flex-1 overflow-y-auto">
            <ul className="divide-y divide-neutral-200">
              {displayedThreads.map((t) => (
                <li key={t.id} className={`cursor-pointer transition-all ${activeId === t.id ? 'bg-gradient-to-r from-blue-50 to-indigo-50/30 border-l-4 border-blue-500' : 'hover:bg-neutral-50'}`}>
                  <button className="w-full text-left" onClick={() => setActiveId(t.id)}>
                    <div className="flex items-start gap-3 p-4">
                      {t.avatarUrl ? (
                        <img className="size-11 rounded-xl object-cover ring-2 ring-blue-200/50 shadow-sm" src={t.avatarUrl} alt="Avatar" />
                      ) : me?.role === 'ADMIN' ? (
                        <div className="size-11 rounded-xl bg-gradient-to-br from-neutral-300 to-neutral-400 shadow-md flex items-center justify-center text-neutral-700">
                        ...
                        <div className="size-11 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 shadow-lg shadow-blue-500/30 flex items-center justify-center text-white">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                            {t.unread && (
                              <span
                                className="inline-block size-2 shrink-0 rounded-full bg-blue-600 animate-pulse shadow-sm shadow-blue-400"
                                aria-label="Unread"
                                title="Unread"
                              />
                            )}
                            <p className={`truncate ${t.unread ? 'font-bold text-gray-900' : 'font-medium text-neutral-800'}`}>{deriveThreadTitle(t)}</p>
                          </div>
                          <time className="shrink-0 text-xs text-neutral-500 font-medium">
                            {new Date(t.updatedAt).toLocaleDateString()}
                          </time>
                        </div>
                        {t.company && <p className="truncate text-sm text-blue-600/70 font-medium mt-0.5">{t.company}</p>}
                        {t.preview && (
                          <p className={`mt-1 line-clamp-2 text-sm ${t.unread ? 'text-neutral-700 font-medium' : 'text-neutral-500'}`}>{t.preview}</p>
                        )}
                      </div>
                    </div>
                  </button>
              ...
              {!threads.length && !loading && (
                <li className="p-8 text-center">
                  <div className="inline-flex items-center justify-center size-16 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 mb-3">
                    <svg className="size-8 text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
                    </svg>
                  </div>
                  <p className="text-sm text-neutral-600 font-medium">No conversations yet</p>
                  <p className="text-xs text-neutral-500 mt-1">Start a new conversation to get started</p>
                </li>
              )}
            </ul>
          </div>
  ...
  <main className="min-h-0 flex flex-col bg-neutral-25">
          {/* Header */}
          <header className="flex items-center justify-between gap-4 border-b-2 border-blue-200/30 px-6 py-4 bg-gradient-to-r from-white to-blue-50/30">
            <div className="flex items-center gap-3 min-w-0">
              {activeThread?.avatarUrl || (me?.role === 'ADMIN' ? ADMIN_AVATAR_URL : undefined) ? (
                <img
                  className="size-12 rounded-xl object-cover ring-2 ring-blue-200/50 shadow-md"
                  src={(activeThread?.avatarUrl || (me?.role === 'ADMIN' ? ADMIN_AVATAR_URL : undefined)) as string}
                  alt=""
                />
              ) : me?.role === 'ADMIN' ? (
                <div className="size-12 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 shadow-lg shadow-blue-500/30 flex items-center justify-center text-white">
                ...
                <div className="size-12 rounded-xl bg-gradient-to-br from-neutral-300 to-neutral-400 shadow-md flex items-center justify-center text-neutral-700">
              ...
              <div className="min-w-0">
                <h2 className="truncate font-bold text-lg bg-gradient-to-r from-gray-900 to-blue-600 bg-clip-text text-transparent">{activeThread?.title ?? 'Select a conversation'}</h2>
                {activeThread?.company && (
                  <p className="truncate text-sm text-blue-600/70 font-medium">{activeThread.company}</p>
                )}
              </div>
            </div>
          </header>
          ...
          {/* Messages */}
          <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto bg-gradient-to-br from-neutral-50 via-blue-50/20 to-neutral-50 px-6 pt-6 pb-28 overscroll-contain">
            {activeId ? (
              <div className="space-y-4">
                {msgs.map((m) => (
                  <div key={m.id} className={`flex items-start gap-3 ${m.sender === 'me' ? 'justify-end' : ''}`}>
                    {m.sender === 'them' && (
                      me?.role === 'ADMIN' ? (
                        <div className="size-9 rounded-xl bg-gradient-to-br from-neutral-300 to-neutral-400 shadow-md flex items-center justify-center text-neutral-700" title="User">
                          <svg className="size-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                            <path d="M12 12c2.761 0 5-2.239 5-5S14.761 2 12 2 7 4.239 7 7s2.239 5 5 5zm0 2c-4.418 0-8 2.015-8 4.5V21h16v-2.5c0-2.485-3.582-4.5-8-4.5z" />
                          </svg>
                        </div>
                      ) : (
                        <div className="size-9 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 shadow-lg shadow-blue-500/30 flex items-center justify-center text-white" title="Admin">
                    ...
                    <div
                      className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-md border ${
                        m.sender === 'me'
                          ? 'bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-600 text-white rounded-tr-md border-transparent shadow-indigo-500/30'
                          : 'bg-white rounded-tl-md border-neutral-200/50'
                      }`}
                    >
                      {m.sender === 'me' ? (
                        <div className="rounded-xl bg-white/95 text-neutral-900 px-3 py-2 shadow-sm">
                      ...
                      <div className={`mt-2 flex items-center gap-3 text-xs font-medium ${m.sender === 'me' ? 'justify-end text-indigo-200' : 'text-neutral-500'}`}>
                ...
                {!msgs.length && (
                  <div className="flex flex-col items-center justify-center py-16">
                    <div className="size-20 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center mb-4">
                      <svg className="size-10 text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
              ...
              <div className="flex flex-col items-center justify-center h-full">
                <div className="size-24 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center mb-4">
                  <svg className="size-12 text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                    <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
                    <path d="M9 10h.01M15 10h.01" strokeWidth="2"/>
                  </svg>
                </div>
                <p className="text-base text-neutral-700 font-semibold">Select a conversation</p>
                <p className="text-sm text-neutral-500 mt-1">Choose a thread from the sidebar to start messaging</p>
              </div>
            )}
          </div>

          {/* Composer */}
          <footer className="sticky bottom-0 inset-x-0 border-t-2 border-blue-200/30 bg-gradient-to-r from-white via-blue-50/20 to-white supports-[backdrop-filter]:backdrop-blur-sm px-6 py-4 shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] items-end gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="rounded-2xl border-2 border-blue-200 bg-white px-4 py-2 focus-within:ring-2 focus-within:ring-blue-500/30 focus-within:border-blue-400 transition-all shadow-sm max-w-full">
                  <textarea
                    ref={taRef}
                    rows={1}
                    placeholder={activeId ? 'Type your message…' : 'Select a conversation to start messaging…'}
                    className="w-full resize-none bg-transparent outline-none max-h-[120px] min-h-[1.5rem] overflow-y-auto text-neutral-900 placeholder:text-blue-400/60"
                  ...
                  <div className="mt-2 flex items-center justify-between flex-wrap gap-2">
                    <p className="text-xs text-neutral-500 font-medium">Press Enter to send • Shift+Enter for new line</p>
                    <button
                      className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-2 text-white font-medium shadow-md shadow-blue-500/30 hover:shadow-lg hover:shadow-blue-500/40 hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 disabled:shadow-none transition-all"
                      onClick={handleSend}
              ...
              <div className="flex items-center gap-2 px-1 pt-2 text-xs text-neutral-600 relative flex-wrap md:justify-end md:self-end md:pl-2 shrink-0">
                <button
                  className="rounded-lg border-2 border-blue-200 bg-blue-50/50 px-3 py-1.5 hover:bg-blue-100 hover:border-blue-300 transition-all font-medium text-blue-700"
                  type="button"
                  onClick={() => {
                ...
                <>
                  ...
                  <label
                    ...
                    className="rounded-lg border-2 border-blue-200 bg-blue-50/50 px-3 py-1.5 hover:bg-blue-100 hover:border-blue-300 transition-all font-medium text-blue-700 cursor-pointer inline-flex items-center gap-1.5"
                  >
                    <svg className="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M7 10v12M21 10v12M12 3l9 7-9 7-9-7 9-7z"/>
                    </svg>
                    Upload
                  </label>
                </>
                <div className="inline-block relative">
                  <button
                    className="rounded-lg border-2 border-blue-200 bg-blue-50/50 px-3 py-1.5 hover:bg-blue-100 hover:border-blue-300 transition-all font-medium text-blue-700 inline-flex items-center gap-1.5"
                    type="button"
