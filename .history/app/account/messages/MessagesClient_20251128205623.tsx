...
async function fetchMessages(threadId: string): Promise<Message[]> {
  try {
    ...
    console.log('fetchMessages debug:', {
      ...
      apiResponse: data,
      allMessageUsers: data.messages?.map((m: any) => ({ 
        id: m.user?.id, 
        role: m.user?.role, 
        email: m.user?.email,
    ...
    console.log('fetchMessages final result:', {
      ...
      messagesSummary: transformedMessages.map(m => ({ id: m.id, sender: m.sender, text: m.text.substring(0, 20) + '...' }))
    });
...
export default function MessagesClient() {
  ...
  useEffect(() => {
    ...
    console.log('Loading messages for active thread:', activeId);
    fetchMessages(activeId).then((m) => {
      if (!mounted) return;
      console.log('Initial load: got', m.length, 'messages for thread', activeId);
      setMsgs(m);
  ...
  useEffect(() => {
    if (!activeId) {
      console.log('Polling: No active thread, skipping poll');
      return;
    }
    
    console.log('Polling: Starting interval for thread', activeId);
    const id = setInterval(async () => {
      console.log('Polling: Fetching messages for thread', activeId);
      try {
        const m = await fetchMessages(activeId);
    ...
    return () => {
      console.log('Polling: Clearing interval for thread', activeId);
      clearInterval(id);
    };
  }, [activeId]);
  ...
  <main className="min-h-0 flex flex-col bg-neutral-25">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-br from-neutral-50 via-blue-50/20 to-neutral-50 overscroll-contain" ref={scrollRef}>
            {activeId ? (
                ...
                {msgs.map((m) => {
                  ...
                  return (
                    <div key={m.id} className={`flex ${isFromMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] ${isFromMe ? 'text-right' : 'text-left'}`}>
                        ...
                        <div className="text-xs mt-1 text-gray-500">
                          {isFromMe ? (me?.name || me?.email || 'You') : 'Support'} • {new Date(m.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </div>
                      </div>
                    </div>
                  );
          ...
          {/* Composer */}
          <footer className="bg-gradient-to-r from-white via-blue-50/20 to-white supports-[backdrop-filter]:backdrop-blur-sm px-6 py-4 border-t-2 border-blue-200/30 shadow-lg">
