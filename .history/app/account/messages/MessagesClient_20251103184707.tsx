...
export default function MessagesClient() {
  ...
  return (
    <section className="h-screen w-full bg-neutral-50 text-neutral-900 flex flex-col">
      {/* Top toggle */}
      <div className="mx-auto w-full max-w-[1400px] px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between text-gray-900">
        <h1 className="text-xl font-bold bg-gradient-to-r from-gray-900 to-blue-600 bg-clip-text text-transparent">Messages</h1>
      </div>

      {mode === 'alerts' ? (
        <div className="mx-auto flex-1 w-full max-w-[1400px] px-4 sm:px-6 lg:px-8 min-h-0 grid grid-cols-[180px_320px_1fr] lg:grid-cols-[200px_360px_1fr] border-2 border-blue-200/50 rounded-2xl overflow-hidden shadow-lg bg-white">
      ...
      <div className="mx-auto flex-1 w-full max-w-[1400px] px-0 sm:px-0 lg:px-0 min-h-0 grid grid-cols-[180px_320px_1fr] lg:grid-cols-[200px_360px_1fr] border border-neutral-200 rounded-2xl overflow-hidden shadow-sm bg-white">
        {/* Far-left: Mode selector */}
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex flex-col gap-2">
                {/* Keep a stable order: Inbox first, Alerts second */}
                <button
                  className={`w-full rounded-lg px-3 py-2 text-sm text-left ${isInbox ? 'bg-neutral-900 text-white' : 'bg-neutral-100 text-neutral-700 hover:bg-neutral-200'}`}
                  onClick={() => setMode('inbox')}
                >Inbox</button>
                
                  <Link
                    href="/account/alerts"
                    className={`w-full rounded-lg px-3 py-2 text-sm text-left bg-neutral-100 text-neutral-700 hover:bg-neutral-200`}
                  >Alerts</Link>
                <Link
                  href="/account/orders/tracking"
                  className={`w-full rounded-lg px-3 py-2 text-sm text-left ${trackingActive ? 'bg-neutral-900 text-white' : 'bg-neutral-100 text-neutral-700 hover:bg-neutral-200'}`}
                >
                  🚚 Order Tracking
                </Link>
              </div>
  ...
  <aside className="min-h-0 flex flex-col border-r border-neutral-200 bg-white">
          <div className="p-4 border-b border-neutral-200">
            ...
            <label className="mt-4 flex items-center gap-2 rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2 focus-within:ring-2 focus-within:ring-neutral-900">
              <svg className="size-4 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="11" cy="11" r="7" strokeWidth="1.5"/><path d="m20 20-3.5-3.5" strokeWidth="1.5"/></svg>
              <input placeholder="Search" className="w-full bg-transparent outline-none placeholder:text-neutral-400" />
            </label>
            {me?.role === 'ADMIN' && (
              <div className="mt-3 rounded-xl border border-neutral-200 bg-neutral-50 p-2">
                <p className="mb-2 text-xs font-medium text-neutral-700">Message a user (admin)</p>
                <div className="relative">
                  <div className="flex items-center gap-2">
                    <input
                      type="email"
