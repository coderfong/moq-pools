...
function formatDate(dt?: string | Date | null, opts: Intl.DateTimeFormatOptions = {}) {
  ...
  try {
    // Use a stable locale/timezone to avoid SSR/CSR hydration mismatches
    const fmt = new Intl.DateTimeFormat('en-US', { timeZone: 'UTC', year: 'numeric', month: 'short', day: '2-digit', ...opts });
    return fmt.format(d);
  } catch {
    return String(d);
  }
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {hoursSinceUpdate(s) >= 72 && normalizeStatus(s.status) !== 'delivered' && (
                      <div className="mt-2 inline-flex items-center gap-2 rounded-md bg-amber-50 px-2 py-1 text-xs text-amber-800">
                        <span className="inline-block size-2 rounded-full bg-amber-500" />
                        <span>No carrier updates in 72h — we can check with the carrier.</span>
                        <button
                          className="underline underline-offset-2"
                          onClick={() => {
