...
export default function AlertsClient({ initialAlerts }: AlertsClientProps) {
  ...
  // Generate smart link based on alert type and context
  const getAlertLink = (alert: Alert): string => {
    // If alert has a specific link, use it
    if (alert.link) {
      // Fix old/incorrect links
      if (alert.link.startsWith('/pools/') && alert.poolId) {
        return `/pools/${alert.poolId}`;
      }
      return alert.link;
    }

    // Generate link based on alert type
    switch (alert.type) {
      case 'GROUP_UPDATE':
        return alert.poolId ? `/pools/${alert.poolId}` : '/pools';
      
      case 'PAYMENT':
        return '/account/orders';
      
      case 'ORDER':
        return alert.poolId ? `/pools/${alert.poolId}` : '/account/orders';
      
      case 'SHIPPING':
        return '/account/orders/tracking';
  ...
  // Get button text based on alert type
  const getAlertButtonText = (alert: Alert): string => {
    switch (alert.type) {
      case 'GROUP_UPDATE':
        return 'View Pool';
      case 'PAYMENT':
  ...
  useEffect(() => {
    setVisibleCount(10);
  }, [filterType, filterStatus, filterProduct]);
                  ...
                  {hasMore && (
                    <div className="flex justify-center pt-4">
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold shadow-md hover:shadow-lg hover:shadow-orange-500/30 hover:scale-105 transition-all"
                      >
                        <span>Show More</span>
                        <span className="text-xs opacity-80">
                          ({filteredAlerts.length - visibleCount} remaining)
                        </span>
