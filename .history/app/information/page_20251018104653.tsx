...
export default function InformationPage() {
    ...
    try {
      const res = await fetch("/api/register/information", {
        ...
        body: JSON.stringify({
          ...
          individualBuyer,
          fullName,
          mobile: `${country.dial} ${mobile}`.trim(),
        }),
      });
      const json = await res.json();
      if (!res.ok || !json?.ok) {
        const reason = json?.reason || "unknown";
        setError(reason === "unauthorized" ? "Please sign in again." : "Failed to submit information.");
        return;
      }
      router.push("/account");
    } catch (err) {
  ...
  <section className="relative w-screen min-h-screen grid lg:grid-cols-12 overflow-hidden" aria-label="Complete registration information section">
        <div className="absolute inset-0 -z-10 pointer-events-none">
          <Image src={BG_URL} alt="" fill priority className="object-contain object-left origin-left scale-[1.14] md:scale-[1.18]" />
        </div>

        <div className="order-2 lg:order-1 lg:col-span-6 xl:col-span-7" />

        <div className="order-1 lg:order-2 lg:col-span-6 xl:col-span-5 px-6 md:px-10 py-10 md:py-12 self-center justify-self-center w-full max-w-[640px]">
