...
export default function AdminAlertsClient({ initialAlerts }: { initialAlerts: any[] }) {
  ...
  const [assigneeFilter, setAssigneeFilter] = React.useState<'all'|'assigned'|'unassigned'|'me'>('all');
  ...
  }, [page, pageSize, readFilter, triageFilter, assigneeFilter, sortBy, sortDir, showPriority, q, dateFrom, dateTo]);
  ...
  return (
    <div className="mt-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        />
        <select className="rounded-lg border border-neutral-200 px-2 py-1 text-sm" aria-label="Assignee filter" value={assigneeFilter} onChange={e => { setAssigneeFilter(e.target.value as any); setPage(1); }}>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <a className="rounded-lg border border-neutral-200 px-3 py-1.5 text-sm hover:bg-neutral-50" href={`#/`} onClick={(ev) => { ev.preventDefault(); const p = new URLSearchParams(); if(readFilter!=='all'){p.set('read',readFilter);} if(triageFilter!=='all'){p.set('triage',triageFilter);} if(assigneeFilter!=='all'){p.set('assignee',assigneeFilter);} if(showPriority) p.set('priority','1'); if(q) p.set('q', q); if(dateFrom) p.set('from', dateFrom); if(dateTo) p.set('to', dateTo); const url = `/api/admin/alerts/export?${p.toString()}`; window.open(url, '_blank'); }}>Export CSV</a>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between text-sm text-neutral-600">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="rounded border px-2 py-1" onClick={async () => {
            const p = new URLSearchParams(); if(readFilter!=='all'){p.set('read',readFilter);} if(triageFilter!=='all'){p.set('triage',triageFilter);} if(assigneeFilter!=='all'){p.set('assignee',assigneeFilter);} if(showPriority) p.set('priority','1'); if(q) p.set('q', q); if(dateFrom) p.set('from', dateFrom); if(dateTo) p.set('to', dateTo);
