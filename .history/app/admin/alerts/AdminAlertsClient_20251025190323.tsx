...
export default function AdminAlertsClient({ initialAlerts }: { initialAlerts: any[] }) {
  ...
  async function fetchPage(resetPage?: boolean) {
    ...
    if (statusFilter === 'unread' || statusFilter === 'read') {
      params.set('read', statusFilter);
    } else if (statusFilter === 'resolved' || statusFilter === 'archived') {
      params.set('triage', statusFilter);
    }
    ...
    <div className="mt-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <a className="rounded-lg border border-neutral-200 px-3 py-1.5 text-sm hover:bg-neutral-50" href={`#/`} onClick={(ev) => { ev.preventDefault(); const p = new URLSearchParams(); if(statusFilter==='unread'||statusFilter==='read'){p.set('read',statusFilter);} else if(statusFilter==='resolved'||statusFilter==='archived'){p.set('triage',statusFilter);} if(showPriority) p.set('priority','1'); if(q) p.set('q', q); if(dateFrom) p.set('from', dateFrom); if(dateTo) p.set('to', dateTo); const url = `/api/admin/alerts/export?${p.toString()}`; window.open(url, '_blank'); }}>Export CSV</a>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between text-sm text-neutral-600">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="rounded border px-2 py-1" onClick={async () => {
            const p = new URLSearchParams(); if(statusFilter==='unread'||statusFilter==='read'){p.set('read',statusFilter);} else if(statusFilter==='resolved'||statusFilter==='archived'){p.set('triage',statusFilter);} if(showPriority) p.set('priority','1'); if(q) p.set('q', q); if(dateFrom) p.set('from', dateFrom); if(dateTo) p.set('to', dateTo);
