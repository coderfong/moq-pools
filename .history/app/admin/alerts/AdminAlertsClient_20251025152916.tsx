...
export default function AdminAlertsClient({ initialAlerts }: { initialAlerts: any[] }) {
  ...
  }, [page, pageSize, statusFilter, showPriority, q, dateFrom, dateTo]);
  ...
  return (
    <div className="mt-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select className="rounded-lg border border-neutral-200 px-2 py-1 text-sm" aria-label="Status filter" value={statusFilter} onChange={e => { setStatusFilter(e.target.value as any); setPage(1); }}>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <a className="rounded-lg border border-neutral-200 px-3 py-1.5 text-sm hover:bg-neutral-50" href={`#/`} onClick={(ev) => { ev.preventDefault(); const p = new URLSearchParams(); if(statusFilter!=='all') p.set('status',statusFilter); if(showPriority) p.set('priority','1'); if(q) p.set('q', q); if(dateFrom) p.set('from', dateFrom); if(dateTo) p.set('to', dateTo); const url = `/api/admin/alerts/export?${p.toString()}`; window.open(url, '_blank'); }}>Export CSV</a>
        </div>
      </div>
                ...
                <tr key={a.id} className="border-t border-neutral-100 align-top">
                  ...
                  <td className="p-2 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      ...
                      <button className="rounded border border-neutral-200 px-2 py-0.5 text-xs hover:bg-neutral-50" onClick={() => { setSelected({ [a.id]: true }); bulkAction('resolve'); }}>Resolve</button>
                      <button className="rounded border border-neutral-200 px-2 py-0.5 text-xs hover:bg-neutral-50" onClick={() => { setSelected({ [a.id]: true }); bulkAction('archive'); }}>Archive</button>
      ...
      <div className="mt-3 flex items-center justify-between text-sm text-neutral-600">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="rounded border px-2 py-1" onClick={async () => {
            const p = new URLSearchParams(); if(statusFilter!=='all') p.set('status',statusFilter); if(showPriority) p.set('priority','1'); if(q) p.set('q', q); if(dateFrom) p.set('from', dateFrom); if(dateTo) p.set('to', dateTo);
