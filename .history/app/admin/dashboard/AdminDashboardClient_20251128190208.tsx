        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          {lowStockPools.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm font-medium text-green-600">No urgent actions needed!</p>
              <p className="text-xs text-gray-500 mt-1">All pools are running smoothly</p>
            </div>
          ) : (
              ...
              {lowStockPools.map((pool) => (
                <div
                  ...
                  className={`p-4 rounded-lg border ${
                    pool.priority === 'urgent' ? 'border-red-500 bg-red-50' :
                    pool.priority === 'high' ? 'border-orange-500 bg-orange-50' :
                    'border-yellow-500 bg-yellow-50'
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-sm truncate flex-1 pr-2">{pool.title}</h3>
                    <span className={`text-xs px-2 py-1 rounded font-medium ${
                      pool.priority === 'urgent' ? 'bg-red-100 text-red-800' :
                      pool.priority === 'high' ? 'bg-orange-100 text-orange-800' :
                  ...
                  <p className="text-sm text-gray-700 mb-3">{pool.actionMessage}</p>
                  
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-600 mb-3">
                    <div>Progress: {pool.progress.toFixed(1)}%</div>
                    <div>Orders: {pool.orderCount}</div>
                    <div>Status: {pool.status}</div>
                    {pool.daysUntilDeadline !== null && (
                      <div>
                        Deadline: {pool.daysUntilDeadline > 0 ? `${pool.daysUntilDeadline}d` : 'Past due'}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Link
                      href={`/pools/${pool.id}`}
                      className={`flex-1 text-center text-xs py-2 px-3 rounded font-medium transition-colors ${
                        pool.priority === 'urgent' ? 'bg-red-600 hover:bg-red-700 text-white' :
                        pool.priority === 'high' ? 'bg-orange-600 hover:bg-orange-700 text-white' :
                    ...
                    <Link
                      ...
                      className="text-xs py-2 px-3 rounded border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors"
                    >
                      Admin View
                    </Link>
                  </div>
