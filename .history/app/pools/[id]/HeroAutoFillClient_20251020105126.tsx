    ...
    try {
      ...
      try {
        ...
        if (tiersHost && progressHost) {
          ...
          tierEls.forEach((el) => {
            const strong = el.querySelector('span.font-semibold');
            const rangeSpan = el.querySelector('span.ml-2');
            const price = (strong?.textContent || '').trim();
            const label = (rangeSpan?.textContent || '').trim();
          ...
          if (parsed.length) {
            ...
            wrap.className = 'pricing-card';
            wrap.innerHTML = `
              <div class="qty-row" style="display:flex;align-items:center;gap:10px;margin:10px 0 10px;">
                <label for="qty" style="font-weight:600;font-size:14px;color:#111;">Qty</label>
                <input id="qty" type="number" value="${Math.max(1, parsed[0].min)}" min="1" step="1" style="width:120px;padding:8px 10px;border:1px solid #e5e7eb;border-radius:8px;font-size:14px;color:#111;outline:none;" />
              </div>
              <div class="progress-wrap" style="margin-top:6px;">
                <div class="progressbar" role="progressbar" aria-label="Order quantity toward best price tier" aria-valuemin="0" aria-valuemax="${MAX}" aria-valuenow="0" id="progressbar" style="position:relative;height:14px;border-radius:999px;background:#f3f4f6;overflow:hidden;border:1px solid #e5e7eb;">
                  <div class="fill" id="fill" style="position:absolute;inset:0 auto 0 0;width:0%;background:#111;transition:width .25s ease;"></div>
                </div>
                <div class="legend" style="display:flex;justify-content:space-between;font-size:12px;color:#6b7280;margin-top:8px;"></div>
              </div>
            `;
            ...
            // Add ticks
            const addTick = (xPct: number, label: string) => {
              ...
              em.style.top = '100%';
              em.style.left = '50%';
              em.style.transform = 'translate(-50%,6px)';
            ...
            const labels = ['Starter', 'Better', 'Best'];
            const segs = Math.min(3, parsed.length);
            legend.innerHTML = '';
            ...
            const highlightTier = (q: number) => {
              tierEls.forEach((el, idx) => {
                ...
                // Minimal active styling
                (el as HTMLElement).style.borderRadius = '999px';
                (el as HTMLElement).style.padding = '6px 10px';
                (el as HTMLElement).style.background = el.classList.contains('active') ? '#111' : 'transparent';
                (el as HTMLElement).style.color = el.classList.contains('active') ? '#fff' : '#111';
                const strong = el.querySelector('span.font-semibold') as HTMLElement | null;
                const range = el.querySelector('span.ml-2') as HTMLElement | null;
                if (strong) strong.style.color = el.classList.contains('active') ? '#fff' : '#111';
                if (range) range.style.color = el.classList.contains('active') ? '#fff' : '#6b7280';
              });
            };
