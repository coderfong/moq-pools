...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  // Best-effort detail fetch with a short SSR budget to avoid slow first paint
  let detail: any = null;
  try {
    const mod = await import('@/lib/providers/detail');
    const fn = force ? mod.refreshProductDetail : mod.fetchProductDetailCached;
    const FETCH_BUDGET_MS = 1200; // don't block render beyond ~1.2s
    const bg = fn(listing).catch(() => null);
    const timed = new Promise<any>((resolve) => {
      const to = setTimeout(() => resolve(null), FETCH_BUDGET_MS);
      bg.then((v) => { clearTimeout(to); resolve(v); }).catch(() => { clearTimeout(to); resolve(null); });
    });
    detail = await timed;
    // Allow the background promise to continue even if we timed out
    // (it will persist detailJson for future requests via fetchProductDetailCached)
  } catch {}
  ...
  const soldCount: number | undefined = (detail as any)?.soldCount != null ? Number((detail as any).soldCount) : undefined;
              ...
              <div class="flex items-baseline justify-between gap-3">
                ...
                ${(() => {
                  ...
                  if (typeof ratingVal === 'number' && !isNaN(ratingVal)) {
                    ...
                    const rc = (typeof reviewsCount === 'number' && isFinite(reviewsCount)) ? ` (${reviewsCount.toLocaleString()} review${reviewsCount === 1 ? '' : 's'})` : '';
                    parts.push(`${r}${rc}`);
                  } else if (typeof reviewsCount === 'number' && isFinite(reviewsCount)) {
                    parts.push(`${reviewsCount.toLocaleString()} review${reviewsCount === 1 ? '' : 's'}`);
                  }
                  if (typeof soldCount === 'number' && isFinite(soldCount)) {
                    parts.push(`${soldCount.toLocaleString()} sold`);
                  }
                ...
                ${priceTiers.length ? (() => {
                  ...
                  <div class="tp-inline">
                          ...
                          <div class=\"chip inline-flex items-baseline gap-2 rounded-full px-3 py-1\">
                            ...
                            <i class=\"rng not-italic\">${esc(
                              r.max === 'inf'
                                ...
                                : (Number(r.max) === r.min
                                    ? `${r.min.toLocaleString()}`
                                    : `${r.min.toLocaleString()}–${Number(r.max).toLocaleString()}`)
                            )}</i>
                          </div>
                        </div>
                      `).join('')}
                    ...
                    <div class="relative mt-2 w-full h-5 text-[12px] text-gray-500">
                      ${stops.map(s => `<span class=\"absolute\" style=\"left:${pct(Number(s)).toFixed(2)}%;transform:translateX(-50%)\">${Number(s).toLocaleString()}</span>`).join('')}
