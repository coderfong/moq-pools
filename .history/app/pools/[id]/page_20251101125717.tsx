            ...
            <a href=\"/pools/${esc(id)}?refresh=1\" class=\"text-xs text-gray-600 underline hover:text-gray-900\">Refresh details${isWeak ? ' (fetched live)' : ''}</a>
              ...
              <div class="mt-2 text-black">
                ${priceTiers.length ? (() => {
                  ...
                  // Use a logarithmic scale so small ranges (e.g., 0–30) don't overlap near the origin
                  const log10 = (x: number) => Math.log10(Math.max(1, x + 1)); // shift by +1 to handle 0 safely
                  const pct = (q: number) => {
                    const lo = log10(start);
                    const hi = log10(domainEnd);
                    const denom = Math.max(1e-6, (hi - lo));
                    const v = (log10(q) - lo) / denom;
                    return Math.max(0, Math.min(100, v * 100));
                  };
                  // Place markers at the geometric mean of each tier's bounds in log-space
                  const tiersMid = raw.map(r => {
                    const a = Math.max(start, r.min);
                    const b = r.max === 'inf' ? domainEnd : Number(r.max);
                    const gm = Math.pow(10, (log10(a) + log10(b)) / 2) - 1; // invert (x+1) shift
                    return { ...r, mid: gm } as T;
                  });
