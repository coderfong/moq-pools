...
export default function HeroAutoFillClient() {
  useEffect(() => {
    try {
      ...
      if (!variations.length) {
        ...
        try {
          const vars = figs
            ...
            .filter((v) => v.label || v.img);
          if (vars.length) variations = vars;
        } catch {}
      }
      ...
      // ---------- Normalize ----------
      // Placeholders for optional sections when not found in-page
      const customizationOptions: { name: string; addOn?: string; moq?: string }[] = [];
      ...
      try {
        ...
        if (rightCol && !hasVariationsSSR && productData.variations && productData.variations.length) {
          ...
          sec.className = 'mt-8';
          sec.setAttribute('data-variations-injected', 'true');
      ...
      try {
        const rightCol = pool.querySelector('.space-y-8.h-full') as HTMLElement | null;
        const hasVariationsSSR = !!(rightCol && Array.from(rightCol.querySelectorAll('h2')).some(h => /variations/i.test((h.textContent || '').trim())));
        const hasInjected = !!(rightCol && rightCol.querySelector('[data-variations-injected]'));
        if (rightCol && !hasVariationsSSR && !hasInjected) {
          (async () => {
            // Try to locate the original listing URL from the header action or data attribute
            ...
            const dataSrc = (document.querySelector('.pool-hero-right') as HTMLElement | null)?.getAttribute('data-source-url') || '';
            const srcUrl = linkEl?.href || dataSrc || '';
            if (srcUrl) {
              const base = (process.env.NEXT_PUBLIC_BASE_URL || '').trim();
              const api = `${base}/api/external/detail-html?src=${encodeURIComponent(srcUrl)}`.replace(/^\/?\/?/, '/');
              ...
              if (html) {
                ...
                const imgsA = Array.from(doc.querySelectorAll('[data-testid="sku-list"] [data-testid="sku-list-item"] img')) as HTMLImageElement[];
                const imgsB = Array.from(doc.querySelectorAll('[data-testid="double-bordered-box"] img')) as HTMLImageElement[];
                const imgs = [...imgsA, ...imgsB];
                const seenSrc = new Set<string>();
                const isPlaceholder = (u: string) => {
                  const low = u.toLowerCase();
                  return /tps-\d+-\d+\.png$/.test(low) || /@img\//.test(low) || /(sprite|logo|favicon|badge|watermark)/.test(low);
                };
                const vars = imgs.map((img, i) => {
                  let src = img.getAttribute('src') || img.getAttribute('data-src') || '';
                  if (src.startsWith('//')) src = 'https:' + src;
                  if (!src || isPlaceholder(src)) return { src: '', alt: '' };
                  const alt = img.getAttribute('alt') || `Variant ${i+1}`;
                  return { src, alt };
                }).filter(v => v.src && (seenSrc.has(v.src) ? false : (seenSrc.add(v.src), true)));
                if (vars.length) {
                  const sec = document.createElement('section');
                  sec.className = 'mt-8';
                  sec.setAttribute('data-variations-injected', 'true');
      ...
      // ---------- LEFT COLUMN: Embed provider SKU module below the Join/Protection section ----------
      try {
        let anchor = document.querySelector('[data-variations-anchor]') as HTMLElement | null;
        if (!anchor) {
          // Fallback: legacy selector for the actions block under the left image
          anchor = document.querySelector('.pool-hero-left .mt-3.flex.flex-col.items-center.gap-2') as HTMLElement | null;
        }
        if (!anchor) {
          console.debug('[Variations] No anchor found (data-variations-anchor)');
        }
        // Prevent duplicate injections (section right after the anchor)
        if (anchor && anchor.nextElementSibling && (anchor.nextElementSibling as HTMLElement).hasAttribute('data-variations-injected')) {
          console.debug('[Variations] Already injected under anchor');
        } else if (anchor) {
          (async () => {
            // Resolve source URL from nearest ancestor with data-source-url, then global
            const nearest = anchor.closest('[data-source-url]') as HTMLElement | null;
            const globalSrc = document.querySelector('[data-source-url]') as HTMLElement | null;
            const srcUrl = (nearest?.getAttribute('data-source-url') || globalSrc?.getAttribute('data-source-url') || '').trim();
            if (!srcUrl) {
              console.debug('[Variations] No data-source-url found');
              return;
            }
            const api = `/api/external/alibaba/sku?src=${encodeURIComponent(srcUrl)}`;
            console.debug('[Variations] Fetching', api);
            const resp = await fetch(api, { cache: 'no-store', credentials: 'same-origin' }).catch(() => null);
            if (!resp || !resp.ok) {
              console.debug('[Variations] API fetch failed', resp?.status);
              return;
            }
            const data = await resp.json().catch(() => null) as { html?: string; found?: boolean } | null;
            const html = (data && typeof data.html === 'string') ? data.html : '';
            if (!html) {
              console.debug('[Variations] No fragment returned');
              return;
            }
            // Build a styled section wrapper and inject below anchor; insert HTML directly
            const section = document.createElement('section');
            section.className = 'mt-8';
            section.setAttribute('data-variations-injected', 'true');
            section.innerHTML = `
              <h2 class="text-lg font-semibold text-gray-900">Variations</h2>
              <div class="mt-3 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm p-4"></div>
            `;
            const slot = section.querySelector('div')!;
            // Prefer extracting SKU items and rendering with our stable UI, fallback to raw fragment
            try {
              const parser = new DOMParser();
              const doc = parser.parseFromString(html, 'text/html');
              const imgsA = Array.from(doc.querySelectorAll('[data-testid="sku-list"] [data-testid="sku-list-item"] img')) as HTMLImageElement[];
              ...
              if (vars.length) {
                ...
                `).join('');
                slot.innerHTML = `<div class="p-3 grid grid-cols-[repeat(auto-fit,minmax(88px,1fr))] gap-3">${items}</div>`;
              } else {
                // Fallback: insert provider fragment verbatim
                slot.insertAdjacentHTML('beforeend', html);
                slot.querySelectorAll('img[src^="//"]').forEach((img) => {
                  const s = img.getAttribute('src');
                  if (s) img.setAttribute('src', 'https:' + s);
                });
              }
            } catch {
              slot.insertAdjacentHTML('beforeend', html);
            ...
            console.debug('[Variations] Injected module_sku under Join Pool');
          })();
        }
      } catch {}
          ...
          section.innerHTML = `
            ...
            <div style="display:grid;gap:16px">
              ...
              <div>
                ...
                <ul style="margin:8px 0 0 16px;">
                  ${productData.customizationOptions
                    .map(
                      (c: { name: string; addOn?: string; moq?: string }) => `
              ...
              <div>
                ...
                <ul style="margin:8px 0 0 16px;">
                  ${productData.supplierAbilities.map((a: string) => `<li style="margin:4px 0;">${a}</li>`).join('')}
                </ul>
              </div>`
                : ``}
