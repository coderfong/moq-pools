...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  async function computeHeroImage(): Promise<string> {
    ...
    const gallery0: string = Array.isArray(detail?.gallery) && detail!.gallery!.length ? String(detail!.gallery![0]) : '';
    ...
    // 3) Try detail gallery first image
    if (gallery0) {
      if (gallery0.startsWith('/cache/') && !isBadImageHashFromPath(gallery0)) {
        return gallery0;
      }
      if (gallery0.startsWith('http://') || gallery0.startsWith('https://') || gallery0.startsWith('//')) {
        try {
          const { localPath } = await cacheExternalImage(gallery0, { preferJpgForIndiaMart: preferIM });
          if (localPath && !isBadImageHashFromPath(localPath)) return localPath;
        } catch {}
      }
    }
    ...
    return fallback || raw || gallery0 || '/seed/sleeves.jpg';
  }
