...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  try {
    ...
    // If Alibaba listing and social proof missing, try a one-time live refresh to populate rating/sold
    try {
      const plat = String((listing as any)?.platform || '').toLowerCase();
      const u = String((listing as any)?.url || '');
      const isAlibaba = plat.includes('alibaba') || /alibaba\./i.test(u);
      const hasSocial = !!(detail?.rating?.value || detail?.rating?.count || detail?.soldCount);
      if (!force && isAlibaba && !hasSocial) {
        const fresh = await mod.refreshProductDetail(listing);
        if (fresh && (fresh as any)) detail = fresh;
      }
    } catch {}
  } catch {}
  ...
  const ratingValue: number | undefined = (detail as any)?.rating?.value;
  const ratingCount: number | undefined = (detail as any)?.rating?.count;
            ...
            <div class="pool-hero-right min-w-0">
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  ...
                  ${(() => {
                    const parts: string[] = [];
                    if (typeof ratingValue === 'number' && isFinite(ratingValue)) {
                      const rv = ratingValue.toFixed(1);
                      if (typeof ratingCount === 'number' && isFinite(ratingCount)) {
                        const rc = Number(ratingCount).toLocaleString();
                        parts.push(`${rv} (${rc} review${ratingCount === 1 ? '' : 's'})`);
                      } else {
                        parts.push(`${rv}`);
                      }
                    } else if (typeof ratingCount === 'number' && isFinite(ratingCount)) {
                      const rc = Number(ratingCount).toLocaleString();
                      parts.push(`${rc} review${ratingCount === 1 ? '' : 's'}`);
                    }
                    if (typeof soldCount === 'number' && isFinite(soldCount)) {
                      const sc = Number(soldCount).toLocaleString();
                      parts.push(`${sc} sold`);
                    }
                    return parts.length ? `<div class=\"mt-1 text-sm text-gray-700\">${parts.join(' • ')}<\/div>` : '';
                  })()}
                </div>
              ...
              <div class="mt-2 text-black">
                ${priceTiers.length ? (() => {
                  ...
                  // Create a virtual tail width so the ∞ tier has visible span; use last segment width or 20% of domain as fallback
                  ...
                  const domainTail = Math.max(lastSegWidth, Math.round((endFinite - start) * 0.2));
                  const hasInf = raw.some(r => r.max === 'inf');
                  const domainEnd = hasInf ? (endFinite + domainTail) : endFinite;
                  ...
                  const stops = [
                      start,
                      ...raw.map(r => r.min),
                      ...raw.map(r => (r.max === 'inf' ? NaN : Number(r.max))).filter(n => Number.isFinite(n)) as number[]
                    ]
