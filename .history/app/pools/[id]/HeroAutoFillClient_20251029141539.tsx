      ...
      try {
        ...
        if (anchor && anchor.nextElementSibling && (anchor.nextElementSibling as HTMLElement).hasAttribute('data-variations-injected')) {
          ...
          (async () => {
            ...
            const api = `/api/external/detail-html?src=${encodeURIComponent(srcUrl)}`;
            console.debug('[Variations] Fetching', api);
            const resp = await fetch(api, { cache: 'no-store', credentials: 'same-origin' }).catch(() => null);
            if (!resp || !resp.ok) {
              console.debug('[Variations] detail-html fetch failed', resp?.status);
              return;
            }
            const data = await resp.json().catch(() => null) as any;
            const html = (data && typeof data.html === 'string') ? data.html : '';
            if (!html) {
              console.debug('[Variations] Empty HTML payload');
              return;
            }
            const parser = new DOMParser();
            const doc = parser.parseFromString(html, 'text/html');
            // Prefer the full module container if available
            let skuEl: Element | null =
              doc.querySelector('[data-module-name="module_sku"]') ||
              doc.querySelector('[data-testid="sku-layout"]');
            // Fallback: reconstruct using only the inner list
            if (!skuEl) {
              const list = doc.querySelector('[data-testid="sku-list"]');
              if (list) {
                const wrapper = doc.createElement('div');
                wrapper.setAttribute('data-module-name', 'module_sku');
                wrapper.className = 'module_sku';
                wrapper.innerHTML = `
                  <div data-testid="sku-layout" data-show-type="skuSummary">
                ...
                wrapper.querySelector('[data-testid="sku-layout"]')?.appendChild(list.cloneNode(true));
                skuEl = wrapper;
              }
            }
            if (!skuEl) {
              console.debug('[Variations] No SKU module found in provider HTML');
              return;
            }
            // Normalize // URLs to https://
            skuEl.querySelectorAll?.('img[src^="//"]').forEach?.((img: any) => {
              const src = img.getAttribute('src');
              if (src && src.startsWith('//')) img.setAttribute('src', 'https:' + src);
            });
            // Build a styled section wrapper and inject below anchor
            ...
            `;
            section.querySelector('div')!.appendChild(skuEl as Node);
