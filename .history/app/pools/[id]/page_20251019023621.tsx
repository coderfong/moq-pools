...
import { fetchProductDetailCached } from '@/lib/providers/detail';

export const dynamic = 'force-dynamic';

export default async function PoolDetailPage({ params }: { params: { id: string } }) {
  const { id } = params;
  const listing = await prisma.savedListing.findUnique({ where: { id }, select: { id: true, title: true, image: true, url: true, platform: true, categories: true, terms: true, description: true, detailJson: true, detailUpdatedAt: true } });
  if (!listing) return notFound();
  const esc = (s: string) => String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\"/g, '&quot;').replace(/'/g, '&#39;');
  // Fetch dynamic product details with caching (best-effort; falls back to SavedListing fields)
  const detail = await fetchProductDetailCached(listing).catch(() => null);
  const gallery = Array.isArray(detail?.gallery) ? detail!.gallery! : [];
  const primaryImg = gallery[0] || listing.image || '/seed/sleeves.jpg';
