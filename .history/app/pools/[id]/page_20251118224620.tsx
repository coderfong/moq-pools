...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  try {
    ...
    // SYNTHETIC ATTRIBUTES: When scraping is blocked or detail is missing, generate useful attributes from listing data
    if ((isWeak || !detail) && (!detail?.attributes || detail.attributes.length === 0)) {
      // Initialize detail object if it's null
      if (!detail) detail = {};
    ...
    if ((isWeak || !detail) && (!detail?.packaging || detail.packaging.length === 0)) {
      detail.packaging = [
    ...
    if ((isWeak || !detail) && (!detail?.protections || detail.protections.length === 0)) {
      detail.protections = [
        {
