...
import DetailHtmlClient from './DetailHtmlClient';
...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  async function computeHeroImage(): Promise<string> {
    ...
    // 1) If parser resolved a heroImage, try caching and use it (prefer parser over older cached thumbs)
    if (hero) {
    ...
    // 2) Prefer already-cached listing image if valid (after parser hero check)
  ...
  return (
    <div className="px-6 md:px-10 xl:px-16 py-6">
        ...
        {process.env.NODE_ENV !== 'production' ? (
          <div className="mt-4 text-xs text-gray-600">
            <details>
              ...
              <pre className="mt-2 overflow-x-auto bg-gray-50 p-2 rounded border border-gray-200">
                {JSON.stringify({
                  sourceUrl: listing.url || null,
                  priceText: (detail as any)?.priceText || null,
                  priceTiers: Array.isArray((detail as any)?.priceTiers) ? (detail as any).priceTiers.length : 0,
                  heroImage: !!(detail as any)?.heroImage,
                  attributes: Array.isArray((detail as any)?.attributes) ? (detail as any).attributes.length : 0,
                  packaging: Array.isArray((detail as any)?.packaging) ? (detail as any).packaging.length : 0,
                  protections: Array.isArray((detail as any)?.protections) ? (detail as any).protections.length : 0,
                  supplier: {
                    name: (detail as any)?.supplier?.name || null,
                    logo: !!(detail as any)?.supplier?.logo,
                    profileUrl: (detail as any)?.supplier?.profileUrl || null,
                  },
                  isWeak,
                }, null, 2)}
              </pre>
      ...
      {/* Always offer a sanitized details fallback; loads lazily on expand */}
      {listing.url ? (
        <div className="mt-4">
          <DetailHtmlClient src={String(listing.url)} autoOpen={false} />
        </div>
      ) : null}
