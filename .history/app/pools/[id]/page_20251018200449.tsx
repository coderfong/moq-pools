...
export default async function PoolDetailPage({ params }: { params: { id: string } }) {
  const { id } = params;
  const listing = await prisma.savedListing.findUnique({ where: { id }, select: { id: true, title: true, image: true, url: true, platform: true, categories: true } });
        ...
        <div class="sr-proMainInfo-baseInfo" faw-module="main_function" faw-exposure="" faw-id="1j7rgj09j70a">
          <div class="sr-proMainInfo-baseInfo-name">
            <h1 class="sr-proMainInfo-baseInfoH1 J-baseInfo-name" data-prod-tag="">
              <span class="J-meiAward"></span> Fashionable Electric Plastic Lighters for Outdoor Picnic Smokers
            </h1>
                      ...
                      <tbody>
                        <tr style="display: inline-flex;align-items: baseline; flex-wrap: wrap;flex-direction: column;" class="only-one-priceNum-tr">
                          <td style="padding: 0 !important;">
                            <span class="only-one-priceNum-td-left">US$0.04 - 0.056</span>
                          </td>
                          <td style="padding: 0" class="sa-only-property-price only-one-priceNum-price">
                            <span style="color: #222">300,000 PCS</span>
                            <span style="color: #222">(MOQ)</span>
                          </td>
                        </tr>
                      </tbody>
                    </table>
              ...
              <table>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>Disposable, Refillable</td>
                  </tr>
                  <tr>
                    <th width="166" class="th-label">Usage:</th>
                    <td>Kitchen, BBQ, Torch</td>
                  </tr>
                </tbody>
              </table>
              <div>
                <div class="sr-layout-block contact-block J-contact-fix two-button">
                  <div class="button-block contact-btn">
                    <a fun-inquiry-product="" class="btns button-link-contact" target="_blank" href="https://www.made-in-china.com/sendInquiry/prod_kxjUuCLYnwrN_hwPtnvWOZgDz.html?plant=en&amp;from=shrom&amp;type=cs&amp;style=3&amp;page=p_detail" rel="nofollow" ads-data="st:24,pdid:kxjUuCLYnwrN,pcid:hwPtnvWOZgDz"><i class="ob-icon icon-mail"></i> Contact Supplier </a>
                  </div>
                ...
                <div faw-module="verified_company_homepage" faw-exposure="" faw-id="1j7rgj09k41a">
                  ...
                  <div class="sr-linkTo-comInfo J-linkTo-comInfo" ads-data="">
                    <div class="sr-com com-place-one com-place-one-new">
                      ...
                      <div class="sr-com-info">
                        <div class="sr-comInfo-title has360 ">
                          <div class="title-txt">
                            <a href="https://lianhualigher.en.made-in-china.com" target="_blank" ads-data="st:3" title="Shaodong Lianhua Lighter Manufacturing Co., Ltd."><span class="text-ellipsis">Shaodong Lianhua Lighter Manufacturing Co., Ltd.</span> <i class="ob-icon icon-right"></i></a>
                          </div>
                        </div>
                        <div class="info-item-out">
                          <span class="info-item info-businessType" title="Manufacturer/Factory &amp; Trading Company" ads-data="st: 237;">Manufacturer/Factory &amp; Trading Company</span>
                        </div>
                      </div>
                      <div class="operate-wrapper">
                        ...
                        <div class="company-location" ads-data="st:225">
                          <a class="J-location" target="_blank" href="https://lianhualigher.en.made-in-china.com/company-Shaodong-Lianhua-Lighter-Manufacturing-Co-Ltd-.html">
                            <i class="ob-icon icon-coordinate"></i>
                            <div class="tip arrow-bottom tip-gold"><div class="gold-content"><div class="tip-con">Hunan, China</div><span class="arrow arrow-out"><span class="arrow arrow-in"></span></span></div></div>
                          </a>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="sr-comInfo-sign com-place-two com-place-two-new ">
                      <div class="sign-item" ads-data="st:186"><i class="item-icon icon-diamond"></i> Diamond Member <span class="txt-year">Since 2025</span></div>
                    </div>
                  </div>
        ...
        <nav aria-label="breadcrumb" className="w-full">
          <ol className="flex flex-wrap items-center gap-1">
            {(() => {
              ...
              let siteLabel = platformLabel(plat as any) || plat;
              try {
                if (listing.url) {
              ...
              // Map category keys to labels when known; limit depth to max 3
              const catsRaw = Array.isArray(listing.categories) ? listing.categories : [];
              const MAX_LEVELS = 3;
              const keyToLabel = new Map(CATEGORIES.map(c => [c.key, c.label] as const));
              const cats = catsRaw.slice(0, MAX_LEVELS).map(c => {
                const raw = String(c || '').trim();
                const label = keyToLabel.get(raw) || raw;
                return { key: raw, label };
              }).filter(c => !!c.label);
              for (const c of cats) {
                crumbs.push({ label: c.label, href: `/products?platform=${encodeURIComponent(plat)}&q=${encodeURIComponent(c.label)}` });
              }
