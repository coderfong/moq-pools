          ...
          <div class="pool-hero grid grid-cols-1 md:grid-cols-[360px_1fr] gap-6 items-start">
            ...
            <div class="pool-hero-right min-w-0">
              ...
              <div class="mt-2 text-black">
                ${priceTiers.length ? `
                <header id="tiers" class="flex flex-wrap items-baseline gap-3">
                  ${priceTiers.map((t, idx) => `
                    <span class=\"tier inline-flex items-baseline gap-2 rounded-full px-3 py-1\">
                      <strong class=\"font-semibold\">${esc(t.price)}</strong>
                      <i class=\"not-italic text-sm\">${esc(normalizeTierRange(t.range))}</i>
                    </span>
                    ${idx < priceTiers.length - 1 ? '<span class=\\"text-gray-400\\">•</span>' : ''}
                  `).join('')}
                </header>
                <div id="pricing-progress" class="mt-3"></div>
                ` : `
                <div class="text-base">
