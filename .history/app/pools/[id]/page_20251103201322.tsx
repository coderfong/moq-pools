...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  const html = `
          ...
          <div class="pool-hero grid grid-cols-1 md:grid-cols-[400px_1fr] gap-8 items-start" data-source-url="${listing.url ? esc(String(listing.url)) : ''}">
            <div class="pool-hero-left">
              <div class="relative w-full aspect-square bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl overflow-hidden border-2 border-orange-200/50 shadow-xl">
                <img src="${imgSrc}" alt="${titleText}" class="w-full h-full object-cover" referrerpolicy="no-referrer" />
              </div>
              <div class="mt-4 flex flex-col items-center gap-3" data-variations-anchor>
                <a href="/checkout?listingId=${esc(id)}" class="inline-flex items-center justify-center rounded-full font-bold btn-mag shadow-lg hover:shadow-xl text-xl px-8 py-4 w-full text-white">Join Pool</a>
                <a href="/information/payment-protection" class="text-sm text-orange-600 font-medium underline hover:text-orange-700">🔒 How your payment is protected</a>
              </div>
            </div>
            <div class="pool-hero-right min-w-0" data-source-url="${listing.url ? esc(String(listing.url)) : ''}">
              <div class="flex items-baseline justify-between gap-4">
                <h1 class="text-3xl font-bold bg-gradient-to-r from-gray-900 via-orange-600 to-amber-600 bg-clip-text text-transparent">${titleText}</h1>
                <div id="pool-timer" data-deadline="${timerDeadlineISO}" class="text-sm md:text-base text-gray-900 flex-shrink-0">
                  <span class="inline-flex items-center gap-1.5 rounded-full border-2 border-orange-200 bg-gradient-to-r from-orange-50 to-amber-50 px-4 py-2 font-semibold shadow-sm">
