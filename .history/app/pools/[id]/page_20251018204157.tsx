...
export default async function PoolDetailPage({ params }: { params: { id: string } }) {
  const { id } = params;
  const listing = await prisma.savedListing.findUnique({ where: { id }, select: { id: true, title: true, image: true, url: true, platform: true, categories: true, terms: true, description: true } });
  if (!listing) return notFound();
  const esc = (s: string) => String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\"/g, '&quot;').replace(/'/g, '&#39;');
  const imgSrc = (listing.image && listing.image.trim().length) ? listing.image : '/seed/sleeves.jpg';
