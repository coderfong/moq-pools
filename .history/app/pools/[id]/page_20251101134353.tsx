...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  // HARD refresh: bypass all TTL/memos and then re-read from DB in the same request
  let freshListing: any = listing;
  let detail: any = null;
  if (force) {
    try {
      const { refreshProductDetail } = await import('@/lib/providers/detail');
      // Prefer headless for tougher pages by default on forced refresh
      if (process.env.SCRAPE_HEADLESS == null) process.env.SCRAPE_HEADLESS = '1';
      await refreshProductDetail(listing);
      // Always re-pull the latest persisted payload after an optional refresh
      freshListing = await prisma.savedListing.findUnique({ where: { id } });
      detail = (freshListing as any)?.detailJson ?? null;
    } catch {}
  } else {
    try {
      const mod = await import('@/lib/providers/detail');
      detail = await mod.fetchProductDetailCached(listing);
    } catch {}
  }
  ...
  try {
    const { normalizeDetail, isCorrect } = await import('@/lib/detail-contract');
    normalized = normalizeDetail(
      {
        title: String((detail as any)?.title || ''),
        priceText: (detail as any)?.priceText ?? null,
        priceTiers: Array.isArray((detail as any)?.priceTiers) ? (detail as any).priceTiers : [],
        soldCount: (detail as any)?.soldCount ?? null,
        attributes: Array.isArray((detail as any)?.attributes) ? ((detail as any).attributes as Array<{label:string; value:string}>).map(p => [String(p.label||''), String(p.value||'')] as [string,string]) : [],
        packaging: Array.isArray((detail as any)?.packaging) ? ((detail as any).packaging as Array<{name:string; value:string}>).map(p => [String(p.name||''), String(p.value||'')] as [string,string]) : [],
        protections: Array.isArray((detail as any)?.protections) ? ((detail as any).protections as Array<{header?:string; body?:string}>).map(p => [p.header, p.body].filter(Boolean).join(': ').trim()).filter(Boolean) : [],
      ...
      {
        title: String((freshListing ?? listing).title || ''),
        priceRaw: (freshListing ?? listing as any)?.priceRaw ?? null,
        priceMin: (freshListing ?? listing as any)?.priceMin ?? null,
        priceMax: (freshListing ?? listing as any)?.priceMax ?? null,
        currency: (freshListing ?? listing as any)?.currency ?? null,
        ordersRaw: (freshListing ?? listing as any)?.ordersRaw ?? null,
        image: (freshListing ?? listing as any)?.image ?? null,
      }
    );
  ...
  try { isWeak = !isCorrect(normalized as any); } catch {}
  } catch {}
  ...
  if (!force) {
    try {
      if (isWeak) {
        const { refreshProductDetail } = await import('@/lib/providers/detail');
        if (process.env.SCRAPE_HEADLESS == null) process.env.SCRAPE_HEADLESS = '1';
        const fresh = await refreshProductDetail(freshListing ?? listing).catch(() => null);
        if (fresh) {
          // Re-read saved listing after heal to avoid rendering stale JSON
          try { freshListing = await prisma.savedListing.findUnique({ where: { id } }) as any; } catch {}
          detail = (freshListing as any)?.detailJson ?? fresh;
          try {
            const { normalizeDetail, isCorrect } = await import('@/lib/detail-contract');
            normalized = normalizeDetail(
              {
  ...
  const keyAttrSec2 = (Array.isArray(detail?.attributes) && detail.attributes.length) ? `
                <section class=\"mt-8 h-full flex flex-col\">
                  <h2 class=\"text-lg font-semibold text-gray-900\">Key attributes</h2>
                  <div class=\"mt-3 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm flex-1\">
                    <table class=\"sec-table w-full text-sm\"><tbody>
                        ${detail.attributes.map((r: any, i: number) => `<tr key=\"${esc(r.label)}|${esc(r.value)}|${i}\"><th width=\"166\" class=\"th-label\">${esc(toTitleCase(r.label))}</th><td>${esc(r.value)}</td></tr>`).join('')}
                    </tbody></table>
                  </div>
                ...
                <div class=\"mt-3 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm\">
                  <div class=\"p-3 grid grid-cols-[repeat(auto-fit,minmax(88px,1fr))] gap-3\">
                    ${variations.map((v: any, i: number) => `
                      <figure key=\"var-${i}\" class=\"flex flex-col items-center text-center\">
                        ${v.img ? `<img src=\"${esc(String(v.img || '').replace(/^\/\//, 'https://'))}\" alt=\"${esc(String(v.label || ''))}\" class=\"w-20 h-20 object-contain rounded-lg border border-gray-200\" referrerpolicy=\"no-referrer\" />` : ''}
                        <figcaption class=\"mt-1 text-xs text-gray-700 line-clamp-2\">${esc(String(v.label || ''))}</figcaption>
                      </figure>
                    `).join('')}
                  </div>
                </div>
  ...
  const protectionsSec = (Array.isArray(protections) && protections.length) ? `
              <section class=\"mt-8\">
                ...
                <div class=\"mt-3 grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-4\">
                  ${protections.map((p, i) => `
                    <div key=\"prot-${i}\" class=\"prot-card bg-white border border-gray-200 rounded-xl p-4 shadow-sm\"> 
  ...
  const html = `
          <style>
            ...
            .sec-table th.th-label { text-align:left; color:#374151; font-weight:600; padding:10px 12px; width:180px; background:rgba(0,0,0,0.03); }
            .sec-table td { padding:10px 12px; color:#111827; }
            .sec-table tr:nth-child(odd) td { background:#fafafa; }
            .prot-card { transition: box-shadow .2s ease; }
            .prot-card:hover { box-shadow: 0 10px 22px rgba(0,0,0,.08); }
          </style>
          <div class="pool-hero grid grid-cols-1 md:grid-cols-[360px_1fr] gap-6 items-start" data-source-url="${listing.url ? esc(String(listing.url)) : ''}">
            <div class="pool-hero-left">
              <div class="w-[360px] h-[360px] bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
                <img src="${imgSrc}" alt="${titleText}" class="w-full h-full object-cover" referrerpolicy="no-referrer" />
              </div>
              <div class="mt-3 flex flex-col items-center gap-2" data-variations-anchor>
            ...
            <div class="pool-hero-right min-w-0" data-source-url="${listing.url ? esc(String(listing.url)) : ''}">
              ...
              <div class=\"mt-1\">
            <a href=\"/pools/${esc(id)}?refresh=1\" class=\"text-xs text-gray-600 underline hover:text-gray-900\">Refresh details${isWeak ? ' (fetched live)' : ''}</a>
              </div>
              ` : ''}
              ...
              <div class="mt-2 text-black">
                ${priceTiers.length ? (() => {
                  ...
                  return `
                  <style>
                    .tp-inline .marker{transform:translateX(-50%); top:18px}
