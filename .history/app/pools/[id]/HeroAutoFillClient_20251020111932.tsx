          ...
          if (parsed.length) {
            ...
            const alignTicksToSeparators = () => {
              ...
              seps.forEach((sep) => {
                ...
                const tick = document.createElement('span');
                tick.className = 'tick-sep absolute top-1/2 h-3 w-px -translate-y-1/2 bg-gray-400';
                tick.style.left = `${pct}%`;
                bar.appendChild(tick);
              });
            };
            ...
            window.addEventListener('resize', alignTicksToSeparators);
            ...
            const update = (q: number) => {
              ...
              bar.setAttribute('aria-valuenow', String(Math.min(qty, MAX)));
              highlightTier(qty);
            };
            ...
            (window as any).setPricingProgress = (q: number) => {
              update(q);
              // Re-align ticks in case header reflowed due to content updates
              alignTicksToSeparators();
            };
          }
        }
