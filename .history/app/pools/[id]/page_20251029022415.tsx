            ...
            <div class="pool-hero-right min-w-0">
              ...
              <div class="mt-2 text-black">
                ${priceTiers.length ? (() => {
                  ...
                  const raw: T[] = priceTiers.map((t: any) => {
                  ...
                  const lastMin = raw.length ? raw[raw.length - 1].min : 0;
                  const end = finiteMaxes.length ? Math.max(...finiteMaxes) : Math.max(1, Math.floor(lastMin * 1.5));
                  const tiersMid = raw.map(r => ({
                    ...r,
                    mid: (r.max === 'inf' ? (r.min + end) / 2 : (r.min + Number(r.max)) / 2)
                  }));
                  const pct = (q: number) => Math.max(0, Math.min(100, (q / (end || 1)) * 100));
                  const stops = [0, ...raw.map(r => (r.max === 'inf' ? lastMin : Number(r.max)))];
                  return `
                  <style>
