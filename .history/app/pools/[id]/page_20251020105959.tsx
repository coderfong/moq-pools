  ...
  const html = `
          <div class="pool-hero grid grid-cols-1 md:grid-cols-[360px_1fr] gap-6 items-start">
            ...
            <div class="pool-hero-right min-w-0">
              ...
              <div class="mt-2">
                ${priceTiers.length ? `
                <div id="tiers" class="space-y-2 text-black">
                  ${priceTiers.map(t => `
                    <div class=\"tier text-base flex items-baseline whitespace-nowrap\"><span class=\"font-semibold\">${esc(t.price)}</span><span class=\"ml-2\">${esc(normalizeTierRange(t.range))}</span></div>
                  `).join('')}
                </div>
