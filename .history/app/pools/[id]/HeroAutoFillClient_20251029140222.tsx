...
export default function HeroAutoFillClient() {
  useEffect(() => {
    try {
      ...
      try {
        const leftCol = document.querySelector('.pool-hero-left') as HTMLElement | null;
        if (leftCol && !leftCol.querySelector('[data-embed="sku-module"]')) {
          const anchor = leftCol.querySelector('.mt-3.flex.flex-col.items-center.gap-2') as HTMLElement | null;
          const dataSrc = (document.querySelector('.pool-hero-right') as HTMLElement | null)?.getAttribute('data-source-url') || '';
          const linkEl = Array.from(document.querySelectorAll('a')).find(a => /Open Original/i.test((a.textContent || '').trim())) as HTMLAnchorElement | undefined;
          const srcUrl = linkEl?.href || dataSrc || '';
          if (anchor && srcUrl) {
            (async () => {
              ...
              const parser = new DOMParser();
              const doc = parser.parseFromString(html, 'text/html');
              // Prefer the full module_sku or the sku layout container
              const skuModule = doc.querySelector('[data-module-name="module_sku"]') as HTMLElement | null;
              const skuLayout = (skuModule || doc).querySelector('[data-testid="sku-layout"]') as HTMLElement | null;
              const toEmbed = skuModule || skuLayout || doc.querySelector('[data-testid="sku-list"]')?.closest('[data-testid]') as HTMLElement | null;
              if (toEmbed) {
                const wrapper = document.createElement('div');
                wrapper.setAttribute('data-embed', 'sku-module');
                wrapper.className = 'mt-6';
                // Keep original structure; ensure it fits narrow column
                wrapper.innerHTML = toEmbed.outerHTML;
