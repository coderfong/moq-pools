          ...
          <div class="pool-hero grid grid-cols-1 md:grid-cols-[360px_1fr] gap-6 items-start">
            ...
            <div class="pool-hero-right min-w-0">
              ...
              <div class="mt-2 text-black">
                ${priceTiers.length ? `
                <header id="tiers" class="flex flex-wrap items-baseline gap-3 text-base md:text-lg">
                  ${priceTiers.map((t, idx) => `
                    <span class=\"tier inline-flex items-baseline gap-2 rounded-full px-4 py-2\"> 
                      <strong class=\"font-semibold text-lg md:text-xl\">${esc(t.price)}</strong>
                      <i class=\"not-italic text-sm md:text-base\">${esc(normalizeTierRange(t.range))}</i>
                    </span>
                    ${idx < priceTiers.length - 1 ? '<span class="text-gray-400">•</span>' : ''}
                  `).join('')}
                </header>
                <div class="mt-3">
                  <div id="pricing-progress"
                       class="relative h-3 w-full overflow-hidden rounded-full border border-gray-200 bg-gray-100"
                    ...
                    <span class="absolute left-[1%] top-1/2 h-3 w-px -translate-y-1/2 bg-gray-400" aria-hidden="true"></span>
                    <span class="absolute left-[10%] top-1/2 h-3 w-px -translate-y-1/2 bg-gray-400" aria-hidden="true"></span>
                    <span class="absolute left-[100%] top-1/2 h-3 w-px -translate-y-1/2 bg-gray-400" aria-hidden="true"></span>
                  </div>
                  <div class="mt-1 flex justify-between text-[12px] text-gray-500">
                    <span>24</span><span>240</span><span>2400</span>
                  </div>
                </div>
                ` : `
                <div class="text-base">
