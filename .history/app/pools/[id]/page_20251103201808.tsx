...
export default async function PoolDetailPage({ params, searchParams }: { params: { id: string }, searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  const rightCol = [variationsSec, packagingSec, protectionsSec, supplierSec].filter(Boolean).join('\n');
  ...
  const html = `
          ...
          <div class="pool-hero grid grid-cols-1 md:grid-cols-[420px_1fr] gap-10 items-start" data-source-url="${listing.url ? esc(String(listing.url)) : ''}">
            <div class="pool-hero-left">
              <div class="relative w-full aspect-square bg-gradient-to-br from-orange-100 via-amber-50 to-orange-50 rounded-3xl overflow-hidden border-4 border-orange-300/60 shadow-2xl ring-4 ring-orange-100/50">
                <img src="${imgSrc}" alt="${titleText}" class="w-full h-full object-cover" referrerpolicy="no-referrer" />
              </div>
              <div class="mt-6 flex flex-col items-center gap-4" data-variations-anchor>
                <a href="/checkout?listingId=${esc(id)}" class="inline-flex items-center justify-center rounded-2xl font-bold btn-mag text-2xl px-10 py-5 w-full text-white">
                  <span class="mr-2">🛒</span> Join Pool
                </a>
                <a href="/information/payment-protection" class="text-base text-orange-600 font-semibold underline hover:text-orange-700 flex items-center gap-2">
                  <span class="text-xl">🔒</span> Payment Protection Guaranteed
                </a>
              </div>
            </div>
            <div class="pool-hero-right min-w-0" data-source-url="${listing.url ? esc(String(listing.url)) : ''}">
              <div class="flex items-start justify-between gap-4 mb-4">
                <h1 class="text-4xl font-black bg-gradient-to-r from-gray-900 via-orange-600 to-amber-600 bg-clip-text text-transparent leading-tight">${titleText}</h1>
                <div id="pool-timer" data-deadline="${timerDeadlineISO}" class="text-base md:text-lg text-gray-900 flex-shrink-0">
                  <span class="inline-flex items-center gap-2 rounded-2xl border-3 border-orange-300 bg-gradient-to-r from-orange-100 to-amber-100 px-6 py-3 font-bold shadow-lg">
