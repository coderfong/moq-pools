...
export default function RootLayout({ children }: { children: ReactNode }) {
  return (
    <html lang="en" className={`${inter.variable} ${jakarta.variable} overflow-x-hidden`}>
      <body className="min-h-screen font-display bg-base text-ink overflow-x-hidden">
        <PlatformProviders>
          <AuthSessionProvider>
            <Navbar />
            <RevealRoot />
            {/* Full-bleed main with responsive gutters; remove vertical padding to keep hero flush */}
            <main className="w-full min-w-0 overflow-x-hidden px-0 md:px-0 xl:px-0">{children}</main>
            <Footer />
            <CompareModal />
          </AuthSessionProvider>
        </PlatformProviders>
