...
export default function RootLayout({ children }: { children: ReactNode }) {
  return (
    <html lang="en" className={`${inter.variable} ${jakarta.variable}`} style={{overflowX: 'hidden'}}>
      <body className="min-h-screen font-display bg-base text-ink" style={{overflowX: 'hidden', maxWidth: '100vw'}}>
        <PlatformProviders>
          <AuthSessionProvider>
            <Navbar />
            <RevealRoot />
            {/* Full-bleed main with responsive gutters; remove vertical padding to keep hero flush */}
            <main className="w-full px-0 md:px-0 xl:px-0" style={{maxWidth: '100vw'}}>{children}</main>
            <Footer />
            <CompareModal />
          </AuthSessionProvider>
        </PlatformProviders>
