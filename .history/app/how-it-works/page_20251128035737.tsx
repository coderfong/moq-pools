    ...
    <div className="w-full">
      ...
      <section className="px-6 md:px-10 xl:px-16 py-12 md:py-16">
        <div className="max-w-7xl mx-auto">
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} transition={{ staggerChildren: 0.06, duration: 0.4, ease: "easeOut" }} className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div variants={item} transition={{ duration: 0.4, ease: "easeOut" }}>
              ...
              <p className="text-xl text-muted mb-6">
                Traditional wholesale requires buying in bulk quantities you're not ready for. We solve this by pooling demand across multiple buyers, allowing everyone to access factory pricing without overcommitting.
              </p>
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  <CheckCircle2 className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" />
                  <div>
                    <h4 className="font-semibold text-xl">Better pricing</h4>
                    <p className="text-lg text-muted">Access volume discounts that were previously unavailable to small buyers.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle2 className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" />
                  <div>
                    <h4 className="font-semibold text-xl">No risk</h4>
                    <p className="text-lg text-muted">Pay only when the pool fills. Full refunds if it doesn't meet MOQ.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle2 className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" />
                  <div>
                    <h4 className="font-semibold text-2xl">We handle everything</h4>
                    <p className="text-xl text-muted">From negotiation to delivery, we manage the entire process for you.</p>
                  </div>
                </div>
              </div>
            </motion.div>
            <motion.div variants={item} className="relative">
              <div className="aspect-square rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 p-8 flex items-center justify-center overflow-hidden">
                <img 
                  src="/istockphoto-1347494044-612x612.jpg" 
      ...
      {/* CTA */}
      <section className="px-6 md:px-10 xl:px-16 py-12 md:py-16 bg-primary text-primary-foreground">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} transition={{ staggerChildren: 0.06, duration: 0.4, ease: "easeOut" }}>
            <motion.h2 variants={item} transition={{ duration: 0.4, ease: "easeOut" }} className="font-display text-2xl md:text-4xl font-extrabold tracking-tight mb-4">
              Ready to start saving?
            </motion.h2>
            <motion.p variants={item} transition={{ duration: 0.4, ease: "easeOut" }} className="text-primary-foreground/90 mb-8 max-w-2xl mx-auto">
              Join thousands of buyers already accessing factory pricing through MOQ pools.
            </motion.p>
            <motion.div variants={item} transition={{ duration: 0.4, ease: "easeOut" }} className="flex items-center justify-center gap-4">
              <Button size="lg" variant="secondary" className="px-8 h-12 rounded-2xl">
                Browse products <ArrowRight className="ml-2 w-4 h-4" />
              </Button>
              <Button size="lg" variant="outline" className="px-8 h-12 rounded-2xl border-primary-foreground/20 text-primary-foreground hover:bg-primary-foreground/10">
                Learn more
              </Button>
            </motion.div>
          </motion.div>
