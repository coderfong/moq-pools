    ...
    <div className="w-full">
      ...
      <section className="px-6 md:px-10 xl:px-16 py-16 md:py-24 bg-gradient-to-b from-white to-gray-50">
        <div className="w-full">
          ...
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} transition={{ staggerChildren: 0.12 }} className="space-y-12 max-w-7xl mx-auto">
            {steps.map((step, i) => {
              ...
              return (
                <motion.div key={i} variants={item} className="relative">
                  {/* Connector Line */}
                  {i < steps.length - 1 && (
                    <div className="hidden lg:block absolute left-[3.75rem] top-[7rem] w-1 h-[calc(100%+3rem)] bg-gradient-to-b from-orange-400 via-orange-300 to-orange-100 z-0 rounded-full"></div>
                  )}

                  {/* Step Card */}
                  <div className="relative bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-200 hover:border-orange-300 group">
                    {/* Desktop Layout */}
                    <div className="hidden md:grid md:grid-cols-[120px_1fr_280px] gap-8 p-8 items-center">
                      {/* Left - Number & Icon */}
                      <div className="flex flex-col items-center gap-4">
                        <div className="relative z-10 w-24 h-24 rounded-2xl bg-gradient-to-br from-orange-500 via-orange-600 to-amber-600 flex items-center justify-center shadow-2xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-500">
                          <span className="text-4xl font-bold text-white">{i + 1}</span>
                        </div>
                        <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-orange-100 to-amber-100 flex items-center justify-center group-hover:bg-gradient-to-br group-hover:from-orange-200 group-hover:to-amber-200 transition-all duration-300">
                      ...
                      <div className="flex-1 py-4">
                        <h3 className="text-3xl font-bold mb-4 text-gray-900 group-hover:text-orange-600 transition-colors">
                          {step.title}
                        </h3>
                      ...
                      {/* Right - Image */}
                      <div className="relative w-full h-64 rounded-2xl overflow-hidden shadow-xl border-2 border-gray-100 group-hover:border-orange-200 transition-all duration-500">
                        <div className="absolute inset-0 bg-gradient-to-br from-orange-500/10 to-amber-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10"></div>
                        <img 
                          src={step.image} 
                    ...
                    <div className="md:hidden p-6">
                      <div className="flex items-center gap-4 mb-6">
                        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center shadow-xl flex-shrink-0">
                      ...
                      <div className="w-full h-48 rounded-2xl overflow-hidden shadow-lg border-2 border-gray-100 mb-4">
                        <img 
                          src={step.image} 
                          alt={`${step.title} illustration`} 
      ...
      {/* WHO THIS IS FOR */}
      <section className="px-6 md:px-10 xl:px-16 py-16 md:py-20 bg-gradient-to-b from-white to-gray-50">
        <div className="max-w-6xl mx-auto">
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} transition={{ staggerChildren: 0.1 }} className="text-center mb-12">
            ...
            <motion.p variants={item} className="text-xl text-gray-600">
              Anyone who wants wholesale prices without bulk orders
            </motion.p>
          </motion.div>

          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} transition={{ staggerChildren: 0.12 }} className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <motion.div variants={item} className="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transition-all border border-gray-100">
              <div className="text-4xl mb-4">🏪</div>
              <h3 className="font-bold text-xl mb-2">Small Businesses</h3>
              <p className="text-gray-600">Get wholesale prices for inventory without tying up capital in bulk orders.</p>
            </motion.div>

            <motion.div variants={item} className="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transition-all border border-gray-100">
              <div className="text-4xl mb-4">🎨</div>
              <h3 className="font-bold text-xl mb-2">Entrepreneurs</h3>
              <p className="text-gray-600">Test product ideas and launch businesses with minimal upfront investment.</p>
            </motion.div>

            <motion.div variants={item} className="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transition-all border border-gray-100">
              <div className="text-4xl mb-4">🛒</div>
              <h3 className="font-bold text-xl mb-2">Resellers</h3>
              <p className="text-gray-600">Source products at factory prices to maximize profit margins on Amazon, eBay, Shopify.</p>
            </motion.div>

            <motion.div variants={item} className="bg-white rounded-2xl p-6 shadow-md hover:shadow-xl transition-all border border-gray-100">
              <div className="text-4xl mb-4">👤</div>
              <h3 className="font-bold text-xl mb-2">Individual Buyers</h3>
              <p className="text-gray-600">Buy quality products for personal use at prices usually only available to wholesalers.</p>
            </motion.div>
          </motion.div>
        </div>
      </section>
      ...
      {/* WHY IT WORKS */}
      <section className="px-6 md:px-10 xl:px-16 py-16 md:py-20 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} transition={{ staggerChildren: 0.1 }} className="text-center mb-12">
            <motion.h2 variants={item} className="font-display text-4xl md:text-5xl font-extrabold tracking-tight mb-4">
              Why MOQ Pools Work
            ...
            <motion.p variants={item} className="text-xl text-gray-600 max-w-3xl mx-auto">
              The economics that make this possible for everyone
            </motion.p>
          </motion.div>

          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} transition={{ staggerChildren: 0.1 }} className="grid md:grid-cols-3 gap-8">
            <motion.div variants={item} className="bg-white rounded-2xl p-8 shadow-lg border-2 border-orange-100">
              ...
              <h3 className="font-bold text-2xl mb-3">Volume = Lower Prices</h3>
              <p className="text-gray-600 text-lg leading-relaxed">
                Factories reduce per-unit costs significantly at higher volumes. By pooling 100 buyers, we unlock these MOQ tiers that would be impossible alone.
              </p>
            </motion.div>

            <motion.div variants={item} className="bg-white rounded-2xl p-8 shadow-lg border-2 border-emerald-100">
              ...
              <h3 className="font-bold text-2xl mb-3">Escrow Protects Everyone</h3>
              <p className="text-gray-600 text-lg leading-relaxed">
                Buyers know their money is safe. Suppliers get paid when MOQ is confirmed. Everyone wins with secure, trusted transactions.
              </p>
            </motion.div>

            <motion.div variants={item} className="bg-white rounded-2xl p-8 shadow-lg border-2 border-blue-100">
              ...
              <h3 className="font-bold text-2xl mb-3">We Handle the Complexity</h3>
              <p className="text-gray-600 text-lg leading-relaxed">
                Coordinating 100 buyers is hard. We manage payments, communications, negotiations, and logistics—so you don't have to.
              </p>
            </motion.div>
          </motion.div>
        </div>
