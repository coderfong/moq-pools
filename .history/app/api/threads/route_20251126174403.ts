...
export async function POST(req: Request) {
  try {
    ...
    // If poolId is provided, create/get 1-on-1 conversation between user and admin for this pool
    if (poolId) {
      ...
      if (!admin) {
        return j(500, { ok: false, error: 'no_admin_found' });
      }

      // Check if conversation already exists between this user and admin for this pool
      ...
      // Create 1-on-1 conversation between user and admin for this pool
      const convo = await db.conversation.create({
        data: {
          poolId,
          title: `${pool.product.title} - Support`,
