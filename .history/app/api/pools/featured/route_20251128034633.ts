  ...
  try {
    ...
    const poolsWithData = await Promise.all(
      pools.map(async (pool) => {
        ...
        const savedListing = pool.product.sourceUrl
          ? await prisma.savedListing.findFirst({
              where: {
                url: pool.product.sourceUrl,
              },
            })
          : null;
        ...
        return {
          ...
          title: pool.product.title || savedListing?.title || 'Untitled Product',
          image: imageUrl,
          category: savedListing?.categories?.[0] || 'General',
          targetQty: pool.targetQty,
          pledgedQty: pool.pledgedQty,
          progressPercentage,
          price: Number(pool.product.unitPrice || 0),
          originalPrice: savedListing?.priceMax || Number(pool.product.unitPrice || 0) * 2,
          daysLeft,
          status: pool.status,
        };
      })
