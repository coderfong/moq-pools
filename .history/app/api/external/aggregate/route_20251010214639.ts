...
export async function GET(req: Request) {
  ...
  if (platform === 'ALL') {
        ...
        const [a, c, m, i] = await Promise.all([
          ...
          // Enable cacheImages for IndiaMART so thumbnails are converted to local /cache paths for display on ALL tab
          withTimeout(fetchIndiaMart(q, per, { headless: !prefetch && headless, forceHeadless, debug, cacheImages: true }), prefetch ? 12000 : 20000, 'indiamart').catch((err) => { if(debug) console.warn('agg indiamart', String(err)); return []; }),
        ]);
        items = [...(a || []), ...(c || []), ...(m || []), ...(i || [])];
        ...
        items = await withTimeout(fetchIndiaMart(q, fastLimit, { headless: !prefetch && headless, forceHeadless, debug }), prefetch ? 10000 : 20000, 'indiamart').catch(() => []);
      }
