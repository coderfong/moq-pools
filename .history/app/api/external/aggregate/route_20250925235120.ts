...
export async function GET(req: Request) {
  ...
  const limitRaw = Number.isFinite(Number(searchParams.get('limit'))) ? Number(searchParams.get('limit')) : 30;
  ...
  try {
    ...
    let mergedFiltered: ExternalListing[] | undefined = sharedCache.get(cacheKey) as ExternalListing[] | undefined;
  ...
  if (platform === 'ALL') {
        ...
        const per = Math.min(240, Math.max(60, Math.ceil(target / 3)));
        const [a, e, c] = await Promise.all([
          withTimeout(fetchAlibaba(q, per, { headless }), 15000, 'alibaba').catch((err) => { console.warn('agg alibaba', String(err)); return []; }),
        ...
        items = [...(a || []), ...(e || []), ...(c || [])];
      } else if (platform === 'ALIBABA') items = await withTimeout(fetchAlibaba(q, Math.min(800, offset + limit), { headless }), 15000, 'alibaba').catch(() => []);
      else if (platform === 'ALIEXPRESS') items = await withTimeout(fetchAliExpress(q, Math.min(600, offset + limit)), 10000, 'aliexpress').catch(() => []);
      else if (platform === 'C1688') items = await withTimeout(fetchC1688(q, Math.min(600, offset + limit)), 10000, 'c1688').catch(() => []);
      ...
      sharedCache.set(cacheKey, mergedFiltered, 5 * 60 * 1000);
    }

    const arr = mergedFiltered || [];
    const total = arr.length;
    const page = arr.slice(offset, offset + limit);
    return NextResponse.json({ items: page, total }, { headers: { 'Cache-Control': 'public, s-maxage=120, stale-while-revalidate=60' } });
  } catch (e: any) {
