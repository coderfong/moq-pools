...
export async function GET(req: Request) {
  ...
  if (platform === 'ALL') {
        ...
        const [a, c, m, i] = await Promise.all([
          ...
          withTimeout(fetchIndiaMart(q, per, { headless: !prefetch && headless, forceHeadless, debug }), prefetch ? 12000 : 20000, 'indiamart').catch((err) => { if(debug) console.warn('agg indiamart', String(err)); return []; }),
        ]);
        items = [...(a || []), ...(c || []), ...(m || []), ...(i || [])];
