...
import { fetchTaobao } from '@/lib/providers/taobao';
...
export async function GET(req: Request) {
  ...
  if (platform === 'ALL') {
        ...
        const [a, e, c, t, m] = await Promise.all([
          ...
          withTimeout(fetchTaobao(q, per, { headless }), 20000, 'taobao').catch((err) => { console.warn('agg taobao', String(err)); return []; }),
          withTimeout(fetchMadeInChina(q, per, { headless }), 20000, 'madeinchina').catch((err) => { console.warn('agg madeinchina', String(err)); return []; }),
        ]);
        items = [...(a || []), ...(e || []), ...(c || []), ...(t || []), ...(m || [])];
      ...
      else if (platform === 'TAOBAO') items = await withTimeout(fetchTaobao(q, Math.min(800, offset + limit), { headless }), 20000, 'taobao').catch(() => []);
      else if (platform === 'MADE_IN_CHINA') items = await withTimeout(fetchMadeInChina(q, Math.min(800, offset + limit), { headless }), 20000, 'madeinchina').catch(() => []);
      ...
      const filtered = uniq.filter((raw) => {
        ...
        // For 1688 specifically, require a detectable MOQ
        if ((raw.platform || '').toString().toUpperCase() === 'C1688' && m == null) return false;
      ...
      try {
        ...
        console.warn('listing snapshot save failed', e);
      }
