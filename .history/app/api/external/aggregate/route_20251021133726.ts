...
export async function GET(req: Request) {
  ...
  if (platform === 'ALL') {
        ...
        const [a, c, m, i, g] = await Promise.all([
          ...
          withTimeout(fetchGlobalSources(q, per, { headless: !prefetch && headless }), prefetch ? 12000 : 20000, 'globalsources').catch((err) => { if(debug) console.warn('agg globalsources', String(err)); return []; }),
        ]);
        items = [...(a || []), ...(c || []), ...(m || []), ...(i || []), ...(g || [])];
  ...
  else if (platform === 'GLOBAL_SOURCES') items = await withTimeout(fetchGlobalSources(q, Math.min(prefetch ? 120 : 800, offset + limit), { headless: !prefetch && headless }), prefetch ? 12000 : 20000, 'globalsources').catch(() => []);
