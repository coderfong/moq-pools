...
export async function GET(req: Request) {
  ...
  if (platform === 'ALL') {
        ...
        const [a, c, m, i] = await Promise.all([
          ...
          withTimeout(fetchMadeInChina(q, per, { headless, upgradeImages: false }), 20000, 'madeinchina').catch((err) => { console.warn('agg mic', String(err)); return []; }),
          withTimeout(fetchIndiaMart(q, per, { headless, forceHeadless, debug }), 20000, 'indiamart').catch((err) => { console.warn('agg indiamart', String(err)); return []; }),
        ]);
        items = [...(a || []), ...(c || []), ...(m || []), ...(i || [])];
  ...
  else if (platform === 'MADE_IN_CHINA') items = await withTimeout(fetchMadeInChina(q, Math.min(800, offset + limit), { headless, upgradeImages: false }), 20000, 'madeinchina').catch(() => []);
  else if (platform === 'INDIAMART') items = await withTimeout(fetchIndiaMart(q, Math.min(800, offset + limit), { headless, forceHeadless, debug }), 20000, 'indiamart').catch(() => []);
