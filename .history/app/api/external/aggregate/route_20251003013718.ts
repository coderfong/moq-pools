...
export async function GET(req: Request) {
  ...
  if (platform === 'ALL') {
        ...
        const [a, c, m, i] = await Promise.all([
          ...
          withTimeout(fetchIndiaMart(q, per, { headless }), 20000, 'indiamart').catch((err) => { console.warn('agg indiamart', String(err)); return []; }),
        ]);
        items = [...(a || []), ...(c || []), ...(m || []), ...(i || [])];
  ...
  else if (platform === 'INDIAMART') items = await withTimeout(fetchIndiaMart(q, Math.min(800, offset + limit), { headless }), 20000, 'indiamart').catch(() => []);
