...
export async function GET(request: NextRequest) {
  try {
    ...
    const userGrowthResult = await prisma.$queryRawUnsafe(userGrowthQuery, startDate);
    const userGrowthData = (userGrowthResult as any[]).map((row, index) => ({
      label: `Week ${index + 1}`,
    ...
    const revenueResult = await prisma.$queryRawUnsafe(revenueQuery, startDate);
    const revenueData = (revenueResult as any[]).map((row, index) => {
    ...
    const poolStatusResult = await prisma.$queryRawUnsafe(poolStatusQuery);
    const poolStatusData = (poolStatusResult as any[]).map(row => ({
      label: row.status || 'Unknown',
      value: Number(row.count),
      color: getStatusColor(row.status)
    }));

    // Top Categories (based on pool products)
    ...
    const topCategoriesResult = await prisma.$queryRawUnsafe(topCategoriesQuery);
    const topCategoriesData = (topCategoriesResult as any[]).map((row, index) => ({
    ...
    const avgOrderValue = Number(stats[0]?.avg_order_value || 0);
    const totalOrders = Number(stats[0]?.total_orders || 0);
    const totalPools = Number(stats[0]?.total_pools || 0);
    const successfulPools = Number(stats[0]?.successful_pools || 0);
    const avgDaysToDeadline = Number(stats[0]?.avg_days_to_deadline || 0);
