...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create conversation for PayPal order
    try {
          ...
          if (adminUser) {
            ...
            await (prisma as any).message.create({
              data: {
                ...
                sender: null,
                text: `Thank you for joining the pool for **${pool.product?.title || 'this product'}**! 🎉\n\nYour payment of ${amount.toFixed(2)} ${currency} is now held securely in escrow via PayPal. We'll keep you updated as the pool progresses toward its MOQ.\n\nFeel free to ask any questions here!`,
              }
            });
          }
        }
      ...
      console.error('Failed to create conversation (PayPal):', convError);
    }
