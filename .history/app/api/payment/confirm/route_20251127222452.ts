    ...
    if (effectivePoolId && prisma) {
      try {
        ...
        if (!poolItem) {
          ...
          // Find the SavedListing ID for this pool (for correct navigation link)
          ...
          if (pool.product.sourceUrl) {
            try {
              ...
              if (savedListing) {
                ...
                console.log('[PAYMENT CONFIRM] Found SavedListing:', savedListingId);
              }
            } catch (e) {
              console.log('[PAYMENT CONFIRM] Could not find SavedListing for sourceUrl');
            }
          }
