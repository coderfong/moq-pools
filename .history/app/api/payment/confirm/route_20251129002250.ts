    ...
    if (effectivePoolId && prisma) {
      try {
        ...
        if (!poolItem) {
          ...
          // 2. Create automated welcome message in pool conversation
          try {
            console.log('[PAYMENT CONFIRM] Creating welcome message in pool conversation...');
            
            // Find or create pool conversation
            ...
            if (!conversation) {
              console.log('[PAYMENT CONFIRM] Creating new conversation for pool:', pool.id);
              conversation = await (prisma as any).conversation.create({
                data: {
                  poolId: pool.id,
                  title: pool.product.title,
                  preview: `Pool discussion for ${pool.product.title}`,
                }
              });
            }
            ...
            if (!existingParticipant) {
              ...
              console.log('[PAYMENT CONFIRM] Added user as conversation participant');
            }
            ...
            if (adminUser) {
              const existingAdminParticipant = await (prisma as any).conversationParticipant.findUnique({
                where: {
                  conversationId_userId: {
                    conversationId: conversation.id,
                    userId: adminUser.id
              ...
              if (!existingAdminParticipant) {
                ...
                console.log('[PAYMENT CONFIRM] Added admin as conversation participant');
              }
            }

            // Create automated welcome message
            const welcomeMessage = await (prisma as any).message.create({
              data: {
                conversationId: conversation.id,
                sender: 'System',
                senderUserId: null, // System message has no user
                text: `🎉 Welcome to the pool, ${userName}! Your order of ${quantity} unit${quantity > 1 ? 's' : ''} has been confirmed and your payment is held securely in escrow. Pool progress: ${updatedPool.pledgedQty}/${pool.targetQty} units.`
              }
            });

            console.log('[PAYMENT CONFIRM] ✅ Welcome message created:', welcomeMessage?.id);
          } catch (messageError) {
            console.error('[PAYMENT CONFIRM] ❌ Failed to create welcome message:', messageError);
          }
