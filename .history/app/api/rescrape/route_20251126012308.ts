...
const MAX_CONCURRENT_SCRAPES = 3; // Reduced for stability

// Cache to prevent redundant scrapes (10 minute TTL for better caching)
const scrapeCache = new Map<string, { timestamp: number; data: any }>();
const CACHE_TTL_MS = 10 * 60 * 1000; // 10 minutes

export async function POST(req: NextRequest) {
  try {
    // Rate limiting: max 500 scrape requests per minute (increased for bulk operations with retries)
    const limiter = getRateLimiter('rescrape-api', { capacity: 500, refillRate: 500/60 });
