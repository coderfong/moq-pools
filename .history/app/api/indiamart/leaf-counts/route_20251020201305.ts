...
export async function GET(req: NextRequest) {
  ...
  const c = await db.savedListing.count({ where: { platform: 'INDIAMART' as any, categories: { has: key } } });
      counts[key] = c;
    }));
  }
  ...
  const totalListings = await db.savedListing.count({ where: { platform: 'INDIAMART' as any } });
