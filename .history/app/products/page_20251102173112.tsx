...
export default async function Products({ searchParams }: { searchParams: { platform?: string, lplatform?: string, q?: string, lk?: string, minPrice?: string, maxPrice?: string, minMoq?: string, maxMoq?: string, moqLeft?: string, page?: string, per?: string, cols?: string } }) {
  ...
  let savedTitleMap = new Map<string, { id: string; title: string; detailTitle?: string; priceText?: string; priceTiers?: Array<{ price: string; range: string }> }>();
  try {
    ...
    if ((urls.length || keys.length) && prisma) {
      ...
      const map = new Map<string, { id: string; title: string; detailTitle?: string; priceText?: string; priceTiers?: Array<{ price: string; range: string }> }>();
      for (const r of rows as Array<{ id: string; url: string; title: string; detailJson?: any }>) {
        const dj = (r.detailJson && typeof r.detailJson === 'object') ? r.detailJson : undefined;
        const detailTitle = dj ? String(dj?.title || '').trim() : '';
        const priceText = dj && typeof dj?.priceText === 'string' ? String(dj.priceText).trim() : '';
        const priceTiers = Array.isArray(dj?.priceTiers) ? (dj.priceTiers as Array<{ price: string; range: string }>).filter(t => t && typeof t.price === 'string') : undefined;
        const val = { id: r.id, title: r.title, detailTitle: detailTitle || undefined, priceText: priceText || undefined, priceTiers: (priceTiers && priceTiers.length ? priceTiers : undefined) };
  ...
  return (
  <div className="container mx-auto px-6 md:px-10 xl:px-16 py-8 space-y-6">
      {/* Side panel host rendered once at page root */}
      <DetailPanelHost />
      {/* Persist and restore grid density (cols) preference */}
      <GridDensityPreference />
      <RevealSection className="sticky top-[72px] z-20 bg-base/80 backdrop-blur border-b border-hairline py-3 flex items-center justify-between gap-3">
        <h1 className="text-3xl font-semibold">MOQ Pools</h1>
      </RevealSection>
      {platform === 'INDIAMART' && (
      ...
      <div className="mt-4 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 ms-auto">
            {(() => {
              ...
              return (
                <>
                  <div className="text-xs text-gray-600 dark:text-gray-400 hidden sm:block">Total: {formatNumberEN(total)}</div>
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-gray-500 dark:text-gray-400">Per row:</span>
                    <div className="inline-flex rounded-md overflow-hidden ring-1 ring-gray-300 dark:ring-gray-700">
                      {colSizes.map((cc, idx) => {
                        ...
                        return (
                          <a key={cc} href={buildColsUrl(cc)} className={`${base} ${tone} ${sep}`}>{cc}</a>
                        );
                      })}
                    </div>
                  </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">
            {platform === 'ALL' ? 'Listings across supported platforms' : `Listings on ${platformLabel(platform)}`}
          </h2>
        ...
        {STRICT_OFFLINE === '1' ? (
          ...
          <>
              ...
              {pageItems.map((it, idx) => {
                ...
                return (
                  <RevealItem key={keyFromListing(it, idx)}>
                    <div className="rounded-2xl p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-card hover:shadow-lg transition hover:-translate-y-0.5 transform will-change-transform h-full flex flex-col">
                      {/* Main clickable block (single anchor) */}
                      <a href={it.url || '#'} target="_blank" rel="noreferrer" className="block flex-1 flex flex-col">
                        {(() => {
                          ...
                          return (
                            <div className="relative aspect-square bg-gray-100 rounded-lg mb-2 overflow-hidden group">
                              <Image
                                src={src}
                                ...
                                try {
                                  ...
                                  return (
                                    <div className="absolute top-2 left-2">
                                      <PlatformBadge code={code} className="text-base" />
                                    </div>
                                  );
                                } catch { return null; }
                              })() : null}
                        ...
                        <div className="text-[11px] text-gray-600 dark:text-gray-400 truncate min-h-[14px]" title={it.storeName || ''}>{it.storeName || ''}</div>
                        <div className="font-medium text-sm mt-0.5 text-gray-900 dark:text-gray-100 min-h-[2.5rem]" title={title || undefined}>{title || 'See listing'}</div>
                        {(() => {
                          const primary = it.price && it.price.toString().trim().length ? it.price : '';
                          const savedPriceText = savedForUrl?.priceText || '';
                          const savedTierPrice = (savedForUrl?.priceTiers && savedForUrl.priceTiers.length) ? (savedForUrl.priceTiers[0]?.price || '') : '';
                          const tierPrice = tiers.length ? (tiers[0]?.priceText || '') : '';
                          let display = savedPriceText || savedTierPrice || primary || tierPrice;
                          if (!display) {
                          ...
                          const show = (() => {
                            ...
                            // If the display already contains a unit or slash, don't double-append "/ unit"
                            const hasUnitToken = /(\/\s*|\b(unit|piece|pc|pcs|set|sets|pair|pairs|bag|bags|lot|lots)\b)/i.test(s);
                            return hasUnitToken ? display : (display ? `${display} / unit` : 'See listing');
                          })();
                          return (
                            <div className="mt-1 flex items-baseline gap-2 min-h-[28px]">
                              <div className="text-lg font-semibold text-orange-600">{show}</div>
                              {soldText ? (
                                <div className="text-sm text-gray-700">{soldText}</div>
                              ) : null}
                            </div>
                          );
                        })()}
                          ...
                          try {
                            ...
                            return (
                              <div className="mt-2 space-y-2">
                                <div className="flex justify-between text-sm">
                                  <span className="text-muted-foreground">Progress to MOQ</span>
                                  <span className="font-semibold text-foreground">{formatNumberEN(current)}/{formatNumberEN(target)} orders</span>
                                </div>
                                <Progress value={percent} label="Progress to MOQ" className="bg-secondary" />
                              </div>
                            );
                          } catch {
                            ...
                            return (
                              <div className="mt-2 space-y-2">
                                <div className="flex justify-between text-sm">
                                  <span className="text-muted-foreground">Progress to MOQ</span>
                                  <span className="font-semibold text-foreground">0/100 orders</span>
                                </div>
                                <Progress value={0} label="Progress to MOQ" className="bg-secondary" />
                              </div>
                            );
                          }
                        })()}
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <a href={savedId ? `/pools/${savedId}` : (it.url || '#')} target={savedId ? undefined : "_blank"} rel={savedId ? undefined : "noreferrer"} className="inline-flex items-center justify-center rounded-full font-medium btn-mag btn-primary-gradient shadow-card hover:shadow-lg text-xs px-3 py-1.5">Join Pool</a>
                        {it.url && ((it.url as string).includes('alibaba.com') || (it.url as string).includes('made-in-china.com') || (it.url as string).includes('indiamart.com')) ? (
                          <DetailLink url={String(it.url)} title={title} className="text-xs px-3 py-1.5 rounded-full border border-gray-200 hover:bg-gray-50" />
                        ) : (
                          <a href={it.url || '#'} target="_blank" rel="noreferrer" className="text-xs px-3 py-1.5 rounded-full border border-gray-200 hover:bg-gray-50">Details</a>
                        )}
                      </div>
                    </div>
                  </RevealItem>
            ...
            {(() => {
              ...
              return (
                <RevealSection className="flex items-center justify-center gap-2 mt-6">
                  <a href={buildUrl(Math.max(1, page - 1))} className={`px-3 py-1 border rounded ${page <= 1 ? 'pointer-events-none opacity-40' : ''}`}>Prev</a>
                  {Array.from({ length: pEnd - pStart + 1 }, (_, i) => pStart + i).map((p) => (
                    <a key={p} href={buildUrl(p)} className={`px-3 py-1 border rounded ${p === page ? 'bg-black text-white' : 'bg-white'}`}>{p}</a>
                  ))}
                  <a href={buildUrl(Math.min(totalPages, page + 1))} className={`px-3 py-1 border rounded ${page >= totalPages ? 'pointer-events-none opacity-40' : ''}`}>Next</a>
                </RevealSection>
              );
            })()}
          </>
  ...
  <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Pools on {platform === 'ALL' ? 'All Platforms' : platformLabel(platform)}</h2>
      ...
      {platform !== 'ALL' && (
        <div className="mt-6 border rounded-xl p-4 bg-gray-50 dark:bg-gray-900">
