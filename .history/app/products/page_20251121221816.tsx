...
export default async function Products({ searchParams }: { searchParams: { platform?: string, lplatform?: string, q?: string, lk?: string, minPrice?: string, maxPrice?: string, minMoq?: string, maxMoq?: string, moqLeft?: string, page?: string, per?: string, cols?: string, sort?: string } }) {
  ...
  try {
    ...
    // REMOVED: All scraping code for image enhancement has been disabled
    // Images should be pre-cached during product ingestion via background scripts
    /* OLD CODE REMOVED - was scraping on every page load:
    ...
    const mapped = await Promise.all(pageItems.map(async (it) => {
      ... Alibaba/IndiaMART image scraping ...
    }));
    */
    // Placeholder for old code structure - now simplified
    const mapped = pageItems; // No longer mapping/scraping
    if (false) { // Dead code block - kept for reference only
      let img = '';
                    ...
                    <div className="group relative h-full">
                      ...
                      <div className="p-4 pt-0 flex flex-wrap items-center gap-2">
                        ...
                        {it.url && ((it.url as string).includes('alibaba.com') || (it.url as string).includes('made-in-china.com') || (it.url as string).includes('indiamart.com')) ? (
                          <DetailLink url={String(it.url)} title={title} className="flex-1 min-w-[90px] px-4 py-3 rounded-xl border-2 border-orange-200/60 hover:border-orange-500 hover:bg-gradient-to-br hover:from-orange-50 hover:to-amber-50 transition-all duration-300 text-sm font-bold text-gray-700 hover:text-orange-600 hover:shadow-lg text-center" />
                        ) : (
                          <a href={it.url || '#'} target="_blank" rel="noreferrer" className="flex-1 min-w-[90px] px-4 py-3 rounded-xl border-2 border-orange-200/60 hover:border-orange-500 hover:bg-gradient-to-br hover:from-orange-50 hover:to-amber-50 transition-all duration-300 text-sm font-bold text-gray-700 hover:text-orange-600 hover:shadow-lg text-center">
                            Details
                          </a>
                        )}
                        <ProductCardButtons
