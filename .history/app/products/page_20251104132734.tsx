...
export default async function Products({ searchParams }: { searchParams: { platform?: string, lplatform?: string, q?: string, lk?: string, minPrice?: string, maxPrice?: string, minMoq?: string, maxMoq?: string, moqLeft?: string, page?: string, per?: string, cols?: string } }) {
  ...
  // Server-side upgrade for Made-in-China listings on ALL tab:
  // Fetch detail page to get proper title and price when listing data is incomplete
  if (!FAST && isAllTab && Array.isArray(extItems) && extItems.length) {
    const toFixMic = extItems.filter((it) => {
      try {
        const url = String(it.url || '');
        const title = String(it.title || '').trim();
        const isMic = url.includes('made-in-china.com');
        // Check if title looks bad (pagination text, too short, no letters)
        const badTitle = !title || title.length < 4 || /^\d+[\s\/\-]*\d*$/.test(title);
        return isMic && badTitle;
      } catch { return false; }
    }).slice(0, 40); // Limit to 40 to avoid timeout
    
    if (toFixMic.length) {
      console.log(`[PRODUCTS] Fixing ${toFixMic.length} Made-in-China listings with bad titles...`);
      const concurrency = 3;
      let idx = 0;
      async function worker() {
        while (idx < toFixMic.length) {
          const i = idx++;
          const it = toFixMic[i];
          try {
            const { fetchProductDetailCached } = await import('@/lib/providers/detail');
            const detail = await fetchProductDetailCached({ url: String(it.url || '') });
            if (detail?.title) {
              it.title = detail.title;
              console.log(`[PRODUCTS] Fixed MIC title: "${detail.title}"`);
            }
            if (detail?.priceText && !it.price) {
              it.price = detail.priceText;
            }
            if (detail?.heroImage && (!it.image || it.image.startsWith('/seed/'))) {
              it.image = detail.heroImage;
            }
          } catch (err) {
            console.error(`[PRODUCTS] Failed to fix MIC listing:`, err);
          }
        }
      }
      await Promise.all(Array.from({ length: Math.min(concurrency, toFixMic.length) }, () => worker()));
    }
  }
  ...
  const uniqExt = (() => {
    ...
    for (const it of src) {
      ...
      // Filter out listings with obviously bad/incomplete titles
      const title = String(it.title || '').trim();
      const looksJunky = (s: string) => {
        if (!s || s.length < 3) return true;
        // Filter pagination text like "1/ 3", "2/5", etc.
        if (/^\d+[\s\/\-]*\d*$/.test(s) && !/[a-z]/i.test(s)) return true;
        // Filter very short titles without letters
        if (s.length < 4 && !/[a-z]/i.test(s)) return true;
        return false;
      };
      if (looksJunky(title)) {
        console.log('[PRODUCTS] Filtering bad listing:', { title, url: it.url });
