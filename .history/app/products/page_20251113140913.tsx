...
export default async function Products({ searchParams }: { searchParams: { platform?: string, lplatform?: string, q?: string, lk?: string, minPrice?: string, maxPrice?: string, minMoq?: string, maxMoq?: string, moqLeft?: string, page?: string, per?: string, cols?: string } }) {
  ...
  (hasDb && prisma ? withTimeout(prisma.product.groupBy({ 
    ...
    (async (): Promise<ExternalListing[]> => {
      try {
        console.log('[PRODUCTS DEBUG] Starting external listings fetch, platform:', platform, 'searchTerm:', searchTerm);
        // 0) Local SavedListings (do not early-return unless we have a reasonably sized set)
        // Limit reduced to avoid disk space issues with large result sets
        const savedLimit = searchTerm ? Math.min(500, perPage * 2) : Math.min(5000, perPage * 10);
        ...
        console.log('[PRODUCTS DEBUG] SavedListings fetched:', saved.length, 'searchTerm:', searchTerm, 'platform:', platform, 'limit:', savedLimit);
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50/30 via-white to-amber-50/20 relative">
      ...
      <div className="space-y-6">
        ...
        <div className="relative group">
          ...
          <div className="relative bg-white/95 backdrop-blur-sm rounded-2xl p-5 border-2 border-orange-200/60 shadow-xl shadow-orange-500/5">
        ...
        {STRICT_OFFLINE === '1' ? (
          ...
          <>
            <div className="text-xs text-gray-500 dark:text-gray-400 mb-4">Showing {start + 1}-{end} of {total} · Page {page} / {totalPages}</div>
            {total === 0 ? (
            ...
            <RevealSection className={`${gridColsClass} gap-6 auto-rows-fr items-stretch`}>
                  ...
                  <RevealItem key={keyFromListing(it, idx)}>
                    ...
                    <div className="group relative h-full">
                      ...
                      <LinkComponent {...linkProps} className="block flex-1 flex flex-col p-4">
                        ...
                        <div className="font-bold text-sm text-gray-900 line-clamp-2 mb-3 min-h-[2.5rem] leading-snug">{title || 'See listing'}</div>
                        {(() => {
