...
export default async function Products({ searchParams }: { searchParams: { platform?: string, lplatform?: string, q?: string, lk?: string, minPrice?: string, maxPrice?: string, minMoq?: string, maxMoq?: string, moqLeft?: string, page?: string, per?: string, cols?: string, sort?: string } }) {
  ...
  const where: any = {
    isActive: true,
    moqQty: { gt: 0 },
    // NOTE: Removed sourceUrl requirement to show all database products
    // NOT: [{ sourceUrl: null }],
  };
  ...
  if (platform !== 'ALL') {
    ...
    // When ALL is selected, only include allowed platforms
    where.sourcePlatform = { in: ALL_ALLOWED_PLATFORMS as any };
  }
  ...
  (hasDb && prisma ? withTimeout(prisma.product.groupBy({ 
    ...
    (async (): Promise<ExternalListing[]> => {
      try {
        ...
        // Only query SavedListing table - no external API calls
        const savedLimit = searchTerm ? Math.min(2000, perPage * 10) : Math.min(10000, perPage * 20);
  ...
  try {
    ...
    if (FAST) {
      ...
      pageItems = await Promise.all(pageItems.map(async (it: any) => {
        ...
        // Enrich with cached price tiers from detailJson (fast - no external fetches)
        ...
        try {
          ...
          if (detailJson && Array.isArray(detailJson.priceTiers) && detailJson.priceTiers.length) {
            console.log('[TIER DEBUG]', it.title?.substring(0, 40), 'Raw tiers:', JSON.stringify(detailJson.priceTiers));
            tiers = detailJson.priceTiers.map((tier: any) => {
              const range = String(tier.range || '').trim();
              const price = String(tier.price || '').trim();
              console.log('[TIER DEBUG] Parsing tier:', JSON.stringify({range, price}));
              let min = 0;
              ...
              console.log('[TIER DEBUG] Parsed:', JSON.stringify({min, max, priceText: price}));
              return { min, max, priceText: price, label: range };
            }).filter((t: any) => t.min > 0);
            console.log('[TIER DEBUG] Final tiers after filter:', tiers?.length || 0);
          }
        } catch {}
    ...
    const mapped = await Promise.all(pageItems.map(async (it) => {
      ...
      let tiers: { min: number; max?: number; priceText: string; label?: string }[] | undefined;
      try {
        // First, check if we have cached price tiers in detailJson
        const detailJson = (it as any)?.detailJson;
        console.log('[ENRICHMENT] Processing:', it.title?.substring(0, 40), 'detailJson:', !!detailJson, 'priceTiers:', detailJson?.priceTiers?.length);
        if (detailJson && Array.isArray(detailJson.priceTiers) && detailJson.priceTiers.length) {
          // Convert Alibaba format to our internal format
          tiers = detailJson.priceTiers.map((tier: any) => {
            const range = String(tier.range || '').trim();
            const price = String(tier.price || '').trim();
            // Parse range like "1000 - 9999 pieces" or ">= 10000 pieces"
            let min = 0;
            let max: number | undefined;
            const gteMatch = range.match(/>=?\s*(\d+)/);
            const rangeMatch = range.match(/(\d+)\s*-\s*(\d+)/);
          ...
          console.log('[ENRICHMENT] Converted tiers:', tiers.length, 'tiers ->', JSON.stringify(tiers));
        }
        
        // If no cached tiers, try fetching from API
        if (!tiers && it.url) {
          const u = new URL(it.url);
          const host = u.hostname.toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50/30 via-white to-amber-50/20 relative">
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 xl:px-16 py-6 space-y-6">
        ...
        <RevealSection className="sticky top-[72px] z-20 bg-white/80 backdrop-blur-xl border-b-2 border-orange-200/60 -mx-4 md:-mx-6 lg:-mx-8 xl:-mx-16 px-4 md:px-6 lg:px-8 xl:px-16 py-6 shadow-lg shadow-orange-500/5">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <h1 className="text-3xl lg:text-5xl font-extrabold bg-gradient-to-r from-orange-600 via-orange-500 to-amber-500 bg-clip-text text-transparent leading-tight">
      ...
      <div className="space-y-6 relative z-10">
        ...
        {STRICT_OFFLINE === '1' ? (
          ...
          <>
            ...
            <RevealSection className={`${gridColsClass} gap-6 auto-rows-fr items-stretch relative z-0`}>
                ...
                return (
                  <RevealItem key={keyFromListing(it, idx)}>
                    ...
                    <div className="group relative h-full">
                      ...
                      <div className="absolute -inset-0.5 bg-gradient-to-r from-orange-400 via-amber-400 to-orange-400 rounded-2xl blur opacity-0 group-hover:opacity-30 transition duration-500"></div>
                      
                      <div className="relative rounded-2xl overflow-hidden bg-white/95 backdrop-blur-sm border-2 border-orange-200/40 shadow-lg hover:shadow-2xl hover:shadow-orange-500/10 transition-all duration-300 hover:-translate-y-2 h-full flex flex-col">
                      {/* Main clickable block */}
                      <LinkComponent {...linkProps} className="block flex-1 flex flex-col p-4">
                        {(() => {
                          ...
                          return (
                            <div className="relative aspect-square bg-gradient-to-br from-orange-50 via-amber-50 to-orange-50 rounded-xl mb-3 overflow-hidden border-2 border-orange-100/50">
                        ...
                        <div className="text-xs text-gray-500 truncate mb-2 font-medium" title={it.storeName || ''}>{it.storeName || ''}</div>
                        <div className="font-bold text-sm text-gray-900 line-clamp-3 mb-3 leading-snug" title={title}>{title || 'See listing'}</div>
                        {(() => {
                          const primary = it.price && it.price.toString().trim().length ? it.price : '';
                          let display = primary;
                          console.log('[PRICE DISPLAY]', it.title?.substring(0, 40), 'tiers:', tiers?.length, 'primary:', primary);
                          ...
                          if (tiers && tiers.length >= 1) {
                            ...
                            for (const tier of tiers) {
                              ...
                              // Extract numeric price from text like "US$318" or "$313" or "US$0.20"
                            ...
                            if (prices.length > 0) {
                              ...
                              if (prices.length === 1) {
                                ...
                                console.log('[PRICE DISPLAY] Single tier:', display);
                              } else {
                                // Multiple price tiers - show range from lowest to highest
                                const minPrice = Math.min(...prices);
                                const maxPrice = Math.max(...prices);
                                display = `${currency}${minPrice}-${currency}${maxPrice}`;
                                console.log('[PRICE DISPLAY] Range:', display, 'from', prices);
                              }
                            }
                          }
                          
                          console.log('[PRICE DISPLAY] Final display:', display);
                          ...
                          try {
                            ...
                            return (
                              <div className="mb-4 space-y-2">
                                ...
                                <div className="relative h-2.5 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full overflow-hidden border border-gray-300/50">
                                  <div 
                            ...
                            return (
                              <div className="mb-4 space-y-2">
                                ...
                                <div className="relative h-2.5 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full overflow-hidden border border-gray-300/50">
                                  <div className="h-full bg-gradient-to-r from-emerald-500 via-emerald-400 to-emerald-500 transition-all duration-500 rounded-full shadow-lg shadow-emerald-500/30" style={{ width: '0%' }}></div>
                                </div>
                              </div>
                            );
                          }
                        ...
                        {!tiers.length ? (
                          <div className="mb-2">
                            <Countdown durationMs={5 * 24 * 60 * 60 * 1000} storageKey="products-countdown" />
                          </div>
                        ) : (
                      ...
                      <div className="p-4 pt-0 flex items-center gap-2">
                        ...
                        {it.url && ((it.url as string).includes('alibaba.com') || (it.url as string).includes('made-in-china.com') || (it.url as string).includes('indiamart.com')) ? (
                          <DetailLink url={String(it.url)} title={title} className="px-4 py-3 rounded-xl border-2 border-orange-200/60 hover:border-orange-500 hover:bg-gradient-to-br hover:from-orange-50 hover:to-amber-50 transition-all duration-300 text-sm font-bold text-gray-700 hover:text-orange-600 hover:shadow-lg" />
                        ) : (
                          <a href={it.url || '#'} target="_blank" rel="noreferrer" className="px-4 py-3 rounded-xl border-2 border-orange-200/60 hover:border-orange-500 hover:bg-gradient-to-br hover:from-orange-50 hover:to-amber-50 transition-all duration-300 text-sm font-bold text-gray-700 hover:text-orange-600 hover:shadow-lg">
                            Details
                          </a>
                        )}
                      </div>
