...
import SmartListingImage from '@/components/SmartListingImage';
...
export default async function Products({ searchParams }: { searchParams: { platform?: string, lplatform?: string, q?: string, lk?: string, minPrice?: string, maxPrice?: string, minMoq?: string, maxMoq?: string, moqLeft?: string, page?: string, per?: string, cols?: string } }) {
  ...
  // Default to enabled; you can disable with ?fast=0 or NEXT_PUBLIC_FAST_LISTINGS=0
          ...
          <>
            ...
            <RevealSection className={`${gridColsClass} gap-6 mt-3 auto-rows-fr items-stretch`}>
              {pageItems.map((it, idx) => {
                const base = normalizeListing(it as any);
                const rawUrl = String(base.url || '');
                ...
                const titleBase = savedDetailTitleRaw || savedTitleRaw || detailTitleRaw || computeDisplayTitle(it) || base.title;
                const title = cleanTitleString(titleBase) || computeDisplayTitle(it) || base.title;
                ...
                return (
                  <RevealItem key={keyFromListing(it, idx)}>
                    <div className="rounded-2xl p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-card hover:shadow-lg transition hover:-translate-y-0.5 transform will-change-transform h-full flex flex-col">
                      ...
                      <a href={base.url || '#'} target="_blank" rel="noreferrer" className="block flex-1 flex flex-col">
                        {(() => {
                          ...
                          const raw = String((it as any).image || base.image || '');
                          // On ALL tab, avoid sleeves. If only sleeve remains, render nothing to avoid misleading image.
                          const fallback = isAllTab ? '' : (resolveFallbackImage(raw, (it as any).title || title, (it as any).description || '') || '');
                          ...
                          return (
                            <div className="relative aspect-square bg-gray-100 rounded-lg mb-2 overflow-hidden group">
                              <SmartListingImage
                                listingId={String(base.id || (it as any).id || base.url || keyFromListing(it, idx))}
                                listingUrl={String(base.url || '')}
                                initialSrc={src}
                                alt={title || base.title || 'Listing image'}
                              ...
                              {base.url ? (() => {
                                try {
                                  const u = new URL(String(base.url));
                                  const host = u.hostname.toLowerCase();
                        ...
                        <div className="text-[11px] text-gray-600 dark:text-gray-400 truncate min-h-[14px]" title={(it as any).storeName || ''}>{(it as any).storeName || ''}</div>
                        <div className="font-medium text-sm line-clamp-2 mt-0.5 text-gray-900 dark:text-gray-100 min-h-[2.5rem]">{title || base.title || 'See listing'}</div>
                        {(() => {
                  ...
                  <div className="aspect-square bg-gray-100 rounded-lg mb-3 overflow-hidden">
                    {src ? (
                      <SmartListingImage
                        ...
                        listingUrl={String((p as any).sourceUrl || '')}
                        initialSrc={String(src)}
                        alt={String(p.title || 'Pool image')}
