    ...
    if (toFixMic.length) {
      ...
      async function worker() {
        while (idx < toFixMic.length) {
          ...
          const it = toFixMic[i];
          try {
            const { fetchMadeInChinaDetail } = await import('@/lib/providers/madeinchina');
            const detail = await fetchMadeInChinaDetail(String(it.url || ''));
            if (detail?.title) {
              it.title = detail.title;
              console.log(`[PRODUCTS] Fixed MIC title: "${detail.title}"`);
            }
            if (detail?.price && !it.price) {
              it.price = detail.price;
            }
            if (detail?.image && (!it.image || it.image.startsWith('/seed/'))) {
              it.image = detail.image;
            }
          } catch (err) {
