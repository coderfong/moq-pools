...
4. **Restart your dev server to load new env:**
   ```powershell
   pnpm run dev
   ```

5. **Visit the authorization URL using ngrok:**
   ```
   https://abc123.ngrok-free.app/api/alibaba/authorize
   ```

6. **You should be redirected to Alibaba login page**

7. **After login/approval, you'll be redirected back to:**
   ```
   https://abc123.ngrok-free.app/api/alibaba/callback?code=xxx&state=yyy
   ```

8. **Then redirected to success page:**
   ```
   https://abc123.ngrok-free.app/admin/settings?alibaba_auth=success
   ```

9. **Check your console for:**
   ```
   [Alibaba OAuth] ✓ Successfully obtained access token
     Token expires in: 7200 seconds
   ```

### Option 2: Using Production

Just use your deployed production URL throughout.
