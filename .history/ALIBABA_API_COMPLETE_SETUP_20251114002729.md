...
4. **Configure Callback URL:**
   - **Development (recommended):** Use ngrok tunnel
     - Install: `npm install -g ngrok`
     - Start: `ngrok http 3007`
     - Use: `https://your-ngrok-url.ngrok-free.app/api/alibaba/callback`
   - **Production:** `https://your-domain.com/api/alibaba/callback`
   - **Important:** Must use a real HTTPS URL (localhost won't work)
5. You'll receive:
...
4. **Configure Callback URL:**
   - **Development:** Use ngrok: `https://your-ngrok-url.ngrok-free.app/api/alibaba/callback`
   - **Production:** `https://your-domain.com/api/alibaba/callback`
   - **Important:** Must use a real HTTPS URL (localhost won't work)
5. Receive App Key and App Secret

### Important: Callback URL Requirements

Alibaba requires a **publicly accessible HTTPS URL**. You cannot use `localhost` directly.
...
# Use the ngrok HTTPS URL in Alibaba portal
# Example: https://abc123.ngrok-free.app/api/alibaba/callback
```
...
- Alibaba needs to send the authorization code to your callback URL
- `localhost` is not accessible from the internet
- Even with HTTPS, self-signed certificates are rejected
- Use ngrok or deploy to production for testing
...
ALIBABA_REDIRECT_URI=https://your-ngrok-url.ngrok-free.app/api/alibaba/callback
...
**Example for Development (with ngrok):**
```env
