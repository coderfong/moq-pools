...
(async () => {
  ...
  console.log(`[upgradeIndiaMartListingImage] start count=${targets.length} headless=${headless} dry=${dry} force=${force}`);
  ...
  async function worker() {
    while (idx < targets.length) {
      ...
      try {
        ...
        // Cache locally & compute new hash path
        let final = fresh;
        try { const { localPath } = await cacheExternalImage(fresh, { preferJpgForIndiaMart: true }); if (localPath) final = localPath; } catch {}
        const newIsBlur = hasBlurHash(final);
        if (!force && newIsBlur) { skipped++; continue; }
        if (dry) {
