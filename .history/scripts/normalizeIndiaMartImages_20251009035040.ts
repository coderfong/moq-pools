...
Usage:
  pnpm ts-node scripts/normalizeIndiaMartImages.ts [--limit=500] [--force=1] [--aggressive=1] [--headless=1] [--dry=1] [--debug=1] [--allow-fallback=1]
...
(async () => {
  ...
  console.log(`[normalizeIndiaMartImages] start limit=${limit} force=${force} aggressive=${aggressive} headless=${headless} dry=${dry} debug=${debug} allowFallback=${allowFallback}`);
  ...
  for (const r of rows) {
    try {
      ...
      if (!needs) { skipped++; note('already_good'); if (debug) console.log(`[SKIP already_good] ${r.id}`); continue; }
      ...
      if (!final || isBadImageHashFromPath(final)) {
        if (!r.url) { skipped++; note('no_url'); if (debug) console.log(`[SKIP no_url] ${r.id}`); continue; }
        const candidates = await getIndiaMartDetailImageCandidates(r.url);
  if (!candidates.length) { note('no_candidates'); if (debug) console.log(`[SKIP no_candidates] ${r.id}`); }
  ...
  if (!final) { skipped++; if (!(reasons['no_candidates'] && !reasons['no_final'])) note('no_final'); continue; }
      if (isBadImageHashFromPath(final) && !force) { skipped++; note('only_bad_candidates'); continue; }
      if (dry) {
        note(isBadImageHashFromPath(final) ? 'dry_bad' : 'dry_good');
        if (debug) console.log(`[DRY] ${r.id} -> ${final}`);
      } else {
        await prisma.savedListing.update({ where: { id: r.id }, data: { image: final } });
        updated++;
        note(isBadImageHashFromPath(final) ? 'updated_bad' : 'updated_good');
