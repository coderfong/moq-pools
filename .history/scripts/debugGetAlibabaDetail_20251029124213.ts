...
import { getAlibabaDetailMeta } from '@/lib/providers/alibaba';
...
async function main() {
  ...
  for (const r of rows) {
    try {
      const meta = await getAlibabaDetailMeta(r.url);
      if (meta?.image || meta?.rating || meta?.orders) {
        ok++;
        console.log('OK', r.id, { image: meta.image, rating: meta.rating, orders: meta.orders });
      } else {
        empty++;
        console.log('EMPTY', r.id, r.url);
      }
    } catch (e) {
