...
async function main() {
  ...
  // Filter for listings that need updates (missing detailJson or empty priceTiers)
  const listingsToUpdate = listings.filter(listing => {
    const detail = (listing as any).detailJson;
    return !detail || !detail.priceTiers || detail.priceTiers.length === 0;
  });

  console.log(`${listingsToUpdate.length} listings need price tier updates\n`);
