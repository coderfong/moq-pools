...
async function testMadeInChinaSearch() {
  ...
  // Find actual product items within search results
  const productItems = $('.item, .product-item, .search-product-item, li[class*="product"]');
  console.log(`Found ${productItems.length} product items\n`);
  
  const firstCard = productItems.first();
