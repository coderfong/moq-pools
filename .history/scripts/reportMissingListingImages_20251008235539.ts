...
(async () => {
  const platform = (arg('platform') || 'INDIAMART').toUpperCase();
  const allowedPlatforms = ['C1688','ALIBABA','TAOBAO','MADE_IN_CHINA','INDIAMART','SEA_LOCAL','ALIEXPRESS'];
  if (!allowedPlatforms.includes(platform)) {
    console.error(`Invalid platform ${platform}. Allowed: ${allowedPlatforms.join(',')}`);
    process.exit(1);
  }
  ...
  const rows = await prisma.savedListing.findMany({
    where: { platform: platform as any, image: { startsWith: '/cache/' } },
