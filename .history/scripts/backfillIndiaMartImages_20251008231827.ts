...
(async () => {
  ...
  const headless = parseBool(argFlag('headless'), false);

  if (headless) process.env.IM_DETAIL_HEADLESS = '1';
  console.log(`[backfillIndiaMartImages] start limit=${limit} dry=${dry} cache=${!noCache} verbose=${verbose} includeSeed=${includeSeed} headless=${headless}`);
  ...
  for (const r of rows) {
    try {
      ...
      let finalImg = img;
      if (!noCache) {
        try { const { localPath } = await cacheExternalImage(img, { preferJpgForIndiaMart: true }); finalImg = localPath; } catch {}
      }
      if (dry) {
