...
interface Args { limit: number; terms: number; headless: boolean; resumeJson?: string; debug: boolean; overrides: string[]; minInformative: number; allowAccessories: boolean; }

function parseArgs(argv: string[]): Args {
  const args: Args = { limit: 240, terms: 2, headless: false, debug: false, overrides: [], minInformative: 2, allowAccessories: false } as any;
...
async function main() {
  ...
  console.log(`[IndiaMART Bulk] Start: leaves=${leaves.length} limit=${args.limit} termsPerLeaf=${args.terms} headless=${args.headless} debug=${args.debug} minInf=${args.minInformative} allowAcc=${args.allowAccessories}`);
