...
async function main() {
  ...
  try {
    ...
    if (e && e.code === 'ENOENT') {
      console.error(`[diagnose] cache directory not found: ${cacheDir}`);
      process.exit(1);
    }
    throw e;
  }
  ...
  if (jpgs.length === 0) {
    console.error('[diagnose] no JPGs found in cache; nothing to analyze');
    console.log(JSON.stringify({ scanned: 0, suggestions: [] }, null, 2));
    return;
  }
  let sharp: any = null;
  try {
  ...
  if (!sharp) { console.error('[diagnose] sharp not installed; abort'); process.exit(1); }
  ...
  function dHashFromLuma(luma: Float32Array, w: number, h: number): string {
    // 8x8 horizontal gradient comparisons (9 samples per row)
    ...
    for (let y = 0; y < targetH; y++) {
      for (let x = 0; x < targetW - 1; x++) {
        ...
        const g1 = luma[sy * w + sx];
        const g2 = luma[sy * w + sx2];
        bits.push(g1 < g2 ? '1' : '0');
      }
    }
    return bits.join('').padEnd(64, '0');
  }
  ...
  for (const f of jpgs.slice(0, args.limit)) {
    ...
    try {
      ...
      dhash = dHashFromLuma(lumArr, w, h);
    } catch {}
