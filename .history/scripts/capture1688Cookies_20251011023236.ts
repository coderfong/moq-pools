...
(async () => {
  ...
  let page: any = await ctx.newPage();
  ...
  // If we got bounced to login, try mobile search directly in a new context with mobile UA
  let mobileCtx: any = null;
  if (page.url().includes('login.1688.com')) {
    ...
    // Replace page reference for subsequent waits
  ...
  // Grab cookies from all active contexts and merge
  const contexts: any[] = [ctx].concat(mobileCtx ? [mobileCtx] : []);
  const all: any[] = [];
  for (const c of contexts) {
    try { all.push(...await c.cookies()); } catch {}
  }
  // Deduplicate by name
  ...
  const cookieStr = Array.from(map.entries()).map(([k, v]) => `${k}=${v}`).join('; ');
