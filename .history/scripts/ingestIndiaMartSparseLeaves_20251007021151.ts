...
interface Args { target:number; maxCurrent:number; limitLeaves:number; prefetch:number; termsCap:number; minInformative:number; headless:boolean; debug:boolean; dry:boolean; }
function parseArgs(): Args { const a = process.argv.slice(2); const r:Args = { target:4, maxCurrent:1, limitLeaves:80, prefetch:70, termsCap:5, minInformative:2, headless:true, debug:false, dry:false }; const take=()=>a.shift(); while(a.length){ const k = take(); if(!k) break; switch(k){
...
async function processLeaf(leaf: ReturnType<typeof flattenIndiaMartLeaves>[number], existing:number, args:Args){
  ...
  for(const term of baseTerms){
    ...
    const relaxStages = [ { minInf: args.minInformative, allowAcc:false }, { minInf:1, allowAcc:false }, { minInf:1, allowAcc:true } ];
...
async function main(){
  ...
  console.log(`[SPARSE] start target=${args.target} maxCurrent=${args.maxCurrent} limitLeaves=${args.limitLeaves} prefetch=${args.prefetch} termsCap=${args.termsCap} minInf=${args.minInformative} headless=${args.headless}`);
  ...
  let totalAdded=0; let processed=0;
  for(const leaf of sparse){
    const existing = counts[leaf.key]||0;
    const delta = await processLeaf(leaf, existing, args);
    totalAdded += delta; processed++;
    console.log(`[SPARSE] (${processed}/${sparse.length}) leaf=${leaf.key} existing=${existing} added=${delta} cumAdded=${totalAdded}`);
  }
