...
async function main() {
  ...
  try {
    ...
    catch { chromium = (await import('playwright-core')).chromium; }
  } catch {
    console.error('Playwright not installed. Please add devDependency "playwright" and run: npx playwright install chromium');
    process.exit(1);
  }
  ...
  const wanted = (cookies as any[]).filter((c: any) => /1688\.com$/.test(String(c.domain || '')));
  const cookieHeader = (wanted as any[]).map((c: any) => `${c.name}=${c.value}`).join('; ');
