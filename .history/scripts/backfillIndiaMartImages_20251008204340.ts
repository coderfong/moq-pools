...
(async () => {
  ...
  for (const r of rows) {
    try {
      ...
      if (!img) { if (verbose) console.log(`MISS ${r.id} (no image)`); skipped++; continue; }
      let finalImg = img;
      if (!noCache) {
        try { const { localPath } = await cacheExternalImage(img); finalImg = localPath; } catch {}
      }
      if (dry) {
        console.log(`[dry] UPDATE ${r.id} -> ${finalImg}`);
        continue;
      }
      await prisma.savedListing.update({ where: { id: r.id }, data: { image: finalImg } });
      if (verbose) console.log(`UPDATE ${r.id} -> ${finalImg}`);
      updated++;
    } catch (e) {
      if (verbose) console.log(`ERROR ${r.id} ${(e as any)?.message || e}`);
      skipped++;
    }
  }
