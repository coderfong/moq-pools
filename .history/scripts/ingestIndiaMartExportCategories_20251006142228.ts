...
async function main(){
  ...
  for (const r of rows) {
    ...
    if (!filtered.length) { if (args.debug) console.log(`[IM-EXPORT-CATS] term='${term}' all listings filtered by MOQ<=1`); continue; }
    totalListings += filtered.length;
    if (!args.dry) {
      await upsertSavedListings(filtered.map(l => ({
        platform: 'INDIAMART',
        url: l.url,
    ...
    console.log(`[IM-EXPORT-CATS] ${term} -> ${filtered.length} (cum ${totalListings})`);
  }
