...
async function main() {
  ...
  const excludeCats = parseListEnv('MIC_EXCLUDE_CATEGORIES'); // e.g. "fabrication-services,transportation-logistics"
  const modifiers = parseListEnv('MIC_MODIFIERS'); // override default modifiers
  const excludeKeywords = parseListEnv('MIC_EXCLUDE_KEYWORDS'); // e.g. "custom,services,logistics"
  ...
  for (const cat of cats) {
    ...
    for (const leaf of featured) {
      const variants = buildVariants(leaf, modifiers, excludeKeywords);
