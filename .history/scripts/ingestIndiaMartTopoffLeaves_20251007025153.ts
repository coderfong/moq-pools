...
#!/usr/bin/env ts-node
/**
 * ingestIndiaMartTopoffLeaves.ts (clean rewrite)
 * Raise sparse IndiaMART leaf counts toward a target using progressive relaxation.
 *
 * Key Flags:
 *   --min-target <n>      Target kept listings per leaf (default 8)
 *   --terms <n>           Max search terms per leaf (default 3)
 *   --prefetch <n>        Fetch size per term attempt (default 70)
 *   --limit <n>           Legacy cap per escalation (default 160, mostly unused)
 *   --concurrency <n>     Parallel leaf workers (default 4)
 *   --min-informative <n> Starting informative threshold (default 2)
 *   --allow-accessories   Start allowing accessories
 *   --relax               Enable staged relaxation (default ON)
 *   --no-relax            Disable staged relaxation
 *   --no-moq              Disable MOQ<=1 filtering (keep ultra-low MOQ listings)
 *   --relaxed             Immediate relaxed mode (minInf=1 + accessories)
 *   --headless            Allow headless escalation when static sparse
 *   --resume <file>       Progress JSON file
 *   --group-filter <s>    Restrict leaves by top-level group label substring
...
main().catch(e => { console.error(e); process.exit(1); });
  ...
  async function topoffLeaf(leafKey:string, existing:number, args:Args, progress:Progress){
    ...
    for(const term of terms){
      ...
      if(args.forceRelaxed){ relaxStages.unshift({ minInf:1, allowAcc:true }); }
      for(const stage of relaxStages){
        if(existing + keptAdd >= target) break;
...
async function main(){
    ...
    const counts = await currentLeafCounts();
    const sparse = leaves.filter(l => (counts[l.key]||0) < args.minTarget);
    console.log(`[TOPOFF] leaves total=${leaves.length} sparse=${sparse.length} target=${args.minTarget} noMoq=${args.noMoq} forceRelaxed=${args.forceRelaxed}`);
    let added=0, processed=0; const queue = sparse.map(l => l.key);
    async function worker(id:number){
      while(queue.length){
        const key = queue.shift()!; processed++;
