...
(async () => {
  ...
  // Use plain require first; fallback to eval(require) in case a bundler scenario strips it.
  let sharp: any = null;
  let sharpPrimaryError: any = null;
  try {
    ...
    try {
      ...
      if (debug) console.error('[convertIndiaMartImagesToJpg] secondary require("sharp") failed', e2);
    }
  }
  if (sharp && debug) {
    try {
      console.log('[convertIndiaMartImagesToJpg] sharp loaded versions:', (sharp as any).versions?.sharp, 'libvips', (sharp as any).versions?.vips);
    } catch {}
  }
  if (!sharp) {
    ...
    if (debug && sharpPrimaryError) {
      console.error('[convertIndiaMartImagesToJpg] primary require("sharp") error:', sharpPrimaryError);
    }
  }
