...
async function runOne(job: Job): Promise<number> {
  ...
  if (platform === 'ALL') {
    const per = Math.max(40, Math.floor(limit / 4));
    ...
    items = ([] as ExternalListing[]).concat(a, i, c, m);
  } else {
    items = await fetchFor(platform as PlatformKey, q, limit).catch(() => []);
    // For Alibaba: top-off with additional modifiers until we reach a minimum per variation
    if (platform === 'ALIBABA') {
      const targetMinEnv = Number(process.env.ALIBABA_MIN_PER || process.env.INGEST_MIN_PER || '50');
      const targetMin = Math.max(10, Math.min(200, targetMinEnv));
      if (items.length < targetMin) {
        const mods = [
          'wholesale','bulk','supplier','factory','manufacturer','oem','odm','custom','custom logo',
          'private label','ready to ship','in stock','low moq','cheap','best','2024'
        ];
        const seen = new Set<string>(items.map((it) => it.url));
        for (const m of mods) {
          if (items.length >= targetMin) break;
          const q2 = `${q} ${m}`;
          const per = Math.min(limit, 180);
          const extra = await fetchFor('ALIBABA', q2, per).catch(() => []);
