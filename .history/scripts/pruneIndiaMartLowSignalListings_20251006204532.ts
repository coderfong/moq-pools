...
interface Args { dry:boolean; limit?:number; concurrency:number; debug:boolean; aggressive:boolean; }
function parseArgs(): Args {
  const out: Args = { dry:true, limit:undefined, concurrency:6, debug:false, aggressive:false };
...
async function main() {
  ...
  console.log(`[IM-PRUNE] candidates=${candidates.length} (from ${all.length}) dry=${args.dry} aggressive=${args.aggressive}`);
