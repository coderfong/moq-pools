 ...
 */
import { SHARED_CATEGORIES } from '@/lib/sharedTaxonomy';
...
async function main() {
  ...
  const categoryDefs: { key: string; featured: string[] }[] = SHARED_CATEGORIES.map((top) => {
    const featured: string[] = [];
    // Collect leaves from first-level children and any nested children
    if (top.leaves) featured.push(...top.leaves.map((l) => l.label));
    if (top.children) {
      for (const sub of top.children) {
        if (sub.leaves) featured.push(...sub.leaves.map((l) => l.label));
        if (sub.children) {
          for (const subsub of sub.children) {
            if (subsub.leaves) featured.push(...subsub.leaves.map((l) => l.label));
          }
        }
      }
    }
    ...
    const seen = new Set<string>();
    const deduped = featured.filter((f) => (seen.has(f) ? false : (seen.add(f), true)));
    return { key: top.key, featured: deduped };
  });

  for (const cat of categoryDefs) {
    for (const leaf of (cat.featured || [])) {
