...
interface Args { limit: number; terms: number; headless: boolean; resumeJson?: string; debug: boolean; overrides: string[]; minInformative: number; allowAccessories: boolean; noMoq: boolean; relaxed: boolean; upgradeImages: boolean; cacheImages: boolean; }

function parseArgs(argv: string[]): Args {
  const args: Args = { limit: 240, terms: 2, headless: false, debug: false, overrides: [], minInformative: 2, allowAccessories: false, noMoq: false, relaxed: false, upgradeImages: true, cacheImages: true } as any;
  ...
  case '--no-moq': args.noMoq = true; break;
      ...
      case '--upgrade-images': {
        const prev = args.upgradeImages;
        args.upgradeImages = /^(1|true|yes)$/i.test(String(take()));
        if (prev !== args.upgradeImages) args.overrides.push(`--upgrade-images (changed to ${args.upgradeImages})`);
        break;
      }
      case '--cache-images': {
        const prev = args.cacheImages;
        args.cacheImages = /^(1|true|yes)$/i.test(String(take()));
        if (prev !== args.cacheImages) args.overrides.push(`--cache-images (changed to ${args.cacheImages})`);
        break;
      }
    }
  }
...
async function main() {
  ...
  console.log(`[IndiaMART Bulk] Start: leaves=${leaves.length} limit=${args.limit} termsPerLeaf=${args.terms} headless=${args.headless} debug=${args.debug} minInf=${args.minInformative} allowAcc=${args.allowAccessories} noMoq=${args.noMoq} relaxed=${args.relaxed} upgradeImages=${args.upgradeImages} cacheImages=${args.cacheImages}`);
  ...
  let items: any[] = [];
      try {
  items = await fetchIndiaMart(term, args.limit, { headless: args.headless, debug: args.debug, upgradeImages: args.upgradeImages, cacheImages: args.cacheImages });
      } catch (e) {
      ...
      try {
  let filteredMoq=0, filteredQuality=0, filteredBanned=0; const seenCanon = new Set<string>();
        const kept: any[] = [];
  for (const it of items) {
    // Keyword exclusion first
    if (isExcludedByKeywords(it.title, it.description).excluded) { filteredBanned++; continue; }
          if (!args.noMoq) {
        ...
        if (!kept.length) {
          console.log(`[filtered] ${key} all removed (banned=${filteredBanned}, moq=${filteredMoq}, quality=${filteredQuality})`);
          progress[key] = 0;
          continue;
        }
  ...
  console.log(`[ok] ${key} -> kept=${kept.length} filtered={banned:${filteredBanned}, moq:${filteredMoq}, quality:${filteredQuality}} (total=${total})`);
      } catch (e) {
