...
async function main() {
  ...
  const categoriesCsv = parseArg('categories');
  const includeCats = categoriesCsv ? categoriesCsv.split(',').map(s => s.trim()).filter(Boolean) : undefined;
  ...
  if (perSubs) {
    const cats = includeCats && includeCats.length
      ...
      : CATEGORIES;
    const listLbl = cats.map(c => c.key).join(',');
    console.log(`Running per featured subcategory across categories: [${listLbl}]`);
