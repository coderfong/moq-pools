...
function isLikelyInvalid(resp:{ ok:boolean; status:number; html:string; finalUrl:string; }): boolean {
  ...
  // If this URL itself is a /products/ landing page and we see zero product anchors, likely obsolete.
  if(/\/products\//.test(resp.finalUrl) && anchorCount === 0 && !hasMarker) return true;
