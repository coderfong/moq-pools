...
async function runOne(job: Job): Promise<number> {
  ...
  if (platform === 'ALL') {
    ...
    // For Alibaba: aggressive top-off with extended/randomized modifiers until we reach a minimum per variation
    if (platform === 'ALIBABA') {
      const targetMinEnv = Number(process.env.ALIBABA_MIN_PER || process.env.INGEST_MIN_PER || '100');
      const targetMin = Math.max(20, Math.min(400, targetMinEnv));
      if (items.length < targetMin) {
        ...
        for (const m of allMods) {
          ...
          const per = Math.min(limit, Math.max(80, Math.min(300, deficit * 5)));
