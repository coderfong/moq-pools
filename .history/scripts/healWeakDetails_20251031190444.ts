...
type SavedListingType = NonNullable<Awaited<ReturnType<NonNullable<typeof prisma>['savedListing']['findFirst']>>>;


async function healOne(listing: SavedListingType | null) {
  if (!listing) return;
  try {
    if (USE_HEADLESS) {
...
async function main() {
  ...
  const activeStatuses = ['OPEN', 'LOCKED', 'ORDER_PLACED', 'FULFILLING'];
  ...
  const sourceUrls = products.map(p => p.sourceUrl).filter(Boolean);
  ...
  const weakCandidates = await prisma.savedListing.findMany({
    where: {},
    orderBy: { id: 'asc' },
  ...
  const weakListings = weakCandidates.filter(l => {
    try {
      return BAD(l.detailJson) || isWeakDetail(l.detailJson);
    } catch { return true; }
  });
