...
async function analyzeMadeInChina() {
  ...
  const noDetailResult = await prisma.$queryRaw`
    SELECT COUNT(*) as count FROM "SavedListing"
    WHERE platform = 'MADE_IN_CHINA' AND "detailJson" IS NULL
  `;
  const noDetail = Number(noDetailResult[0].count);
  ...
  const goodAttr = await prisma.$queryRaw`
    SELECT COUNT(*) as count FROM "SavedListing"
    WHERE platform = 'MADE_IN_CHINA'
  ...
  const byAttributes = [
    ...
    { category: '15-19 attributes (good)', count: Number(goodAttr[0].count) },
