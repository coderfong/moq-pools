...
const BATCH_SIZE = 5; // Even smaller batches for better success
const DELAY_BETWEEN_BATCHES = 8000; // 8 seconds between batches (increased)
const PROGRESS_FILE = path.join(__dirname, 'alibaba-fix-progress.json');
const MAX_CONSECUTIVE_FALLBACKS = 8; // Back off early
const BLOCK_COOLDOWN_MS = 120 * 1000; // 120 seconds cooldown (increased)
const MAX_CONCURRENT_REQUESTS = 1; // Only 1 concurrent - maximum reliability
const RATE_LIMIT_BACKOFF = 12000; // 12 seconds backoff (increased)
const INTER_CHUNK_DELAY = 2000; // 2 seconds between chunks (increased)
const REQUEST_TIMEOUT = 60000; // 60 second timeout per request
...
async function fixAllAlibaba() {
  ...
  while (offset < needFixCount && !isShuttingDown) {
    ...
    const processListing = async (listing, index) => {
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
          ...
          if (!response.ok) {
            if (response.status === 429 || response.status === 503) {
              ...
              if (attempt < maxRetries - 1) {
                const backoff = Math.min(5000 * Math.pow(2, attempt), 20000);
                console.log(`⏸️  [${num}/${needFixCount}] Rate limited, waiting ${backoff/1000}s...`);
            ...
            if (attempt < maxRetries - 1) {
              const backoff = Math.min(3000 * Math.pow(2, attempt), 10000);
              await new Promise(resolve => setTimeout(resolve, backoff));
              continue;
            }
          ...
          if (attempt < maxRetries - 1) {
            const backoff = Math.min(3000 * Math.pow(2, attempt), 10000);
            console.log(`⚠️  [${num}/${needFixCount}] ${error.message}, retrying in ${backoff/1000}s...`);
            await new Promise(resolve => setTimeout(resolve, backoff));
            continue;
          }
          console.log(`❌ [${num}/${needFixCount}] ERROR after ${maxRetries} attempts: ${error.message}`);
          return { success: false, type: 'error', num };
        }
      }
    ...
    for (let i = 0; i < batch.length; i += currentConcurrency) {
      ...
      // Delay between chunks for breathing room
    ...
    // Ultra-conservative adjustment - maximum success rate
    const rateLimitPercent = rateLimitCount / batch.length;
    
    if (rateLimitPercent > 0.3) {
      consecutiveRateLimits++;
      console.log(`🔴 Throttling detected (${rateLimitCount}/${batch.length})`);
      
      // Always drop to 1
      currentConcurrency = 1;
      console.log(`⬇️  Concurrency: 1`);
      
      // Long pause
      await new Promise(resolve => setTimeout(resolve, 10000));
      
    } else if (rateLimitPercent > 0.2) {
      console.log(`🟡 Minor throttling (${rateLimitCount}/${batch.length})`);
    ...
    } else if (rateLimitPercent > 0) {
      console.log(`⚡ ${rateLimitCount} rate limits`);
      consecutiveRateLimits = 0;
      
    } else {
      // Only increase if multiple batches with zero errors
      consecutiveRateLimits = 0;
    ...
    // If any consistent issues, take extended break
    if (consecutiveRateLimits >= 2) {
      console.log(`⏸️  Multiple throttled batches, cooling down 20s...`);
