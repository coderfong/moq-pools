...
async function fixAllMadeInChina() {
  ...
  while (offset < needFixCount && !isShuttingDown) {
    ...
    } else if (rateLimitPercent > 0.15) {
      console.log(`⚡ Light throttling (${rateLimitCount}/${batch.length})`);
      consecutiveRateLimits = 0;
      
    } else {
      // Low/no rate limiting - increase concurrency
      consecutiveRateLimits = 0;
    ...
    // If consistently rate limited, take a break
    if (consecutiveRateLimits >= 4) {
      console.log(`⏸️  Consistently rate limited, cooling down 8s...`);
      await new Promise(resolve => setTimeout(resolve, 8000));
      consecutiveRateLimits = 0;
      currentConcurrency = Math.max(2, Math.floor(currentConcurrency * 0.5));
    }
