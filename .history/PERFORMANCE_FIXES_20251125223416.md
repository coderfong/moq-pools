...
- ❌ 30+ automatic title scrapes per page load
- ❌ 160+ automatic image scrapes per page load
- ❌ Background IndiaMart fetch triggering live scraping
- ❌ No caching
- ❌ No rate limiting
...
### After
- ✅ Maximum 5 concurrent scrapes (only via explicit API calls)
- ✅ **Zero automatic scrapes on page load** - website displays database data only
- ✅ 5-minute result caching for `/api/rescrape`
- ✅ 10 requests/minute rate limiting
- ✅ Expected load times: <1 second
- ✅ Controlled resource usage

**Key Change:** Website now operates as a **database-first application**. All product data is displayed from the `SavedListing` table. Scraping only occurs when explicitly triggered via:
- `/api/rescrape` endpoint (rate-limited, queued)
- Background scripts like `fix-all-alibaba-client.js` (manual execution)
