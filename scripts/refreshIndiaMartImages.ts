...
// Argument helpers support forms:
//   --name=value   (preferred)
//   --name value   (space separated)
//   --name         (implies true for boolean flags)
...
(async () => {
  ...
  const needUpgrade = (img: string | null | undefined): boolean => {
    ...
    if (/\.png$/i.test(img)) return true; // prefer jpeg re-fetch
    if (isBadImageHashFromPath(img)) return true; // explicitly flagged bad hash
    return false;
  };
