...
function isLikelyInvalid(resp:{ ok:boolean; status:number; html:string; finalUrl:string; }): boolean {
  ...
  if(html.length < 800) return true; // extremely short / placeholder

  // Quick negative phrases indicating no products / dead page
  const emptyPhrases = /(No results found|No Result Found|No product found|did not match any product|0\s+results?\b|Try broadening your search|We couldn't find)/i;
  if (emptyPhrases.test(html)) return true;
  // Explicit unavailable page phrase provided by user
  if (/Oh no!\s*It seems this page is not available/i.test(html)) return true;

  // Generic title detection
  const m = html.match(/<title>([^<]{0,160})<\/title>/i); const title = (m?.[1] || '').trim();
  ...
  // DOM-based heuristic for product anchors (avoid counting nav/footer):
  let anchorCount = 0;
  try {
    const $ = cheerio.load(html);
    anchorCount = $("a[href*='/products/?id='], a[href*='/proddetail/'], a[href*='/product/']").length;
  } catch { /* ignore */ }

  const isProductsId = /\/products\/.*[?&]id=/.test(resp.finalUrl);
  // If this URL itself is a /products/?id= landing page and we see zero product anchors, likely obsolete.
  if(isProductsId && anchorCount === 0 && !hasMarker) return true;

  if (isProductsId) {
    // Stricter signals for individual product landing.
    const $ = (() => { try { return cheerio.load(html); } catch { return null; } })();
    let hasCurrency = /(?:₹|Rs\.?|INR)/i.test(html);
    let hasImage = false;
    let hasContact = false;
    if ($) {
      hasImage = $('img').filter((_,el)=>{
        const src = (el.attribs?.src||'') + (el.attribs?.['data-src']||'');
        return /product|prodimg|imghost/i.test(src);
      }).length > 0;
      hasContact = /contact supplier|send enquiry|send inquiry|company video/i.test(html);
    }
    // If none of the strong signals present, treat as invalid.
    if(!hasCurrency && !hasImage && !hasContact) return true;
  }

  // If no marker and short-ish page, treat invalid.
