...
*/
// Use require style to stay friendly with ts-node CommonJS config
...
// Single prisma client instance (renamed to avoid any ambient name collision)
...
async function main() {
  ...
  for (const l of listings) {
    ...
    if (argv.write) {
      // dedupe check
  const existing = await db.savedListing.findFirst({ where: { platform: 'INDIAMART', url: canon } });
      if (existing) {
  await db.savedListing.delete({ where: { id: l.id } });
        deleted++;
        console.log(`[deleted duplicate] ${l.id} -> existing ${existing.id} ${canon}`);
        continue;
      }
  await db.savedListing.update({ where: { id: l.id }, data: { url: canon } });
    }
    changed++;
