...
import { searchC1688NoLogin } from '@/lib/providers/c1688';
...
async function main() {
  ...
  if (singleQ) {
    ...
    cats = JSON.parse(raw) as Record<string, string[]>;
  }

  // If cookies are not provided via env, try to read from scripts/cookies.1688.txt
  try {
    if (!process.env.C1688_COOKIES) {
      const cookiePath = path.resolve('scripts/cookies.1688.txt');
      const cookieHeader = await fs.readFile(cookiePath, 'utf8').then(s => s.trim()).catch(() => '');
      if (cookieHeader) process.env.C1688_COOKIES = cookieHeader;
    }
  } catch {}
  ...
  const chunk = await searchC1688NoLogin(q, p);
