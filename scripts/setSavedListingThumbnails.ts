...
async function loadMappings(filePath: string): Promise<Array<{ id?: string; url: string; image: string }>> {
  const raw = await fs.readFile(filePath, 'utf8');
  if (/\.json$/i.test(filePath)) {
    const arr = JSON.parse(raw);
    if (!Array.isArray(arr)) throw new Error('JSON must be an array');
    return arr.map((o) => ({ id: o.id ? String(o.id) : undefined, url: String(o.url || ''), image: String(o.image || '') }))
  ...
  if ((urlIdx < 0 && idIdx < 0) || imgIdx < 0) throw new Error('CSV header must include (id or url) and image');
  const out: Array<{ id?: string; url: string; image: string }> = [];
  for (let i = 1; i < lines.length; i++) {
    const cols = lines[i].split(/[\t,]/);
    const id = idIdx >= 0 ? (cols[idIdx] || '').trim() : '';
    const url = urlIdx >= 0 ? (cols[urlIdx] || '').trim() : '';
    const image = (cols[imgIdx] || '').trim();
    if ((id || url) && image) out.push({ id: id || undefined, url, image });
  }
  return out;
}
...
async function main() {
  ...
  for (const m of mappings) {
    try {
      ...
      if (m.id) {
        row = await p.savedListing.findUnique({ where: { id: m.id }, select: { id: true } });
      } else if (m.url) {
        row = await p.savedListing.findUnique({ where: { url: m.url }, select: { id: true } });
      }
      ...
      if (dryRun) {
        console.log(`[dry-run] would update ${m.id || m.url} -> ${localPath}`);
        continue;
      }
