...
const { shouldExcludeItem, processListingShipping } = require('../src/lib/shipping.ts');

// Shipping filter function
function shouldExcludeForShipping(listing) {
  const title = listing.title || '';
  const description = listing.description || '';
  
  // Check for excluded keywords (vehicles, heavy machinery, etc.)
  const excludedKeywords = [
    'car', 'truck', 'vehicle', 'motorcycle', 'atv', 'boat', 'yacht',
    'trailer', 'rv', 'camper', 'bus', 'van', 'excavator', 'bulldozer',
    'crane', 'forklift', 'tractor', 'combine', 'harvester', 'compactor',
    'sofa', 'couch', 'bed', 'mattress', 'wardrobe', 'cabinet',
    'washing machine', 'dryer', 'refrigerator', 'freezer', 'dishwasher'
  ];
    ...
    while (true) {
      ...
      for (let i = 0; i < listings.length; i += BATCH_SIZE) {
        ...
        for (const listing of chunk) {
          ...
          // Check if item should be excluded for shipping
          if (shouldExcludeForShipping(listing)) {
            console.log(`    ⚠️  Skipping - too large for shipping`);
            progress.offset++;
            continue;
          }
