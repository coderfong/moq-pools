...
export default function ContactForm() {
  ...
  // Prefill from /api/me and URL query params
  useEffect(() => {
    let cancelled = false;
    const applyQuery = () => {
      try {
        const usp = new URLSearchParams(window.location.search);
        const qOrder = usp.get("orderId");
        const qSubject = usp.get("subject") as SubjectOption | null;
        if (qOrder && !orderId) setOrderId(qOrder);
        if (qSubject && ("General Inquiry|Order Help|Shipping Issue|Supplier Partnership|Other" as string).includes(qSubject)) {
          setSubject(qSubject);
        }
      } catch {}
    };
    const loadMe = async () => {
      try {
        ...
        if (!res.ok) return applyQuery();
        const data = await res.json();
        if (!cancelled && data?.ok && data?.user) {
          if (!name) setName(data.user.firstName || data.user.name || "");
          if (!email) setEmail(data.user.email || "");
        }
      } catch {}
      finally { if (!cancelled) applyQuery(); }
    };
    loadMe();
  ...
  return (
    <form ref={formRef} onSubmit={onSubmit} className="space-y-3">
      ...
      <div className="hidden" aria-hidden>
        <label className="block text-sm" htmlFor="hp-field">Company</label>
        <input id="hp-field" name={honeypotName} type="text" autoComplete="off" placeholder="Leave blank" title="Leave this field empty" className="mt-1 w-full rounded-md border border-neutral-200 bg-white px-3 py-2 text-sm" />
      </div>
