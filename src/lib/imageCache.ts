...
export async function cacheExternalImage(rawSrc: string, opts?: { preferJpgForIndiaMart?: boolean }): Promise<{ localPath: string; absPath: string }> {
  ...
  // BUT for Made-in-China WebP images, convert to JPG since WebP versions are low-res
  const isMadeInChinaWebP = decidedExt === 'webp' && (url.hostname.includes('made-in-china.com') || url.hostname.includes('micstatic.com'));
  
  // Convert Made-in-China WebP to JPG (their WebP is low-res, JPG is full-res)
  if (isMadeInChinaWebP && sharp) {
    try {
      destExt = 'jpg';
      dest = path.join(outDir, `${hash}.jpg`);
      // Convert WebP to high-quality JPG
      await sharp(buf).flatten({ background: { r: 255, g: 255, b: 255 } }).jpeg({ quality: 85 }).toFile(dest);
      return { localPath: `/cache/${hash}.jpg`, absPath: dest };
    } catch {
