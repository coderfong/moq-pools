...
async function tryParse(html: string, base = 'https://s.taobao.com'): Promise<ExternalListing[]> {
  ...
  out.push({ platform: 'TAOBAO', title, image, price, url, storeName, orders } as ExternalListing);
  });
  return out;
}
    ...
    for (let round = 0; round < 20 && items.length < limit; round++) {
      const batch = await page.evaluate(() => {
        ...
        for (const a of anchors) {
          ...
          out.push({ platform: 'TAOBAO', title, image, price, url, orders, storeName } as ExternalListing);
        }
        return out;
      });
