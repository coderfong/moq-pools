...
function normalizeImage(src = '', base = 'https://www.made-in-china.com') {
  ...
  else u = `${base}${u.startsWith('/') ? '' : '/'}${u}`;
  
  // Upgrade Made-in-China CDN images to higher resolution/quality
  // Pattern: image.made-in-china.com/xxxxx.webp or similar
  if (u.includes('image.made-in-china.com') || u.includes('micstatic.com')) {
    try {
      const url = new URL(u);
      // Remove size/quality params if they exist (like _220x220, _small, etc)
      url.pathname = url.pathname.replace(/_\d+x\d+/g, '').replace(/_small|_medium|_thumb/gi, '');
      
      // Convert small WebP images to high-quality JPEG
      // Made-in-China serves WebP at low resolution, but JPG at full resolution
      if (url.pathname.endsWith('.webp')) {
        // Replace .webp with .jpg for full resolution
        url.pathname = url.pathname.replace(/\.webp$/i, '.jpg');
        // Request large size
        url.pathname = url.pathname.replace(/(\.[^.]+)$/, '_640x640$1');
      } else if (url.pathname.endsWith('.jpg') || url.pathname.endsWith('.jpeg')) {
        // For JPG, request 640x640 size if not already sized
        if (!/_\d+x\d+/.test(url.pathname)) {
          url.pathname = url.pathname.replace(/(\.[^.]+)$/, '_640x640$1');
        }
      }
